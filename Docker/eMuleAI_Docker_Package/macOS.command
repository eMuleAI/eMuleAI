...
attach_xpra() {
	...
	if [ -z "${xpra_client}" ] || [ ! -x "${xpra_client}" ]; then
		printf 'WARNING: Xpra client was not found. Install Xpra or set XPRA_CLIENT_EXE_MACOS.\n' >&2
		return 0
	fi
...
for required_key in \
	IMAGE_SOURCE REGISTRY_IMAGE REGISTRY_TAG LOCAL_IMAGE LOCAL_TAG \
	CLEAN_OLD_IMAGES CONTAINER_NAME PLATFORM DISPLAY_MODE DATA_DIR_MACOS \
	PUID_MACOS PGID_MACOS DISPLAY_BIND_ADDRESS DISPLAY_CONNECT_ADDRESS EMULE_BIND_ADDRESS \
...
DATA_DIR="$(expand_home_path "${DATA_DIR_MACOS}")"
PUID="$(resolve_id "${PUID_MACOS}" "$(id -u)")"
PGID="$(resolve_id "${PGID_MACOS}" "$(id -g)")"
XPRA_CLIENT_EXE="$(expand_home_path "${XPRA_CLIENT_EXE_MACOS:-}")"
...
is_id "${PUID}" || fail "PUID_MACOS must be auto or an integer between 0 and 2147483647."
is_id "${PGID}" || fail "PGID_MACOS must be auto or an integer between 0 and 2147483647."
