...
void LogCertificate(PCCERT_CONTEXT pCertContext)
{
	...
	if (pCertInfo) {
		if (CertNameToStr(X509_ASN_ENCODING | PKCS_7_ASN_ENCODING, &pCertInfo->Subject, CERT_X500_NAME_STR, szString, _countof(szString)))
			DebugLog(LOG_DONTNOTIFY, _T("Email Encryption: Subject: %s"), (LPCTSTR)EscPercent(szString));

		if (pCertInfo->SerialNumber.cbData && pCertInfo->SerialNumber.pbData)
			DebugLog(LOG_DONTNOTIFY, _T("Email Encryption: Serial nr.: %s"), (LPCTSTR)EscPercent(GetCertInteger(pCertInfo->SerialNumber.pbData, pCertInfo->SerialNumber.cbData)));
...
bool Encrypt(const CStringA &rstrContentA, CByteArray &raEncrypted, LPCWSTR pwszCertSubject)
{
	...
	if (!CryptAcquireContext(&hCryptProv, pszContainer, NULL, PROV_RSA_FULL, NULL)) {
		...
		if (dwError != (DWORD)NTE_BAD_KEYSET) {
			DebugLogWarning(LOG_DONTNOTIFY, _T("Email Encryption: Failed to acquire certificate context container '%s' - %s"), pszContainer, (LPCTSTR)EscPercent(GetErrorMessage(dwError, 1)));
			return false;
		}
		if (!CryptAcquireContext(&hCryptProv, pszContainer, NULL, PROV_RSA_FULL, CRYPT_NEWKEYSET)) {
			DebugLogWarning(LOG_DONTNOTIFY, _T("Email Encryption: Failed to create certificate context container '%s' - %s"), pszContainer, (LPCTSTR)EscPercent(GetErrorMessage(::GetLastError(), 1)));
			return false;
		}
	}
			...
			if (CryptEncryptMessage(&EncryptParams, 1, &pRecipientCert, (BYTE*)(LPCSTR)rstrContentA, rstrContentA.GetLength(), NULL, &cbEncryptedBlob)) {
				try {
					...
					if (!CryptEncryptMessage(&EncryptParams, 1, &pRecipientCert, (BYTE*)(LPCSTR)rstrContentA, rstrContentA.GetLength(), raEncrypted.GetData(), &cbEncryptedBlob)) {
						DebugLogWarning(LOG_DONTNOTIFY, _T("Email Encryption: Failed to encrypt message - %s"), (LPCTSTR)EscPercent(GetErrorMessage(::GetLastError(), 1)));
						raEncrypted.SetSize(0);
					}
				} catch (CMemoryException *ex) {
					ex->Delete();
					DebugLogWarning(LOG_DONTNOTIFY, _T("Email Encryption: Failed to encrypt message - %s"), (LPCTSTR)EscPercent(_tcserror(ENOMEM)));
				}
			} else
				DebugLogWarning(LOG_DONTNOTIFY, _T("Email Encryption: Failed to get length of encrypted message - %s"), (LPCTSTR)EscPercent(GetErrorMessage(::GetLastError(), 1)));

			VERIFY(CertFreeCertificateContext(pRecipientCert));
		} else
			DebugLogWarning(LOG_DONTNOTIFY, _T("Email Encryption: Failed to find certificate with subject '%ls' - %s"), (LPCTSTR)EscPercent(pwszCertSubject), (LPCTSTR)EscPercent(GetErrorMessage(::GetLastError(), 1)));

		VERIFY(CertCloseStore(hStoreHandle, 0));
	} else
		DebugLogWarning(LOG_DONTNOTIFY, _T("Email Encryption: Failed to open certificate store '%s' - %s"), pszCertStore, (LPCTSTR)EscPercent(GetErrorMessage(::GetLastError(), 1)));
...
bool encoded_word(const CString &src, CStringA &dst)
{
	...
	{
		dst.ReleaseBuffer(0);
		DebugLogWarning(LOG_DONTNOTIFY, _T("'%s' to base64 failed"), (LPCTSTR)EscPercent(src));
		return false;
	}
...
exit:
	...
	if (pmsg)
		DebugLogError(_T("Error: %s returned -0x%04x - %s"), (LPCTSTR)EscPercent(pmsg), -ret, (LPCTSTR)EscPercent(SSLerror(ret)));
}
...
void CemuleDlg::SendNotificationMail(TbnMsg nMsgType, LPCTSTR pszText)
{
	...
	if (pThread) {
		mail.sEncryptCertName = CPreferences::sNotifierMailEncryptCertName;

		pThread->m_mail = mail;
		pThread->m_strSubject = GetResString(_T("EMULENOTIFICATION"));
		LPCTSTR uid;
		switch (nMsgType) {
		case TBN_CHAT:
			uid = _T("PW_TBN_POP_ALWAYS");
			break;
		case TBN_DOWNLOADFINISHED:
			uid = _T("PW_TBN_ONDOWNLOAD");
			break;
		case TBN_DOWNLOADADDED:
			uid = _T("TBN_ONNEWDOWNLOAD");
			break;
		case TBN_LOG:
			uid = _T("PW_TBN_ONLOG");
			break;
