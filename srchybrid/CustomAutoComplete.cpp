...
#include "Emule.h" // Ensure that backup process isn't locking config files.
...
{
	CString strTempFileName = CString(pszFileName) + _T(".tmp");
	FILE* fp = _tfsopen(strTempFileName, _T("wb"), _SH_DENYWR);
	...
	bool m_bClose = fclose(fp);
	MoveFileEx(strTempFileName, pszFileName, MOVEFILE_REPLACE_EXISTING);
	return m_bClose ? false : ret;
}
