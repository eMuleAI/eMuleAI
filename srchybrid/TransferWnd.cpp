...
static const UINT s_auClientFilterCheckboxes[] = {
	IDC_CHECK_FREEEZE,
	IDC_CHECK_ARCHIVED,
	IDC_CHECK_QUERYABLE,
	IDC_CHECK_NOT_QUERIED,
	IDC_CHECK_VALIDIP,
	IDC_CHECK_HIGHID,
	IDC_CHECK_BADCLIENT
};
...
BEGIN_MESSAGE_MAP(CTransferWnd, CResizableFormView)
	...
	ON_BN_CLICKED(IDC_CHECK_FREEEZE, OnBnClickedFreeze)
	ON_BN_CLICKED(IDC_CHECK_ARCHIVED, OnBnClickedArchived)
	ON_BN_CLICKED(IDC_CHECK_QUERYABLE, OnBnClickedQueryable)
	ON_BN_CLICKED(IDC_CHECK_NOT_QUERIED, OnBnClickedQueried)
	ON_BN_CLICKED(IDC_CHECK_VALIDIP, OnBnClickedValidIP)
	ON_BN_CLICKED(IDC_CHECK_HIGHID, OnBnClickedHighId)
	ON_BN_CLICKED(IDC_CHECK_BADCLIENT, OnBnClickedBadClient)
END_MESSAGE_MAP()
...
CTransferWnd::CTransferWnd(CWnd* /*pParent =NULL*/)
	...
	, m_iPendingQueueCount(0)
	, m_bPendingQueueCountValue(false)
	, m_bPendingQueueCountForce(false)
	, m_bPendingCatTabTitleForce(false)
	, m_lQueueCountUpdatePending(0)
	, m_lCatTabTitleUpdatePending(0)
	, m_lCatTabInfoInvalidatePending(0)
	, m_nFilterColumnDownloadList()
	, m_nFilterColumnUploadList()
	, m_nFilterColumnDownloadClients()
	, m_nFilterColumnQueueList()
	, m_nFilterColumnClientList()
	, m_astrFilterTemp()
	, m_nFilterColumnLastAppliedDownloadList()
	, m_nFilterColumnLastAppliedUploadList()
	, m_nFilterColumnLastAppliedDownloadClients()
	, m_nFilterColumnLastAppliedQueueList()
	, m_nFilterColumnLastAppliedClientList()
	, m_bColumnDiff(false)
	, m_strFullFilterExpr()
{
	m_dlTab.m_bClosable = false;
}

CTransferWnd::~CTransferWnd()
{
	m_ctlListHeaderDownloadList.Detach();
	m_ctlListHeaderUploadList.Detach();
	m_ctlListHeaderDownloadClients.Detach();
	m_ctlListHeaderQueueList.Detach();
	m_ctlListHeaderClientList.Detach();
...
void CTransferWnd::OnInitialUpdate()
{
	...
	AddOrReplaceAnchor(this, IDC_QUEUELIST, ANCHOR(0, thePrefs.GetSplitterbarPosition()), BOTTOM_RIGHT);
	AddOrReplaceAnchor(this, IDC_CLIENTLIST, ANCHOR(0, thePrefs.GetSplitterbarPosition()), BOTTOM_RIGHT);
	AddOrReplaceAnchor(this, IDC_DOWNLOADCLIENTS, ANCHOR(0, thePrefs.GetSplitterbarPosition()), BOTTOM_RIGHT);
	AddOrReplaceAnchor(this, IDC_QUEUECOUNT, BOTTOM_LEFT);
	AddOrReplaceAnchor(this, IDC_QUEUECOUNT_LABEL, BOTTOM_LEFT);
	AddOrReplaceAnchor(this, IDC_QUEUE_REFRESH_BUTTON, BOTTOM_RIGHT);
	AddOrReplaceAnchor(this, IDC_DLTAB, TOP_RIGHT);
	AddOrReplaceAnchor(this, IDC_FILETYPE, TOP_RIGHT);
	AddOrReplaceAnchor(this, IDC_CHECK_PREVIEW, TOP_RIGHT);
	AddOrReplaceAnchor(this, IDC_CHECK_FREEEZE, TOP_RIGHT);
	AddOrReplaceAnchor(this, IDC_CHECK_ARCHIVED, TOP_RIGHT);
	AddOrReplaceAnchor(this, IDC_CHECK_QUERYABLE, TOP_RIGHT);
	AddOrReplaceAnchor(this, IDC_CHECK_NOT_QUERIED, TOP_RIGHT);
	AddOrReplaceAnchor(this, IDC_CHECK_VALIDIP, TOP_RIGHT);
	AddOrReplaceAnchor(this, IDC_CHECK_HIGHID, TOP_RIGHT);
	AddOrReplaceAnchor(this, IDC_CHECK_BADCLIENT, TOP_RIGHT);
	AddOrReplaceAnchor(this, IDC_FILTER_DOWNLOAD_LIST, TOP_RIGHT);
	AddOrReplaceAnchor(this, IDC_FILTER_UPLOAD_LIST, TOP_RIGHT);
	AddOrReplaceAnchor(this, IDC_FILTER_QUEUE_LIST, TOP_RIGHT);
	AddOrReplaceAnchor(this, IDC_FILTER_CLIENT_LIST, TOP_RIGHT);
	AddOrReplaceAnchor(this, IDC_FILTER_DOWNLOAD_CLIENTS, TOP_RIGHT);
	...
	// Initialize active download cache for performance optimization
	...
	m_ctlListHeaderDownloadList.Attach(downloadlistctrl.GetHeaderCtrl()->Detach());
	m_ctlFilterDownloadList.OnInit(&m_ctlListHeaderDownloadList);
	...
	m_ctlFilterClientList.OnInit(&m_ctlListHeaderClientList);

	CheckDlgButton(IDC_CHECK_PREVIEW, thePrefs.m_uPreviewCheckState);
	CheckDlgButton(IDC_CHECK_FREEEZE, thePrefs.m_bFreezeChecked);
	CheckDlgButton(IDC_CHECK_ARCHIVED, thePrefs.m_uArchivedCheckState);
	CheckDlgButton(IDC_CHECK_QUERYABLE, thePrefs.m_uQueryableCheckState);
	CheckDlgButton(IDC_CHECK_NOT_QUERIED, thePrefs.m_uNotQueriedCheckState);
	CheckDlgButton(IDC_CHECK_VALIDIP, thePrefs.m_uValidIPCheckState);
	CheckDlgButton(IDC_CHECK_HIGHID, thePrefs.m_uHighIdCheckState);
...
void CTransferWnd::UpdateClientFilterCheckboxVisibility()
{
	const bool bClientListVisible = m_dwShowListIDC == IDC_CLIENTLIST && m_btnWnd1.GetSafeHwnd() != NULL;
	int nMinimumLeft = 0;
	if (bClientListVisible) {
		CRect rcToolbar;
		m_btnWnd1.GetWindowRect(&rcToolbar);
		ScreenToClient(&rcToolbar);
		nMinimumLeft = rcToolbar.right + CLIENT_FILTER_TOOLBAR_GAP;
	}
	...
	for (size_t i = 0; i < _countof(s_auClientFilterCheckboxes); ++i) {
		...
		if (bClientListVisible) {
			...
			pCheckBox->GetWindowRect(&rcCheckBox);
			ScreenToClient(&rcCheckBox);
			bShow = rcCheckBox.left >= nMinimumLeft;
		}
...
void CTransferWnd::ShowQueueCount(INT_PTR number)
{
	if (GetCurrentThreadId() != g_uMainThreadId) {
		PostQueueCountUpdateAsync(true, number, false);
		return;
	}
...
{
	if (GetCurrentThreadId() != g_uMainThreadId) {
		PostQueueCountUpdateAsync(false, 0, bForceImmediateUpdate);
		return;
	}
...
void CTransferWnd::UpdateSplitterRange()
{
	...
	AddOrReplaceAnchor(this, m_btnWnd2, ANCHOR(0, thePrefs.GetSplitterbarPosition()));
	AddOrReplaceAnchor(this, IDC_DOWNLOADLIST, TOP_LEFT, ANCHOR(100, thePrefs.GetSplitterbarPosition()));
	AddOrReplaceAnchor(this, IDC_UPLOADLIST, ANCHOR(0, thePrefs.GetSplitterbarPosition()), BOTTOM_RIGHT);
	AddOrReplaceAnchor(this, IDC_QUEUELIST, ANCHOR(0, thePrefs.GetSplitterbarPosition()), BOTTOM_RIGHT);
	AddOrReplaceAnchor(this, IDC_CLIENTLIST, ANCHOR(0, thePrefs.GetSplitterbarPosition()), BOTTOM_RIGHT);
...
void CTransferWnd::OnSize(UINT nType, int cx, int cy)
{
	CResizableFormView::OnSize(nType, cx, cy);
	if (m_dlTab.GetSafeHwnd() == NULL || m_btnWnd1.GetSafeHwnd() == NULL) {
		m_bCatTabLayoutPending = true;
		return;
	}
	UpdateClientFilterCheckboxVisibility();
...
LRESULT CTransferWnd::OnChangeFilter(WPARAM wParam, LPARAM lParam)
{
	...
	if (!m_bForceApplyFilter) {
		...
		case IDC_DOWNLOADLIST:
			m_nFilterColumnDownloadList = m_nFilterColumnTemp;
			break;
		case IDC_UPLOADLIST:
			m_nFilterColumnUploadList = m_nFilterColumnTemp;
			break;
		case IDC_DOWNLOADCLIENTS:
			m_nFilterColumnDownloadClients = m_nFilterColumnTemp;
			break;
		case IDC_QUEUELIST:
			m_nFilterColumnQueueList = m_nFilterColumnTemp;
			break;
		case IDC_CLIENTLIST:
			m_nFilterColumnClientList = m_nFilterColumnTemp;
		}
		...
		case IDC_UPLOADLIST + IDC_DOWNLOADLIST:
		case IDC_DOWNLOADLIST:
			m_ctlFilterDownloadList.GetWindowText(m_strFullFilterExpr);
			break;
		case IDC_UPLOADLIST:
			m_ctlFilterUploadList.GetWindowText(m_strFullFilterExpr);
			break;
		case IDC_DOWNLOADCLIENTS:
			m_ctlFilterDownloadClients.GetWindowText(m_strFullFilterExpr);
			break;
		case IDC_QUEUELIST:
			m_ctlFilterQueueList.GetWindowText(m_strFullFilterExpr);
			break;
		case IDC_CLIENTLIST:
			m_ctlFilterClientList.GetWindowText(m_strFullFilterExpr);
		}
	}
	...
	case IDC_UPLOADLIST + IDC_DOWNLOADLIST:
	{
		bool bColumnDiff = (m_nFilterColumnDownloadList != m_nFilterColumnLastAppliedDownloadList);
		m_nFilterColumnLastAppliedDownloadList = m_nFilterColumnDownloadList;
		bool bFilterDiff = (m_astrFilterTemp.GetCount() != m_astrFilterDownloadList.GetCount());

		if (!bFilterDiff)
			for (INT_PTR i = m_astrFilterTemp.GetCount(); --i >= 0;)
				if (m_astrFilterTemp[i] != m_astrFilterDownloadList[i]) {
					bFilterDiff = true;
					break;
				}
		...
		if (m_bColumnDiff || bFilterDiff || m_bForceApplyFilter) {
			m_astrFilterDownloadList.RemoveAll();
			m_astrFilterDownloadList.Append(m_astrFilterTemp);
			downloadlistctrl.ReloadList(false, LSF_SELECTION);
			UpdateListCount();
			InvalidateCatTabInfo();
			UpdateCatTabTitlesIfDirty(false);
		}
	...
	case IDC_UPLOADLIST:
	{
		bool bColumnDiff = (m_nFilterColumnUploadList != m_nFilterColumnLastAppliedUploadList);
		m_nFilterColumnLastAppliedUploadList = m_nFilterColumnUploadList;
		bool bFilterDiff = (m_astrFilterTemp.GetCount() != m_astrFilterUploadList.GetCount());

		if (!bFilterDiff)
			for (INT_PTR i = m_astrFilterTemp.GetCount(); --i >= 0;)
				if (m_astrFilterTemp[i] != m_astrFilterUploadList[i]) {
					bFilterDiff = true;
					break;
				}
		...
		// Added m_bForceApplyFilter to force filtering with enter/return keys
		if (m_bColumnDiff || bFilterDiff || m_bForceApplyFilter) {
			m_astrFilterUploadList.RemoveAll();
			m_astrFilterUploadList.Append(m_astrFilterTemp);
			uploadlistctrl.UpdateView();
		}
	...
	case IDC_DOWNLOADCLIENTS:
	{
		bool bColumnDiff = (m_nFilterColumnDownloadClients != m_nFilterColumnLastAppliedDownloadClients);
		m_nFilterColumnLastAppliedDownloadClients = m_nFilterColumnDownloadClients;
		bool bFilterDiff = (m_astrFilterTemp.GetCount() != m_astrFilterDownloadClients.GetCount());

		if (!bFilterDiff)
			for (INT_PTR i = m_astrFilterTemp.GetCount(); --i >= 0;)
				if (m_astrFilterTemp[i] != m_astrFilterDownloadClients[i]) {
					bFilterDiff = true;
					break;
				}
		...
		// Added m_bForceApplyFilter to force filtering with enter/return keys
		if (m_bColumnDiff || bFilterDiff || m_bForceApplyFilter) {
			m_astrFilterDownloadClients.RemoveAll();
			m_astrFilterDownloadClients.Append(m_astrFilterTemp);
			downloadclientsctrl.UpdateView();
		}
	...
	case IDC_QUEUELIST:
	{
		bool bColumnDiff = (m_nFilterColumnQueueList != m_nFilterColumnLastAppliedQueueList);
		m_nFilterColumnLastAppliedQueueList = m_nFilterColumnQueueList;
		bool bFilterDiff = (m_astrFilterTemp.GetCount() != m_astrFilterQueueList.GetCount());

		if (!bFilterDiff)
			for (INT_PTR i = m_astrFilterTemp.GetCount(); --i >= 0;)
				if (m_astrFilterTemp[i] != m_astrFilterQueueList[i]) {
					bFilterDiff = true;
					break;
				}
		...
		// Added m_bForceApplyFilter to force filtering with enter/return keys
		if (m_bColumnDiff || bFilterDiff || m_bForceApplyFilter) {
			m_astrFilterQueueList.RemoveAll();
			m_astrFilterQueueList.Append(m_astrFilterTemp);
			queuelistctrl.UpdateView();
		}
	...
	case IDC_CLIENTLIST:
	{
		bool bColumnDiff = (m_nFilterColumnClientList != m_nFilterColumnLastAppliedClientList);
		m_nFilterColumnLastAppliedClientList = m_nFilterColumnClientList;
		bool bFilterDiff = (m_astrFilterTemp.GetCount() != m_astrFilterClientList.GetCount());

		if (!bFilterDiff)
			for (INT_PTR i = m_astrFilterTemp.GetCount(); --i >= 0;)
				if (m_astrFilterTemp[i] != m_astrFilterClientList[i]) {
					bFilterDiff = true;
					break;
				}
		...
		// Added m_bForceApplyFilter to force filtering with enter/return keys
		if (m_bColumnDiff || bFilterDiff || m_bForceApplyFilter) {
			m_astrFilterClientList.RemoveAll();
			m_astrFilterClientList.Append(m_astrFilterTemp);
			clientlistctrl.ReloadList(false, LSF_SELECTION);
			UpdateListCount();
		}
...
BOOL CTransferWnd::PreTranslateMessage(MSG *pMsg)
{
	...
	case WM_CHAR: //This will direct text entries to edit box
	{
		...
		if (GetKeyState(VK_CONTROL) >= 0 && GetKeyState(VK_SPACE) >= 0 && (!pFocus || !pFocus->IsKindOf(RUNTIME_CLASS(CEdit)))) {
			...
			case IDC_UPLOADLIST + IDC_DOWNLOADLIST:
			case IDC_DOWNLOADLIST:
				m_ctlFilterDownloadList.SetFocus();
				m_ctlFilterDownloadList.SetWindowTextW(EMPTY);
				m_ctlFilterDownloadList.SendMessage(pMsg->message, pMsg->wParam, pMsg->lParam);
				return true;
			case IDC_UPLOADLIST:
				m_ctlFilterUploadList.SetFocus();
				m_ctlFilterUploadList.SetWindowTextW(EMPTY);
				m_ctlFilterUploadList.SendMessage(pMsg->message, pMsg->wParam, pMsg->lParam);
				return true;
			case IDC_DOWNLOADCLIENTS:
				m_ctlFilterDownloadClients.SetFocus();
				m_ctlFilterDownloadClients.SetWindowTextW(EMPTY);
				m_ctlFilterDownloadClients.SendMessage(pMsg->message, pMsg->wParam, pMsg->lParam);
				return true;
			case IDC_QUEUELIST:
				m_ctlFilterQueueList.SetFocus();
				m_ctlFilterQueueList.SetWindowTextW(EMPTY);
				m_ctlFilterQueueList.SendMessage(pMsg->message, pMsg->wParam, pMsg->lParam);
				return true;
			case IDC_CLIENTLIST:
				m_ctlFilterClientList.SetFocus();
				m_ctlFilterClientList.SetWindowTextW(EMPTY);
				m_ctlFilterClientList.SendMessage(pMsg->message, pMsg->wParam, pMsg->lParam);
				return true;
			}
		}
	}
...
void CTransferWnd::UpdateListCount()
{
	if (!m_btnWnd1.IsWindowVisible())
		return; // If m_btnWnd1 is not visible, we don't need to proceed
	...
	if (m_btnWnd2.IsWindowVisible() && m_dwShowListIDC == IDC_DOWNLOADLIST + IDC_UPLOADLIST) {
		switch (m_uWnd2) {
		case wnd2Uploading:
			m_uTotal = uploadlistctrl.m_ListItemsMap.size();
			m_uListed = uploadlistctrl.GetItemCount();
			resStringID = _T("UPLOADING");
			break;
		case wnd2OnQueue:
			m_uTotal = queuelistctrl.m_ListItemsMap.size();
			m_uListed = queuelistctrl.GetItemCount();
			resStringID = _T("ONQUEUE");
			break;
		case wnd2Clients:
			m_uTotal = theApp.clientlist->m_ArchivedClientsMap.GetCount() + theApp.clientlist->list.GetCount();
			m_uListed = clientlistctrl.m_ListedItemsVector.size();
			resStringID = _T("CLIENTLIST");
			break;
		case wnd2Downloading:
			m_uTotal = downloadclientsctrl.m_ListItemsMap.size();
			m_uListed = downloadclientsctrl.GetItemCount();
			resStringID = _T("DOWNLOADING");
			break;
		default:
			ASSERT(0);
		}
		...
		else if (!IsEmpty(resStringID)) // Make sure we didn't go inside "default:" case above
	...
	case IDC_CLIENTLIST:
		// This map has a special case. Even though there is no filter applied m_uTotal and m_uListed may differ.
		// Because active clients may or may not be in archived client list based on having or not having an hash.
		// So we'll use m_ListedItemsVector.size for m_uTotal if there is no filter. This is guaranteed to be valid.
		// If there is a filter, then we'll use total of m_ArchivedClientsMap and list counts for m_uTotal. This will
		// be probably not valid, but this is the best attempt. (Calculating the exact number is possible, but expensive 
		// for this purpose.)
		m_uListed = clientlistctrl.m_ListedItemsVector.size();
		if (thePrefs.m_uArchivedCheckState || thePrefs.m_uConnectedCheckState || thePrefs.m_uQueryableCheckState || thePrefs.m_uNotQueriedCheckState || thePrefs.m_uValidIPCheckState ||
			thePrefs.m_uHighIdCheckState || thePrefs.m_uBadClientCheckState || !theApp.emuledlg->transferwnd->m_pwndTransfer->m_astrFilterClientList.IsEmpty())
			m_uTotal = theApp.clientlist->m_ArchivedClientsMap.GetCount() + theApp.clientlist->list.GetCount();
		else
			m_uTotal = m_uListed;
		resStringID = _T("CLIENTLIST");
...
void CTransferWnd::OnSysColorChange()
{
	...
	// forces the placeholder text
	m_ctlFilterDownloadList.ShowColumnText(true);
	m_ctlFilterUploadList.ShowColumnText(true);
	m_ctlFilterDownloadClients.ShowColumnText(true);
	m_ctlFilterQueueList.ShowColumnText(true);
	m_ctlFilterClientList.ShowColumnText(true);
}
...
{
	LocalizeToolbars();
	SetDlgItemText(IDC_QUEUECOUNT_LABEL, GetResString(_T("TW_QUEUE")));
	SetDlgItemText(IDC_QUEUE_REFRESH_BUTTON, GetResString(_T("SV_UPDATE")));
	...
	SetDlgItemText(IDC_CHECK_PREVIEW, GetResString(_T("PREVIEW_AVAILABLE")));
	SetDlgItemText(IDC_CHECK_FREEEZE, GetResString(_T("FREEEZE")));
	SetDlgItemText(IDC_CHECK_ARCHIVED, GetResString(_T("ARCHIVED")));
	SetDlgItemText(IDC_CHECK_QUERYABLE, GetResString(_T("QUERYABLE")));
	SetDlgItemText(IDC_CHECK_NOT_QUERIED, GetResString(_T("NOT_QUERIED")));
	SetDlgItemText(IDC_CHECK_VALIDIP, GetResString(_T("VALID_IP")));
	SetDlgItemText(IDC_CHECK_HIGHID, GetResString(_T("IDHIGH")));
...
{
	m_btnWnd1.SetWindowText(GetResString(_T("TW_DOWNLOADS")));
	m_btnWnd1.SetBtnText(MP_VIEW1_SPLIT_WINDOW, GetResString(_T("SPLIT_WINDOW")));
	m_btnWnd1.SetBtnText(MP_VIEW1_DOWNLOADS, GetResString(_T("TW_DOWNLOADS")));
	m_btnWnd1.SetBtnText(MP_VIEW1_UPLOADING, GetResString(_T("UPLOADING")));
	m_btnWnd1.SetBtnText(MP_VIEW1_DOWNLOADING, GetResString(_T("DOWNLOADING")));
	m_btnWnd1.SetBtnText(MP_VIEW1_ONQUEUE, GetResString(_T("ONQUEUE")));
	m_btnWnd1.SetBtnText(MP_VIEW1_CLIENTS, GetResString(_T("CLIENTLIST")));
	m_btnWnd2.SetWindowText(GetResString(_T("UPLOADING")));
	m_btnWnd2.SetBtnText(MP_VIEW2_UPLOADING, GetResString(_T("UPLOADING")));
	m_btnWnd2.SetBtnText(MP_VIEW2_DOWNLOADING, GetResString(_T("DOWNLOADING")));
	m_btnWnd2.SetBtnText(MP_VIEW2_ONQUEUE, GetResString(_T("ONQUEUE")));
	m_btnWnd2.SetBtnText(MP_VIEW2_CLIENTS, GetResString(_T("CLIENTLIST")));
}
...
void CTransferWnd::OnNmRClickDltab(LPNMHDR, LRESULT *pResult)
{
	...
	PrioMenu.AppendMenu(MF_STRING, MP_PRIOLOW, GetResString(_T("PRIOLOW")));
	PrioMenu.CheckMenuItem(MP_PRIOLOW, category_Struct && category_Struct->prio == PR_LOW ? MF_CHECKED : MF_UNCHECKED);
	PrioMenu.AppendMenu(MF_STRING, MP_PRIONORMAL, GetResString(_T("PRIONORMAL")));
	...
	PrioMenu.AppendMenu(MF_STRING, MP_PRIOHIGH, GetResString(_T("PRIOHIGH")));
	...
	CMenuXP CatMenu;
	CatMenu.CreateMenu();
	CatMenu.AppendMenu(MF_STRING, MP_CAT_SET0, GetResString(_T("ALL")));
	UINT flag = (!thePrefs.GetCategory(m_rightclickindex)->care4all && m_rightclickindex) ? MF_GRAYED : MF_STRING;
	CatMenu.AppendMenu(flag, MP_CAT_SET0 + 1, GetResString(_T("ALLOTHERS")));

	// selector for regular expression view filter
	if (m_rightclickindex) {
		if (thePrefs.IsExtControlsEnabled())
			CatMenu.AppendMenu(MF_STRING, MP_CAT_SET0 + 18, GetResString(_T("REGEXPRESSION")));

		flag = (thePrefs.GetCategory(m_rightclickindex)->care4all) ? MF_STRING | MF_CHECKED | MF_BYCOMMAND : MF_STRING;

		if (thePrefs.IsExtControlsEnabled())
			CatMenu.AppendMenu(flag, MP_CAT_SET0 + 17, GetResString(_T("CARE4ALL")));
	}

	CatMenu.AppendMenu(MF_SEPARATOR);
	CatMenu.AppendMenu(MF_STRING, MP_CAT_SET0 + 2, GetResString(_T("STATUS_NOTCOMPLETED")));
	CatMenu.AppendMenu(MF_STRING, MP_CAT_SET0 + 3, GetResString(_T("DL_TRANSFCOMPL")));
	CatMenu.AppendMenu(MF_STRING, MP_CAT_SET0 + 4, GetResString(_T("WAITING")));
	CatMenu.AppendMenu(MF_STRING, MP_CAT_SET0 + 5, GetResString(_T("DOWNLOADING")));
	CatMenu.AppendMenu(MF_STRING, MP_CAT_SET0 + 6, GetResString(_T("ERRORLIKE")));
	CatMenu.AppendMenu(MF_STRING, MP_CAT_SET0 + 7, GetResString(_T("PAUSED")));
	CatMenu.AppendMenu(MF_STRING, MP_CAT_SET0 + 8, GetResString(_T("SEENCOMPL")));
	CatMenu.AppendMenu(MF_SEPARATOR);
	CatMenu.AppendMenu(MF_STRING, MP_CAT_SET0 + 10, GetResString(_T("VIDEO")));
	CatMenu.AppendMenu(MF_STRING, MP_CAT_SET0 + 11, GetResString(_T("AUDIO")));
	CatMenu.AppendMenu(MF_STRING, MP_CAT_SET0 + 12, GetResString(_T("SEARCH_ARC")));
	CatMenu.AppendMenu(MF_STRING, MP_CAT_SET0 + 13, GetResString(_T("SEARCH_CDIMG")));
	...
	if (thePrefs.IsExtControlsEnabled()) {
		CatMenu.AppendMenu(MF_SEPARATOR);
		CatMenu.AppendMenu(thePrefs.GetCatFilter(m_rightclickindex) > 0 ? MF_STRING : MF_GRAYED, MP_CAT_SET0 + 19, GetResString(_T("NEGATEFILTER")));
	...
	menu.AppendMenu(MF_STRING, MP_RESUMENEXT, GetResString(_T("DL_RESUMENEXT")), _T("RESUME"));
	if (thePrefs.IsExtControlsEnabled() && m_rightclickindex != 0) {
		menu.AppendMenu(MF_STRING, MP_DOWNLOAD_ALPHABETICAL, GetResString(_T("DOWNLOAD_ALPHABETICAL")));
	...
	menu.AppendMenu(MF_SEPARATOR);
	menu.AppendMenu(MF_STRING, MP_CAT_ADD, GetResString(_T("CAT_ADD")));
	menu.AppendMenu(flag, MP_CAT_EDIT, GetResString(_T("CAT_EDIT")));
	menu.AppendMenu(flag, MP_CAT_REMOVE, GetResString(_T("CAT_REMOVE")));
...
void CTransferWnd::OnLvnBeginDragDownloadList(LPNMHDR pNMHDR, LRESULT *pResult)
{
	...
	if (iSel == -1)
		return;
	if (!downloadlistctrl.IsListedDownloadFileRow(iSel))
		return;
...
void CTransferWnd::OnLButtonUp(UINT /*nFlags*/, CPoint /*point*/)
{
	if (m_bIsDragging) {
		...
		if (m_nDropIndex >= 0 && (downloadlistctrl.curTab == 0 || (UINT)m_nDropIndex != downloadlistctrl.curTab)) {
			downloadlistctrl.ChangeSelectedFilesCategoryFromUi(static_cast<UINT>(m_nDropIndex));
...
BOOL CTransferWnd::OnCommand(WPARAM wParam, LPARAM lParam)
{
	...
	if (wParam >= MP_CAT_SET0 && wParam <= MP_CAT_SET0 + 99) {
		...
		if (EditCatTabLabel(m_isetcatmenu))
			VerifyCatTabSize();
		thePrefs.SaveCats();
		return TRUE;
	}
	...
	case MP_CAT_EDIT:
		{
			...
			if (dialog.DoModal() == IDOK) {
				if (EditCatTabLabel(m_rightclickindex, thePrefs.GetCategoryDisplayTitle(m_rightclickindex)))
					VerifyCatTabSize();
	...
	case MP_STOP:
		...
		break;
	case MP_CANCEL:
		if (LocMessageBox(_T("Q_CANCELDL"), MB_ICONQUESTION | MB_YESNO, 0) == IDYES)
...
{
	// UI state is accessed here; defer to main thread when needed.
	if (GetCurrentThreadId() != g_uMainThreadId) {
		PostCatTabTitlesUpdateAsync(force);
		return;
	}

	if (theApp.emuledlg != NULL && theApp.emuledlg->IsStartupLoadingDialogVisible()) {
		m_bCatTabInfoDirty = true;
		return;
	}
	...
	if (thePrefs.ShowCatTabInfos() && (m_bCatTabInfoDirty || m_cachedActiveDwl < 0))
		RebuildActiveDownloadCache();
	m_bCatTabInfoDirty = false;

	bool bLayoutNeeded = force || m_bCatTabLayoutPending;
	for (int i = m_dlTab.GetItemCount(); --i >= 0;) {
		bLayoutNeeded = EditCatTabLabel(i, thePrefs.GetCategoryDisplayTitle(i)) || bLayoutNeeded;

		TCITEM tabitem;
		memset(&tabitem, 0, sizeof(tabitem));
		tabitem.mask = TCIF_PARAM;
		const DWORD dwTabColor = thePrefs.GetCatColor(i);
		if (!m_dlTab.GetItem(i, &tabitem) || tabitem.lParam != static_cast<LPARAM>(dwTabColor))
			m_dlTab.SetTabTextColor(i, dwTabColor);
	}
...
// Update cat tab titles if dirty (m_bCatTabInfoDirty) or if forced
void CTransferWnd::UpdateCatTabTitlesIfDirty(bool force)
{
	if (GetCurrentThreadId() != g_uMainThreadId) {
		PostCatTabTitlesUpdateAsync(force);
		return;
	...
	if (force || m_bCatTabInfoDirty)
		UpdateCatTabTitles(force);
}

// Invalidate the cached cat tab info (active downloading count) so it will be recomputed on next UpdateCatTabTitlesIfDirty call
void CTransferWnd::InvalidateCatTabInfo()
{
	if (GetCurrentThreadId() != g_uMainThreadId) {
		PostCatTabInfoInvalidateAsync();
		return;
	}

	m_bCatTabInfoDirty = true;
	m_cachedActiveDwl = -1; // Force recalculation on next access
}

void CTransferWnd::PostQueueCountUpdateAsync(bool bHasQueueCount, INT_PTR iQueueCount, bool bForceImmediateUpdate)
{
	if (m_hWnd == NULL || !::IsWindow(m_hWnd) || theApp.IsClosing())
		return;

	bool bPostMessage = false;
	{
		CSingleLock lock(&m_transferMessagePostLock, TRUE);
		if (bHasQueueCount) {
			m_iPendingQueueCount = iQueueCount;
			m_bPendingQueueCountValue = true;
		}
		m_bPendingQueueCountForce = m_bPendingQueueCountForce || bForceImmediateUpdate;
		bPostMessage = InterlockedCompareExchange(&m_lQueueCountUpdatePending, 1, 0) == 0;
	}
	if (bPostMessage && !PostMessage(UM_UPDATE_QUEUE_COUNT, 0, 0)) {
		CSingleLock lock(&m_transferMessagePostLock, TRUE);
		m_bPendingQueueCountValue = false;
		m_bPendingQueueCountForce = false;
		InterlockedExchange(&m_lQueueCountUpdatePending, 0);
	}
}

void CTransferWnd::PostCatTabTitlesUpdateAsync(bool bForce)
{
	if (m_hWnd == NULL || !::IsWindow(m_hWnd) || theApp.IsClosing())
		return;
	...
	{
		CSingleLock lock(&m_transferMessagePostLock, TRUE);
		m_bPendingCatTabTitleForce = m_bPendingCatTabTitleForce || bForce;
		bPostMessage = InterlockedCompareExchange(&m_lCatTabTitleUpdatePending, 1, 0) == 0;
	}
	if (bPostMessage && !PostMessage(UM_UPDATE_CAT_TAB_TITLES, 0, 0)) {
		CSingleLock lock(&m_transferMessagePostLock, TRUE);
		m_bPendingCatTabTitleForce = false;
		InterlockedExchange(&m_lCatTabTitleUpdatePending, 0);
	}
}

void CTransferWnd::PostCatTabInfoInvalidateAsync()
{
	if (m_hWnd == NULL || !::IsWindow(m_hWnd) || theApp.IsClosing())
		return;

	if (InterlockedCompareExchange(&m_lCatTabInfoInvalidatePending, 1, 0) != 0)
		return;
	if (!PostMessage(UM_INVALIDATE_CAT_TAB_INFO, 0, 0))
		InterlockedExchange(&m_lCatTabInfoInvalidatePending, 0);
}

LRESULT CTransferWnd::OnInvalidateCatTabInfo(WPARAM wParam, LPARAM lParam)
{
...
LRESULT CTransferWnd::OnUpdateCatTabTitlesIfDirty(WPARAM wParam, LPARAM lParam)
{
	UNREFERENCED_PARAMETER(lParam);
	bool bForce = (wParam != 0);
	{
		CSingleLock lock(&m_transferMessagePostLock, TRUE);
		bForce = bForce || m_bPendingCatTabTitleForce;
		m_bPendingCatTabTitleForce = false;
		InterlockedExchange(&m_lCatTabTitleUpdatePending, 0);
	}
	UpdateCatTabTitlesIfDirty(bForce);
	return 0;
}

LRESULT CTransferWnd::OnUpdateQueueCount(WPARAM wParam, LPARAM lParam)
{
	bool bForceImmediateUpdate = (lParam != 0);
	INT_PTR iQueueCount = static_cast<INT_PTR>(wParam);
	bool bHasQueueCount = wParam != 0;
	{
		CSingleLock lock(&m_transferMessagePostLock, TRUE);
		if (m_bPendingQueueCountValue) {
			iQueueCount = m_iPendingQueueCount;
			bHasQueueCount = true;
		}
		bForceImmediateUpdate = bForceImmediateUpdate || m_bPendingQueueCountForce;
		m_bPendingQueueCountValue = false;
		m_bPendingQueueCountForce = false;
		InterlockedExchange(&m_lQueueCountUpdatePending, 0);
	}
	if (bHasQueueCount)
		m_iLastKnownQueueCount = iQueueCount;
	m_bQueueCountDirty = true;
	if (bForceImmediateUpdate)
		m_dwLastQueueCountBannedRefreshTick = 0;
	UpdateQueueCountDisplay(bForceImmediateUpdate);
...
bool CTransferWnd::EditCatTabLabel(int index)
{
	return EditCatTabLabel(index, thePrefs.GetCategoryDisplayTitle(index));
}

bool CTransferWnd::EditCatTabLabel(int index, CString newlabel)
{
	...
	if (thePrefs.ShowCatTabInfos()) {
		...
		if (m_bCatTabInfoDirty || m_cachedActiveDwl < 0)
			RebuildActiveDownloadCache(); // Rebuild global cache only when marked dirty or uninitialized
		
		// Count active downloads in this specific category
		if (index == 0)
			activeDwlForCategory = static_cast<int>(m_activeDownloads.size()); // Category 0 = "All" - show total active downloads
		else {
			// Specific category - count only files in this category
			for (CPartFile* pFile : m_activeDownloads)
				if (pFile && pFile->GetCategory() == index)
					++activeDwlForCategory;
		}
		
		newlabel.AppendFormat(_T(" %i/%i"), activeDwlForCategory, count); // Here count is total number of files in this category (including completed). We show active/total
	}

	const int iTabTextBufferChars = 4096;
	CString strCurrentText;
	LPTSTR pszCurrentText = strCurrentText.GetBuffer(iTabTextBufferChars);
	pszCurrentText[0] = _T('\0');
	TCITEM tabitem;
	memset(&tabitem, 0, sizeof(tabitem));
	tabitem.mask = TCIF_TEXT;
	tabitem.pszText = pszCurrentText;
	tabitem.cchTextMax = iTabTextBufferChars;
	const bool bHasCurrentText = m_dlTab.GetItem(index, &tabitem) != FALSE;
	strCurrentText.ReleaseBuffer();
	if (bHasCurrentText && newlabel == strCurrentText)
		return false;
...
CString CTransferWnd::GetTabStatistic(int tab)
{
	...
	for (POSITION pos = NULL; ;) {
		...
		if (pFile != NULL) {
			if (!downloadlistctrl.IsFilteredOut(pFile)) {
				++count;
	...
	CString title(GetResString(_T("FILES")));
	title.AppendFormat(_T(": %i\n\n%s: %i\n%s: %i\n%s: %i\n%s: %i\n\n%s: %s\n\n%s: %.1f %s\n%s: %s/%s\n%s%s") TOOLTIP_AUTOFORMAT_SUFFIX
		, count + completed
		, (LPCTSTR)GetResString(_T("DOWNLOADING")), dwl
		, (LPCTSTR)GetResString(_T("PAUSED")), paus
...
void CTransferWnd::VerifyCatTabSize()
{
	...
	const int nMinimumTabLeft = rcBtn1.right + 10;
	const int nRequiredTabRight = nMinimumTabLeft + size + DOWNLOAD_HEADER_CONTROL_GAP;

	CWnd* pPreview = GetDlgItem(IDC_CHECK_PREVIEW);
	CWnd* pFileType = GetDlgItem(IDC_FILETYPE);
	CWnd* pFilter = GetDlgItem(IDC_FILTER_DOWNLOAD_LIST);
	if (pPreview == NULL || pPreview->GetSafeHwnd() == NULL || pFileType == NULL || pFileType->GetSafeHwnd() == NULL || pFilter == NULL || pFilter->GetSafeHwnd() == NULL) {
		m_bCatTabLayoutPending = true;
		return;
	}
	CRect rcPreview;
	...
	const bool bShowPreview = rcPreview.left - DOWNLOAD_HEADER_CONTROL_GAP >= nRequiredTabRight;
	const bool bShowFileType = rcFileType.left - DOWNLOAD_HEADER_CONTROL_GAP >= nRequiredTabRight;
	if (((pPreview->GetStyle() & WS_VISIBLE) != 0) != bShowPreview)
		pPreview->ShowWindow(bShowPreview ? SW_SHOW : SW_HIDE);
	if (((pFileType->GetStyle() & WS_VISIBLE) != 0) != bShowFileType)
		pFileType->ShowWindow(bShowFileType ? SW_SHOW : SW_HIDE);

	const int nNextControlLeft = bShowPreview ? rcPreview.left : bShowFileType ? rcFileType.left : rcFilter.left;
	wp.rcNormalPosition.right = nNextControlLeft - DOWNLOAD_HEADER_CONTROL_GAP;

	int left = wp.rcNormalPosition.right - size - DOWNLOAD_HEADER_CONTROL_GAP;
	wp.rcNormalPosition.left = (left >= rcBtn1.right + 10) ? left : rcBtn1.right + 10;

	CRect rcCurrentTab;
	m_dlTab.GetWindowRect(&rcCurrentTab);
	ScreenToClient(&rcCurrentTab);
	if (rcCurrentTab.left != wp.rcNormalPosition.left || rcCurrentTab.right != wp.rcNormalPosition.right) {
		RemoveAnchor(m_dlTab);
		m_dlTab.SetWindowPlacement(&wp);
		AddOrReplaceAnchor(this, m_dlTab, ANCHOR(90, 0));
	}
	m_bCatTabLayoutPending = false;
}
...
void CTransferWnd::ShowList(uint32 dwListIDC)
{
	...
	case IDC_CLIENTLIST:
		...
		clientlistctrl.ReloadList(false, LSF_SELECTION);
		clientlistctrl.FlushDeferredReload(LSF_SELECTION);
		break;
	default:
		ASSERT(0);
	}

	m_ctlFilterDownloadList.ShowWindow((dwListIDC == IDC_DOWNLOADLIST) ? SW_SHOW : SW_HIDE);
	m_ctlFilterUploadList.ShowWindow((dwListIDC == IDC_UPLOADLIST) ? SW_SHOW : SW_HIDE);
	m_ctlFilterDownloadClients.ShowWindow((dwListIDC == IDC_DOWNLOADCLIENTS) ? SW_SHOW : SW_HIDE);
	m_ctlFilterQueueList.ShowWindow((dwListIDC == IDC_QUEUELIST) ? SW_SHOW : SW_HIDE);
	m_ctlFilterClientList.ShowWindow((dwListIDC == IDC_CLIENTLIST) ? SW_SHOW : SW_HIDE);
	if (dwListIDC != IDC_DOWNLOADLIST) {
		GetDlgItem(IDC_FILETYPE)->ShowWindow(SW_HIDE);
		GetDlgItem(IDC_CHECK_PREVIEW)->ShowWindow(SW_HIDE);
	}
	UpdateClientFilterCheckboxVisibility();
...
void CTransferWnd::ShowSplitWindow(bool bReDraw)
{
	...
	AddOrReplaceAnchor(this, IDC_CLIENTLIST, ANCHOR(0, thePrefs.GetSplitterbarPosition()), BOTTOM_RIGHT);
	AddOrReplaceAnchor(this, IDC_DOWNLOADCLIENTS, ANCHOR(0, thePrefs.GetSplitterbarPosition()), BOTTOM_RIGHT);
	m_ctlFilterDownloadList.ShowWindow(SW_SHOW);
	m_ctlFilterUploadList.ShowWindow(SW_HIDE);
	m_ctlFilterDownloadClients.ShowWindow(SW_HIDE);
	m_ctlFilterQueueList.ShowWindow(SW_HIDE);
	m_ctlFilterClientList.ShowWindow(SW_HIDE);
...
void CTransferWnd::OnWnd1BtnDropDown(LPNMHDR, LRESULT*)
{
	...
	menu.AppendMenu(MF_STRING | (m_dwShowListIDC == IDC_DOWNLOADLIST + IDC_UPLOADLIST ? MF_GRAYED : 0), MP_VIEW1_SPLIT_WINDOW, GetResString(_T("SPLIT_WINDOW")), _T("SplitWindow"));
	menu.AppendMenu(MF_SEPARATOR);
	menu.AppendMenu(MF_STRING | (m_dwShowListIDC == IDC_DOWNLOADLIST ? MF_GRAYED : 0), MP_VIEW1_DOWNLOADS, GetResString(_T("TW_DOWNLOADS")), _T("DownloadFiles"));
	menu.AppendMenu(MF_STRING | (m_dwShowListIDC == IDC_UPLOADLIST ? MF_GRAYED : 0), MP_VIEW1_UPLOADING, GetResString(_T("UPLOADING")), _T("Upload"));
	menu.AppendMenu(MF_STRING | (m_dwShowListIDC == IDC_DOWNLOADCLIENTS ? MF_GRAYED : 0), MP_VIEW1_DOWNLOADING, GetResString(_T("DOWNLOADING")), _T("Download"));
	if (!thePrefs.IsQueueListDisabled())
		menu.AppendMenu(MF_STRING | (m_dwShowListIDC == IDC_QUEUELIST ? MF_GRAYED : 0), MP_VIEW1_ONQUEUE, GetResString(_T("ONQUEUE")), _T("ClientsOnQueue"));
	if (!thePrefs.IsKnownClientListDisabled())
		menu.AppendMenu(MF_STRING | (m_dwShowListIDC == IDC_CLIENTLIST ? MF_GRAYED : 0), MP_VIEW1_CLIENTS, GetResString(_T("CLIENTLIST")), _T("ClientsKnown"));
...
void CTransferWnd::OnWnd2BtnDropDown(LPNMHDR, LRESULT*)
{
	...
	menu.AppendMenu(MF_STRING | (m_uWnd2 == wnd2Uploading ? MF_GRAYED : 0), MP_VIEW2_UPLOADING, GetResString(_T("UPLOADING")), _T("Upload"));
	menu.AppendMenu(MF_STRING | (m_uWnd2 == wnd2Downloading ? MF_GRAYED : 0), MP_VIEW2_DOWNLOADING, GetResString(_T("DOWNLOADING")), _T("Download"));
	if (!thePrefs.IsQueueListDisabled())
		menu.AppendMenu(MF_STRING | (m_uWnd2 == wnd2OnQueue ? MF_GRAYED : 0), MP_VIEW2_ONQUEUE, GetResString(_T("ONQUEUE")), _T("ClientsOnQueue"));
	if (!thePrefs.IsKnownClientListDisabled())
		menu.AppendMenu(MF_STRING | (m_uWnd2 == wnd2Clients ? MF_GRAYED : 0), MP_VIEW2_CLIENTS, GetResString(_T("CLIENTLIST")), _T("ClientsKnown"));
...
void CTransferWnd::ResetTransToolbar(bool bShowToolbar, bool bResetLists)
{
	...
	AddOrReplaceAnchor(this, m_btnWnd1, TOP_LEFT);
	AddOrReplaceAnchor(this, m_btnWnd2, ANCHOR(0, thePrefs.GetSplitterbarPosition()));
...
void CTransferWnd::OnPaint()
{
	...
	if (m_bCatTabLayoutPending && (m_dwShowListIDC == IDC_DOWNLOADLIST || m_dwShowListIDC == IDC_UPLOADLIST + IDC_DOWNLOADLIST))
		VerifyCatTabSize();
...
void CTransferWnd::OnFileTypeChange()
{
	// get the current selected entry by value (language independent)
	CString strCurSelFileType = _T(ED2KFTSTR_ANY);
	int iItem = m_FileTypeComboBox.GetCurSel();
	if (iItem != CB_ERR) {
		LPCTSTR pszED2KFileType = (LPCTSTR)m_FileTypeComboBox.GetItemDataPtr(iItem);
		ASSERT(pszED2KFileType != NULL);
		strCurSelFileType = pszED2KFileType;
	}

	if (strCurSelFileType != thePrefs.m_strFileTypeSelected) {
		thePrefs.m_strFileTypeSelected = strCurSelFileType;
		downloadlistctrl.ReloadList(false, LSF_SELECTION);
		InvalidateCatTabInfo();
		UpdateCatTabTitlesIfDirty(false);
	}
...
{
	thePrefs.m_uPreviewCheckState = IsDlgButtonChecked(IDC_CHECK_PREVIEW);
	downloadlistctrl.ReloadList(false, LSF_SELECTION);
	InvalidateCatTabInfo();
	UpdateCatTabTitlesIfDirty(false);
}

void CTransferWnd::OnBnClickedFreeze()
{
	thePrefs.m_bFreezeChecked = IsDlgButtonChecked(IDC_CHECK_FREEEZE);
	if (!thePrefs.m_bFreezeChecked) // Only checked to unchecked status change require a full update (reload).
		clientlistctrl.ReloadList(false, LSF_SELECTION);
	InvalidateCatTabInfo();
	UpdateCatTabTitlesIfDirty(false);
}

void CTransferWnd::OnBnClickedArchived()
{
	thePrefs.m_uArchivedCheckState = IsDlgButtonChecked(IDC_CHECK_ARCHIVED);
	clientlistctrl.ReloadList(false, LSF_SELECTION);
	UpdateListCount();
	InvalidateCatTabInfo();
	UpdateCatTabTitlesIfDirty(false);
...
void CTransferWnd::OnBnClickedQueryable()
{
	thePrefs.m_uQueryableCheckState = IsDlgButtonChecked(IDC_CHECK_QUERYABLE);
	clientlistctrl.ReloadList(false, LSF_SELECTION);
	UpdateListCount();
	InvalidateCatTabInfo();
	UpdateCatTabTitlesIfDirty(false);
...
void CTransferWnd::OnBnClickedQueried()
{
	thePrefs.m_uNotQueriedCheckState = IsDlgButtonChecked(IDC_CHECK_NOT_QUERIED);
	clientlistctrl.ReloadList(false, LSF_SELECTION);
	UpdateListCount();
	InvalidateCatTabInfo();
	UpdateCatTabTitlesIfDirty(false);
...
void CTransferWnd::OnBnClickedValidIP()
{
	thePrefs.m_uValidIPCheckState = IsDlgButtonChecked(IDC_CHECK_VALIDIP);
	clientlistctrl.ReloadList(false, LSF_SELECTION);
	UpdateListCount();
	InvalidateCatTabInfo();
	UpdateCatTabTitlesIfDirty(false);
...
void CTransferWnd::OnBnClickedHighId()
{
	thePrefs.m_uHighIdCheckState = IsDlgButtonChecked(IDC_CHECK_HIGHID);
	clientlistctrl.ReloadList(false, LSF_SELECTION);
	UpdateListCount();
	InvalidateCatTabInfo();
	UpdateCatTabTitlesIfDirty(false);
...
void CTransferWnd::OnBnClickedBadClient()
{
	thePrefs.m_uBadClientCheckState = IsDlgButtonChecked(IDC_CHECK_BADCLIENT);
	clientlistctrl.ReloadList(false, LSF_SELECTION);
	UpdateListCount();
	InvalidateCatTabInfo();
	UpdateCatTabTitlesIfDirty(false);
...
// Active download tracking system implementation for performance optimization
void CTransferWnd::AddActiveDownload(CPartFile* pFile)
{
	// Uses UI list state; only update on the main thread.
	if (GetCurrentThreadId() != g_uMainThreadId) {
		InvalidateCatTabInfo();
		return;
	}

	if (pFile && pFile->GetTransferringSrcCount() > 0 && !downloadlistctrl.IsFilteredOut(pFile)) {
		if (m_activeDownloads.insert(pFile).second) { // Returns pair with second=true if insertion took place
			m_cachedActiveDwl = static_cast<int>(m_activeDownloads.size());
		}
	}
}

void CTransferWnd::RemoveActiveDownload(CPartFile* pFile)
{
	// Uses UI list state; only update on the main thread.
	if (GetCurrentThreadId() != g_uMainThreadId) {
		InvalidateCatTabInfo();
	...
	if (pFile && m_activeDownloads.erase(pFile) > 0) // Returns number of elements erased
		m_cachedActiveDwl = static_cast<int>(m_activeDownloads.size());
}

void CTransferWnd::UpdateActiveDownloadStatus(CPartFile* pFile)
{
	// Uses UI list state; only update on the main thread.
	if (GetCurrentThreadId() != g_uMainThreadId) {
		InvalidateCatTabInfo();
	...
	bool shouldBeActive = pFile->GetTransferringSrcCount() > 0 && !downloadlistctrl.IsFilteredOut(pFile);
	bool isCurrentlyActive = m_activeDownloads.find(pFile) != m_activeDownloads.end();
	
	if (shouldBeActive && !isCurrentlyActive) {
		m_activeDownloads.insert(pFile);
		m_cachedActiveDwl = static_cast<int>(m_activeDownloads.size());
	} else if (!shouldBeActive && isCurrentlyActive) {
		m_activeDownloads.erase(pFile);
		m_cachedActiveDwl = static_cast<int>(m_activeDownloads.size());
	}
	// If status hasn't changed, no need to update cache count
}

void CTransferWnd::RebuildActiveDownloadCache()
{
	int oldCount = static_cast<int>(m_activeDownloads.size());
...
int CTransferWnd::GetActiveDownloadCount() const
{
