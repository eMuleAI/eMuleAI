...
bool CUploadDiskIOThread::AssociateFile(CKnownFile *pFile)
{
	...
	if (pFile && pFile->m_hRead == INVALID_HANDLE_VALUE && !pFile->bNoNewReads) {
		...
		if (!IsWin32LongPathsEnabled() && fullname.GetLength() >= MAX_PATH) {
			theApp.QueueDebugLogLineEx(LOG_WARNING, _T("Skipped opening \"%s\" for upload - path too long (%u). Enable long path support to allow this."), (LPCTSTR)EscPercent(fullname), (UINT)fullname.GetLength());
			pFile->bNoNewReads = true;
			return false;
		}
		pFile->m_hRead = ::CreateFile(longFullname, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, NULL, OPEN_EXISTING, FILE_FLAG_OVERLAPPED | FILE_FLAG_SEQUENTIAL_SCAN, NULL);

		if (pFile->m_hRead == INVALID_HANDLE_VALUE) {
			theApp.QueueDebugLogLineEx(LOG_ERROR, _T("Failed to open \"%s\" for overlapped read: %s"), (LPCTSTR)EscPercent(fullname), (LPCTSTR)EscPercent(GetErrorMessage(::GetLastError(), 1)));
			return false;
		}
		if (m_hPort != ::CreateIoCompletionPort(pFile->m_hRead, m_hPort, (ULONG_PTR)pFile, 0)) {
			theApp.QueueDebugLogLineEx(LOG_ERROR, _T("Failed to associate \"%s\" with reading IOCP: %s"), (LPCTSTR)EscPercent(fullname), (LPCTSTR)EscPercent(GetErrorMessage(::GetLastError(), 1)));
	...
	try {
			...
			&& (addedPayloadQueueSession <= nCurQueueSessionPayloadUp || addedPayloadQueueSession - nCurQueueSessionPayloadUp < nBufferLimit))
		{
			...
			if (uTogo > EMBLOCKSIZE * 3)
				throw GetResString(_T("ERR_LARGEREQBLOCK"));
		...
		theApp.QueueDebugLogLineEx(LOG_ERROR, GetResString(_T("ERR_CLIENTERRORED")), (LPCTSTR)EscPercent(pClient->GetUserName()), (LPCTSTR)EscPercent(ex));
	} catch (CFileException *ex) {
		theApp.QueueDebugLogLineEx(LOG_ERROR, _T("Failed to create upload package for %s%s"), (LPCTSTR)EscPercent(pClient->GetUserName()), (LPCTSTR)EscPercent(CExceptionStrDash(*ex)));
		ex->Delete();
	}
