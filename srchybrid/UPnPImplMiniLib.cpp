...
#endif
	{
		if (!m_pOwner->m_bCheckAndRefresh) {
			...
			for (UPNPDev *pDevice = structDeviceList; pDevice != NULL; pDevice = pDevice->pNext)
				DebugLog(_T("Desc: %S, st: %S"), (LPCTSTR)EscPercent(CString(pDevice->descURL)), (LPCTSTR)EscPercent(CString(pDevice->st)));
			...
			*m_pOwner->m_achWanIP = 0;
			...
			switch (iResult) {
			case 1:
				DebugLog(_T("Found valid IGD : %S"), (LPCTSTR)EscPercent(CString(m_pOwner->m_pURLs->controlURL)));
				break;
			case 2:
				DebugLog(_T("Found an IGD with a reserved IP address (%S) : %S"), m_pOwner->m_achWanIP, (LPCTSTR)EscPercent(CString(m_pOwner->m_pURLs->controlURL)));
				bNotFound = true;
				break;
			case 3:
				DebugLog(_T("Found a (not connected?) IGD : %S - Trying to continue anyway"), (LPCTSTR)EscPercent(CString((m_pOwner->m_pURLs->controlURL))));
				break;
			case 4:
				DebugLog(_T("UPnP device found. Is it an IGD? : %S - Trying to continue anyway"), (LPCTSTR)EscPercent(CString(m_pOwner->m_pURLs->controlURL)));
				break;
			default:
				DebugLog(_T("Found device (igd ?) : %S - Aborting"), m_pOwner->m_pURLs->controlURL != NULL ? (LPCTSTR)EscPercent(CString(m_pOwner->m_pURLs->controlURL)) : (LPCTSTR)_T("(none)"));
				bNotFound = true;
			}
			...
			DebugLog(_T("Our LAN IP: %S"), (LPCTSTR)EscPercent(CString(m_pOwner->m_achLanIP)));
