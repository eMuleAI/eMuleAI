...
BOOL CHttpDownloadDlg::OnInitDialog()
{
	...
	// Existing targets are overwritten by the download flow.
	...
	if (!m_FileToWrite.Open(m_sFileToDownloadInto, CFile::modeCreate | CFile::modeWrite | CFile::shareDenyWrite)) {
		...
		CString sMsg;
		sMsg.Format(GetResString(_T("HTTPDOWNLOAD_FAIL_FILE_OPEN")), (LPCTSTR)GetErrorMessage(dwError));
		CDarkMode::MessageBox(sMsg);
		EndDialog(IDCANCEL);
		return TRUE;
	}
	...
	sFileStatus.Format(GetResString(_T("HTTPDOWNLOAD_FILESTATUS")), (LPCTSTR)m_sFilename, (LPCTSTR)m_sServer);
	m_ctrlFileStatus.SetWindowText(sFileStatus);

	// set labels
	SetDlgItemText(IDC_TIMELEFTTEXT, GetResString(_T("ESTTIMELEFT")));
	SetDlgItemText(IDC_TRANSFER_RATE_LABEL, GetResString(_T("TRANSFER_RATE_LABEL")));
...
void CHttpDownloadDlg::SetPercentage(int nPercentage)
{
	...
	CString sCaption;
	sCaption.Format(GetResString(_T("HTTPDOWNLOAD_PERCENTAGE")), (LPCTSTR)sPercentage, (LPCTSTR)m_sFilename);
...
{
	CString sOf;
	sOf.Format(GetResString(_T("HTTPDOWNLOAD_OF")), (LPCTSTR)CastItoXBytes((uint64)dwBytesRead), (LPCTSTR)CastItoXBytes((uint64)dwFileSize));

	CString sTimeLeft;
	sTimeLeft.Format(GetResString(_T("HTTPDOWNLOAD_TIMELEFT")), (LPCTSTR)CastSecondsToHM(dwSecondsLeft), (LPCTSTR)sOf);
...
resend:
	...
	SetStatus(GetResString(_T("HTTPDOWNLOAD_GETTING_FILE_INFORMATION")));
	...
	SetStatus(GetResString(_T("HTTPDOWNLOAD_RETREIVEING_FILE")));
	...
	, LPVOID lpvStatusInformation, DWORD dwStatusInformationLength)
{
	...
	case INTERNET_STATUS_RESOLVING_NAME:
		uid = _T("HTTPDOWNLOAD_RESOLVING_NAME");
		break;
	case INTERNET_STATUS_NAME_RESOLVED:
		uid = _T("HTTPDOWNLOAD_RESOLVED_NAME");
		break;
	case INTERNET_STATUS_CONNECTING_TO_SERVER:
		uid = _T("HTTPDOWNLOAD_CONNECTING");
		break;
	case INTERNET_STATUS_CONNECTED_TO_SERVER:
		uid = _T("HTTPDOWNLOAD_CONNECTED");
		break;
	case INTERNET_STATUS_REDIRECT:
		uid = _T("HTTPDOWNLOAD_REDIRECTING");
		break;
	default:
		uid = EMPTY;
	}
...
void CHttpDownloadDlg::OnCancel()
{
	...
	SetStatus(GetResString(_T("HTTPDOWNLOAD_ABORTING_TRANSFER")));
}
