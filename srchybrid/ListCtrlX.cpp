...
void CListCtrlX::CreateColumns(int iColumns, LCX_COLUMN_INIT *pColumns)
{
	...
	for (int iCol = 0; iCol < iColumns; ++iCol) {
		CString strHeading;
		if (!IsEmpty(pColumns[iCol].strHeadResID))
			strHeading = GetResString(pColumns[iCol].strHeadResID);
	...
	if (hbmHdr != NULL) {
		...
		if (imlHdr.Create(sizeHdrImgListIcon.cx, sizeHdrImgListIcon.cy, ILC_COLOR | ILC_MASK, iHdrImgListImages, 0)) {
			...
			if (theApp.m_ullComCtrlVer < MAKEDLLVERULL(6, 0, 0, 0))
				imlHdr.SetBkColor(GetCustomSysColor(COLOR_BTNFACE));
...
BOOL CListCtrlX::OnHdrBeginDrag(UINT, LPNMHDR, LRESULT*)
{
	if (theApp.m_ullComCtrlVer < MAKEDLLVERULL(6, 0, 0, 0)) {
		...
		if (piml != NULL)
			piml->SetBkColor(GetCustomSysColor(COLOR_3DSHADOW));
	}
...
BOOL CListCtrlX::OnHdrEndDrag(UINT, LPNMHDR, LRESULT*)
{
	if (theApp.m_ullComCtrlVer < MAKEDLLVERULL(6, 0, 0, 0)) {
		CImageList *piml = GetHeaderCtrl()->GetImageList();
		if (piml != NULL)
			piml->SetBkColor(GetCustomSysColor(COLOR_BTNFACE));
	}
	return FALSE; // *Force* default processing of notification!
}
...
void CListCtrlX::DoFind(int iStartItem, int iDirection /*1 = down, -1 = up*/, BOOL bShowError)
{
	...
	if (bShowError)
		CDarkMode::MessageBox(_T("No matching entry found."), MB_ICONINFORMATION);
	else
		MessageBeep(MB_OK);
}
...
void CListCtrlX::OnNmCustomDraw(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	case CDDS_PREPAINT:
		*pResult = CDRF_NOTIFYITEMDRAW;		// request item callbacks
		return;

	case CDDS_ITEMPREPAINT: {
		int idx = static_cast<int>(cd->nmcd.dwItemSpec);
		CRect rcRow; GetItemRect(idx, &rcRow, LVIR_BOUNDS);

		bool sel = (GetItemState(idx, LVIS_SELECTED) & LVIS_SELECTED) != 0;
		COLORREF bk = GetCustomSysColor(sel ? COLOR_HIGHLIGHT : COLOR_WINDOW);

		if (HBRUSH h = CreateSolidBrush(bk)) {
			FillRect(cd->nmcd.hdc, &rcRow, h);	// repaint full row including gutter
			DeleteObject(h);
		}
