...
BOOL CIPFilterDlg::OnInitDialog()
{
	...
	AddOrReplaceAnchor(this, IDC_IPFILTER, TOP_LEFT, BOTTOM_RIGHT);
	AddOrReplaceAnchor(this, IDC_TOTAL_IPFILTER_LABEL, BOTTOM_LEFT);
	AddOrReplaceAnchor(this, IDC_TOTAL_IPFILTER, BOTTOM_LEFT);
	AddOrReplaceAnchor(this, IDC_TOTAL_IPS_LABEL, BOTTOM_LEFT);
	AddOrReplaceAnchor(this, IDC_TOTAL_IPS, BOTTOM_LEFT);
	AddOrReplaceAnchor(this, IDC_REMOVE, BOTTOM_RIGHT);
	AddOrReplaceAnchor(this, IDC_APPEND, BOTTOM_RIGHT);
	AddOrReplaceAnchor(this, IDC_SAVE, BOTTOM_RIGHT);
	...
	m_ipfilter.SetExtendedStyle(LVS_EX_FULLROWSELECT | LVS_EX_INFOTIP | LVS_EX_GRIDLINES | LVS_EX_DOUBLEBUFFER);
	...
	m_pMenuIPFilter = new CMenuXP();
	if (m_pMenuIPFilter->CreatePopupMenu()) {
		m_pMenuIPFilter->AppendMenu(MF_ENABLED | MF_STRING, MP_COPYSELECTED, GetResString(_T("COPY")));
		m_pMenuIPFilter->AppendMenu(MF_ENABLED | MF_STRING, MP_REMOVE, GetResString(_T("REMOVE")));
		m_pMenuIPFilter->AppendMenu(MF_SEPARATOR);
		m_pMenuIPFilter->AppendMenu(MF_ENABLED | MF_STRING, MP_SELECTALL, GetResString(_T("SELECTALL")));
		m_pMenuIPFilter->AppendMenu(MF_SEPARATOR);
		m_pMenuIPFilter->AppendMenu(MF_ENABLED | MF_STRING, MP_FIND, GetResString(_T("FIND")));
	}
	m_ipfilter.m_pMenu = m_pMenuIPFilter;
	m_ipfilter.m_pParent = this;

	// localize
	SetWindowText(GetResString(_T("IPFILTER")));
	SetDlgItemText(IDC_STATICIPLABEL, GetResString(_T("IP_RULES")));
	SetDlgItemText(IDC_TOTAL_IPFILTER_LABEL, GetResString(_T("TOTAL_IPFILTER_LABEL")));
	SetDlgItemText(IDC_TOTAL_IPS_LABEL, GetResString(_T("TOTAL_IPS_LABEL")));
...
void CIPFilterDlg::InitIPFilters()
{
	...
	if (m_ppIPFilterItems == NULL)
		...
		for (UINT i = 0; i < m_uIPFilterItems; ++i) {
			SIPFilter *pFilter = ipfilter[i];
			m_ppIPFilterItems[i] = pFilter;
			m_ulFilteredIPs = m_ulFilteredIPs + pFilter->end - pFilter->start + 1;
		}
	SortIPFilterItems();
	m_ipfilter.SetItemCount(m_uIPFilterItems);
	CString m_strTemp; 
	m_strTemp.Format(_T("%lu"), m_uIPFilterItems);
	SetDlgItemText(IDC_TOTAL_IPFILTER, m_strTemp);
	SetDlgItemText(IDC_TOTAL_IPS, m_ulFilteredIPs.ToDecimalString());
	m_ulFilteredIPs;
}
	...
	if (rItem.mask & LVIF_TEXT) // *have* to check that flag!!
		...
		case IPFILTER_COL_START:
			if (rItem.cchTextMax > 0) {
				_tcsncpy(rItem.pszText, (LPCTSTR)ipstr(CAddress(m_ppIPFilterItems[rItem.iItem]->start, true)), rItem.cchTextMax);
				rItem.pszText[rItem.cchTextMax - 1] = _T('\0');
			}
			break;
		case IPFILTER_COL_END:
			if (rItem.cchTextMax > 0) {
				_tcsncpy(rItem.pszText, (LPCTSTR)ipstr(CAddress(m_ppIPFilterItems[rItem.iItem]->end, true)), rItem.cchTextMax);
				rItem.pszText[rItem.cchTextMax - 1] = _T('\0');
			}
			break;
		case IPFILTER_COL_LEVEL:
...
void CIPFilterDlg::OnBnClickedDelete()
{
	...
	if (m_ipfilter.GetSelectedCount() == m_uIPFilterItems) {
		...
		for (POSITION pos = m_ipfilter.GetFirstSelectedItemPosition(); pos != NULL;) {
			...
			if (pFilter) {
				Kademlia::CUInt128 endip = pFilter->end;
				Kademlia::CUInt128 ulIPRange = endip - pFilter->start + 1;
				if (theApp.ipfilter->RemoveIPFilter(pFilter)) {
					theApp.ipfilter->SetModified();
					aItems.Add(iItem);
					m_ulFilteredIPs = m_ulFilteredIPs - ulIPRange;
				}
			}
		}
...
void CIPFilterDlg::OnBnClickedSave()
{
	...
	try {
		...
		CDarkMode::MessageBox(err, MB_ICONERROR);
	}
}
...
bool CIPFilterDlg::FindItem(const CListCtrlX &lv, int iItem, DWORD_PTR lParam)
{
	...
	ASSERT_VALID(dlg);
	CAddress IP(lv.GetFindText(), false);
	Kademlia::CUInt128 IP128 = IP.ToUInt128(true);
	const SIPFilter* filter = dlg->m_ppIPFilterItems[iItem];
	return (IP128 >= filter->start && IP128 <= filter->end);
}
