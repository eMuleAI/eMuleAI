...
{
	for (POSITION pos = theApp.serverconnect->connectionattempts.GetStartPosition(); pos != NULL;) {
		DWORD tmpkey;
		CServerSocket* tmpsock;
		theApp.serverconnect->connectionattempts.GetNextAssoc(pos, tmpkey, tmpsock);
		if (tmpsock == this) {
			theApp.serverconnect->connectionattempts.RemoveKey(tmpkey);
			break;
		}
	}
	delete cur_server;
	...
	if (cur_server->HasDynIP()) {
		...
		if (thePrefs.GetFilterServerByIP() && theApp.ipfilter->IsFiltered(cur_server->GetIP())) {
			...
			if (thePrefs.GetLogFilteredIPs())
				AddDebugLogLine(false, _T("IPFilter(TCP/DNSResolve): Filtered server \"%s\" (IP=%s) - IP filter (%s)"), pServer ? pServer->GetAddress() : cur_server->GetAddress(), (LPCTSTR)ipstr(cur_server->GetIP()), (LPCTSTR)EscPercent(theApp.ipfilter->GetLastHit()));
			if (pServer)
...
void CServerSocket::OnConnect(int nErrorCode)
{
	...
	if (thePrefs.GetVerbose())
		DebugLogError(_T("Failed to connect to server %s; %s"), cur_server->GetAddress(), (LPCTSTR)EscPercent(GetFullErrorMessage(nErrorCode)));
	serverconnect->DestroySocket(this);
}
				...
				for (int iPos = 0; iPos >= 0;) {
					...
					if (bOutputMessage) {
						if (m_bStartNewMessageLog) {
							...
							theApp.emuledlg->AddServerMessageLine(LOG_INFO, EMPTY);
							if (cur_server) {
								CString strMsg(CTime::GetCurrentTime().Format(thePrefs.GetDateTimeFormat4Log()));
								strMsg += _T(": ");
								strMsg.AppendFormat(GetResString(IsObfusicating() ? _T("CONNECTEDTOOBFUSCATED") : _T("CONNECTEDTO")) + _T(" (%s:%u)")
		...
		case OP_IDCHANGE:
			{
				...
				if (size < sizeof(LoginAnswer_Struct))
					throw GetResString(_T("ERR_BADSERVERREPLY"));
				...
				if (::IsLowID(la->clientid) && dwServerReportedIP != 0)
					theApp.SetPublicIPv4(dwServerReportedIP);
				AddLogLine(false, GetResString(_T("NEWCLIENTID")), la->clientid);
		...
		case OP_SERVERIDENT:
			{
				...
				if (size < 16 + 4 + 2 + 4) {
					if (thePrefs.GetVerbose())
						DebugLogError((LPCTSTR)GetResString(_T("ERR_KNOWNSERVERINFOREC")));
		...
		case OP_SERVERLIST:
			...
			try {
				...
				if (1 + count * (4 + 2) <= size) {
					...
					if (addcount)
						AddLogLine(false, GetResString(_T("NEWSERVERS")), addcount);
				}
				if (thePrefs.GetDebugServerTCPLevel() > 0) {
				...
				if (thePrefs.GetVerbose())
					DebugLogError((LPCTSTR)GetResString(_T("ERR_BADSERVERLISTRECEIVED")));
				ex->Delete();
			}
			break;
		case OP_CALLBACKREQUESTED:
			...
			if (size >= 6) {
				...
				if (theApp.ipfilter->IsFiltered(dwIP)) {
					...
					if (thePrefs.GetLogFilteredIPs())
						AddDebugLogLine(false, _T("Ignored callback request (IP=%s) - IP filter (%s)"), (LPCTSTR)ipstr(dwIP), (LPCTSTR)EscPercent(theApp.ipfilter->GetLastHit()));
					break;
				}

				if (theApp.clientlist->IsBannedClient(ipstr(dwIP))) {
					if (thePrefs.GetLogBannedClients()) {
						CUpDownClient *pClient = theApp.clientlist->FindClientByIP(CAddress(dwIP, false));
						AddDebugLogLine(false, _T("Ignored callback request from banned client %s; %s"), (LPCTSTR)ipstr(dwIP), pClient ? (LPCTSTR)EscPercent(pClient->DbgGetClientInfo()) : (LPCTSTR)EMPTY);
					}
					break;
				}

				uint16 nPort = PeekUInt16(packet + 4);
				CUpDownClient *client = theApp.clientlist->FindClientByConnIP(CAddress(dwIP, false), nPort);
				if (client == NULL) {
					client = new CUpDownClient(0, nPort, dwIP, 0, 0, true); // IPv6-TODO: Check this
...
void CServerSocket::ConnectTo(CServer *server, bool bNoCrypt)
{
	...
	Log(GetResString(bEncrypt ? _T("CONNECTINGTOOBFUSCATED") : _T("CONNECTINGTO")), (LPCTSTR)EscPercent(cur_server->GetListName()), cur_server->GetAddress(), nPort);
	...
	if (!Connect(server->GetAddress(), nPort)) {
		...
		if (dwError != WSAEWOULDBLOCK) {
			LogError(GetResString(_T("ERR_CONNECTIONERROR")), (LPCTSTR)EscPercent(cur_server->GetListName()), cur_server->GetAddress(), nPort, (LPCTSTR)EscPercent(GetFullErrorMessage(dwError)));
...
void CServerSocket::OnError(int nErrorCode)
{
	SetConnectionState(CS_DISCONNECTED);
	if (thePrefs.GetVerbose())
		DebugLogError(GetResString(_T("ERR_SOCKET")), (LPCTSTR)EscPercent(cur_server->GetListName()), cur_server->GetAddress(), cur_server->GetPort(), (LPCTSTR)EscPercent(GetFullErrorMessage(nErrorCode)));
}
