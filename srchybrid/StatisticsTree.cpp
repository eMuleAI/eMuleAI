...
void CStatisticsTree::DoMenu(CPoint doWhere, UINT nFlags)
{
	...
	mnuContext.AppendMenu(MF_STRING, MP_STATTREE_EXPANDMAIN, GetResString(_T("STATS_MNUTREEEXPANDMAIN")), _T("EXPANDMAIN"));
	mnuContext.AppendMenu(MF_STRING, MP_STATTREE_EXPANDALL, GetResString(_T("STATS_MNUTREEEXPANDALL")), _T("EXPANDALL"));
	mnuContext.AppendMenu(MF_STRING, MP_STATTREE_COLLAPSEALL, GetResString(_T("STATS_MNUTREECOLLAPSEALL")), _T("COLLAPSE"));
	mnuContext.AppendMenu(MF_SEPARATOR);
	mnuContext.AppendMenu(MF_STRING, MP_STATTREE_COPYSEL, GetResString(_T("STATS_MNUTREECPYSEL")), _T("COPY"));
	mnuContext.AppendMenu(MF_STRING, MP_STATTREE_COPYVIS, GetResString(_T("STATS_MNUTREECPYVIS")), _T("COPYVISIBLE"));
	mnuContext.AppendMenu(MF_STRING, MP_STATTREE_COPYALL, GetResString(_T("STATS_MNUTREECPYALL")), _T("COPYSELECTED"));
	mnuContext.AppendMenu(MF_SEPARATOR);

	mnuHTML.CreateMenu();
	mnuHTML.AppendMenu(MF_STRING, MP_STATTREE_HTMLCOPYSEL, GetResString(_T("STATS_MNUTREECPYSEL")), _T("COPY"));
	mnuHTML.AppendMenu(MF_STRING, MP_STATTREE_HTMLCOPYVIS, GetResString(_T("STATS_MNUTREECPYVIS")), _T("COPYVISIBLE"));
	mnuHTML.AppendMenu(MF_STRING, MP_STATTREE_HTMLCOPYALL, GetResString(_T("STATS_MNUTREECPYALL")), _T("COPYSELECTED"));
	mnuHTML.AppendMenu(MF_SEPARATOR);
	mnuHTML.AppendMenu(MF_STRING, MP_STATTREE_HTMLEXPORT, GetResString(_T("STATS_EXPORT2HTML")), _T("EXPORTALL"));
	mnuContext.AppendMenu(MF_STRING | MF_POPUP, (UINT_PTR)mnuHTML.m_hMenu, GetResString(_T("STATS_MNUTREEHTML")), _T("WEB"));
...
BOOL CStatisticsTree::OnCommand(WPARAM wParam, LPARAM)
{
	...
	case MP_STATTREE_RESET:
		if (LocMessageBox(_T("STATS_MBRESET_TXT"), MB_YESNO | MB_ICONEXCLAMATION, 0) == IDYES) {
			thePrefs.ResetCumulativeStatistics();
			AddLogLine(false, GetResString(_T("STATS_NFORESET")));
			theApp.emuledlg->statisticswnd->ShowStatistics();

			CString myBuffer;
			myBuffer.Format(GetResString(_T("STATS_LASTRESETSTATIC")), (LPCTSTR)thePrefs.GetStatsLastResetStr(false));
			GetParent()->SetDlgItemText(IDC_STATIC_LASTRESET, myBuffer);
		}
		break;
	case MP_STATTREE_RESTORE:
		if (LocMessageBox(_T("STATS_MBRESTORE_TXT"), MB_YESNO | MB_ICONQUESTION, 0) == IDYES)
			if (!thePrefs.LoadStats(1))
				LogError(LOG_STATUSBAR, GetResString(_T("ERR_NOSTATBKUP")));
			else {
				AddLogLine(false, GetResString(_T("STATS_NFOLOADEDBKUP")));
				CString myBuffer;
				myBuffer.Format(GetResString(_T("STATS_LASTRESETSTATIC")), (LPCTSTR)thePrefs.GetStatsLastResetStr(false));
				GetParent()->SetDlgItemText(IDC_STATIC_LASTRESET, myBuffer);
			}
...
// EX: if(IsBold(myTreeItem)) DarkMode::MessageBox("It's bold.");
...
// false.  Very straightforward.
// EX: if(IsExpanded(myTreeItem)) DarkMode::MessageBox("It's expanded.");
...
// EX:  if(CheckState(myTreeItem, TVIS_BOLD)) DarkMode::MessageBox("It's bold.");
...
CString CStatisticsTree::GetHTML(bool onlyVisible, HTREEITEM theItem, int theItemLevel, bool firstItem)
{
	...
	CString strBuffer;
	if (firstItem)
		strBuffer.Format(_T("<font face=\"Tahoma,Verdana,Courier New,Helvetica\" size=\"2\">\r\n<b>%s %s [%s]</b>\r\n<br /><br />\r\n"), (LPCTSTR)theApp.GetAppVersion(), (LPCTSTR)GetResString(_T("SF_STATISTICS")), (LPCTSTR)thePrefs.GetUserNick());
	while (hCurrent != NULL) {
