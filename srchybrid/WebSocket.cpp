			...
			if (thePrefs.GetWebUseHttps()) {
				...
				while ((ret = mbedtls_ssl_handshake(&ssl)) != 0)
					if (ret != MBEDTLS_ERR_SSL_WANT_READ && ret != MBEDTLS_ERR_SSL_WANT_WRITE) {
						DebugLogWarning(_T("Web Interface handshake failed: %s"), (LPCTSTR)EscPercent(SSLerror(ret)));
						goto thread_exit;
					}
			}
					...
					while (WAIT_OBJECT_0 == ::WaitForMultipleObjects(2, pWait, FALSE, INFINITE)) {
						for (;;) {
							...
							if (thePrefs.GetWSIsEnabled()) {
								...
								// Ensure the created CWinThread will be deleted when the thread finishes so we don't leak CWinThread objects for each accepted connection.
								pAcceptThread->m_bAutoDelete = TRUE;
...
int StartSSL()
{
	...
	if (ret)
		DebugLogError(_T("Web Interface start failed: %s"), (LPCTSTR)EscPercent(SSLerror(ret)));
	else {
		unsigned char fingerprint[20];
		const mbedtls_md_info_t *pMdInfo = mbedtls_md_info_from_type(MBEDTLS_MD_SHA1);
		if (pMdInfo && !mbedtls_md(pMdInfo, srvcert.raw.p, srvcert.raw.len, fingerprint))
			DebugLog(_T("Loaded certificate: %s"), (LPCTSTR)EscPercent(GetCertHash(fingerprint, (int)(sizeof fingerprint))));
		else
			DebugLogWarning(_T("Loaded certificate but failed to calculate SHA1 fingerprint"));
	}
	return ret;
}
