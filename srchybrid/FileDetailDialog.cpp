...
{
	if (paItems == NULL || paItems->GetSize() != 1)
		return false;

	CObject *pObject = (*paItems)[0];
	if (pObject == NULL)
		return false;

	const CKnownFile *pKnownCandidate = static_cast<const CKnownFile*>(pObject);
	bool bPointerAlive = false;
	if (theApp.downloadqueue != NULL && theApp.downloadqueue->IsPartFile(pKnownCandidate))
		bPointerAlive = true;
	else if (theApp.sharedfiles != NULL && theApp.sharedfiles->IsFilePtrInList(pKnownCandidate))
		bPointerAlive = true;
	else if (theApp.knownfiles != NULL && theApp.knownfiles->IsFilePtrInList(pKnownCandidate))
		bPointerAlive = true;
	if (!bPointerAlive)
		return false;

	CShareableFile *pFile = static_cast<CShareableFile*>(pObject);
	if (!pFile->IsKindOf(RUNTIME_CLASS(CShareableFile)))
		return false;
	...
	return (ED2KFT_ARCHIVE == GetED2KFileTypeID(strFileName));
}
