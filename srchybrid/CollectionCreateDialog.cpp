...
BOOL CCollectionCreateDialog::OnInitDialog()
{
	...
	if (m_bCreatemode)
		SetWindowText(GetResString(_T("CREATECOLLECTION")));
	else
		SetWindowText(GetResString(_T("MODIFYCOLLECTION")) + _T(": ") + m_pCollection->m_sCollectionName);
	...
	m_SaveButton.SetWindowText(GetResString(_T("SAVE")));
	m_CancelButton.SetWindowText(GetResString(_T("CANCEL")));
	m_CollectionCreateSignNameKeyCheck.SetWindowText(GetResString(_T("COLL_SIGN")));
	m_CollectionCreateFormatCheck.SetWindowText(GetResString(_T("COLL_TEXTFORMAT")));
	SetDlgItemText(IDC_CCOLL_STATIC_NAME, GetResString(_T("SW_NAME")) + _T(':'));
	SetDlgItemText(IDC_CCOLL_BASICOPTIONS, GetResString(_T("LD_BASICOPT")));
	SetDlgItemText(IDC_CCOLL_ADVANCEDOPTIONS, GetResString(_T("LD_ADVANCEDOPT")));

	AddOrReplaceAnchor(this, IDC_COLLECTIONAVAILLIST, TOP_LEFT, BOTTOM_CENTER);
	AddOrReplaceAnchor(this, IDC_COLLECTIONLISTCTRL, TOP_CENTER, BOTTOM_RIGHT);
	AddOrReplaceAnchor(this, IDC_COLLECTIONLISTLABEL, TOP_CENTER);
	AddOrReplaceAnchor(this, IDC_COLLECTIONLISTICON, TOP_CENTER);
	AddOrReplaceAnchor(this, IDC_COLLECTIONADD, MIDDLE_CENTER);
	AddOrReplaceAnchor(this, IDC_COLLECTIONREMOVE, MIDDLE_CENTER);
	AddOrReplaceAnchor(this, IDC_CCOLL_SAVE, BOTTOM_RIGHT);
	AddOrReplaceAnchor(this, IDC_CCOLL_CANCEL, BOTTOM_RIGHT);
	AddOrReplaceAnchor(this, IDC_CCOLL_BASICOPTIONS, BOTTOM_LEFT, BOTTOM_RIGHT);
	AddOrReplaceAnchor(this, IDC_CCOLL_ADVANCEDOPTIONS, BOTTOM_LEFT, BOTTOM_RIGHT);
	AddOrReplaceAnchor(this, IDC_CCOLL_STATIC_NAME, BOTTOM_LEFT);
	AddOrReplaceAnchor(this, IDC_COLLECTIONNAMEEDIT, BOTTOM_LEFT, BOTTOM_RIGHT);
	AddOrReplaceAnchor(this, IDC_COLLECTIONCREATEFORMAT, BOTTOM_LEFT, BOTTOM_RIGHT);
	...
	if (!sFileName.IsEmpty()) {
		...
		if (::PathFileExists(sFilePath)) {
			...
			if (bDeleteSuccessful) {
				...
				LocMessageBox(_T("COLL_ERR_DELETING"), MB_ICONWARNING | MB_DEFBUTTON2 | MB_YESNO, 0);
		} else
			m_pCollection->WriteToFileAddShared(pSignkey);
...
void CCollectionCreateDialog::OnBnClickedCollectionViewShared()
{
	...
	strTitle.Format(_T("   %s (%d)")
		, (LPCTSTR)GetResString(m_bSharedFiles ? _T("SHARED") : _T("KNOWN"))
