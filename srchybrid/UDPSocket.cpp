...
void CUDPSocket::OnReceive(int nErrorCode)
{
	if (nErrorCode) {
		...
		if (thePrefs.GetVerbose())
			DebugLogError(_T("Error: Server UDP socket: Receive failed - %s"), (LPCTSTR)EscPercent(GetErrorMessage(nErrorCode, 1)));
	}
	...
	if (length != SOCKET_ERROR) {
			...
			&& ((pServer->GetServerKeyUDP() != 0 && pServer->SupportsObfuscationUDP()) || (pServer->GetCryptPingReplyPending() && pServer->GetChallenge() != 0)))
		{
			...
			if (nPayLoadLen == length)
				DebugLogWarning(_T("Expected encrypted packet, but received unencrypted from server %s, UDPKey %u, Challenge: %u"), (LPCTSTR)EscPercent(pServer->GetListName()), pServer->GetServerKeyUDP(), pServer->GetChallenge());
			else if (thePrefs.GetDebugServerUDPLevel() > 0)
				DEBUG_ONLY(DebugLog(_T("Received encrypted packet from server %s, UDPKey %u, Challenge: %u"), (LPCTSTR)EscPercent(pServer->GetListName()), pServer->GetServerKeyUDP(), pServer->GetChallenge()));
		}
		...
		if (dwError == WSAECONNRESET) {
			...
			if (pServer)
				if (!pServer->GetCryptPingReplyPending() && ::GetTickCount() >= pServer->GetLastPinged() + SEC2MS(30)) {
					...
					DEBUG_ONLY(DebugLog(_T("CryptPing failed (WSACONNRESET) for server %s"), (LPCTSTR)EscPercent(pServer->GetListName())));

		} else if (thePrefs.GetVerbose()) {
			CString strServerInfo;
			if (iSockAddrLen > 0 && sockAddr.sin_addr.s_addr != 0 && sockAddr.sin_addr.s_addr != INADDR_NONE)
				strServerInfo.Format(_T(" from %s:%d"), (LPCTSTR)ipstr(sockAddr.sin_addr), ntohs(sockAddr.sin_port) - 4);
			DebugLogError(_T("Error: Server UDP socket: Failed to receive data%s: %s"), (LPCTSTR)strServerInfo, (LPCTSTR)EscPercent(GetErrorMessage(dwError, 1)));
		}
	}
}
		...
		case OP_GLOBSERVSTATRES:
			{
				...
				if (size >= 40) {
					...
					DEBUG_ONLY(DebugLog(_T("New UDP key for server %s: UDPKey %u - Old Key %u"), (LPCTSTR)EscPercent(pServer->GetListName()), dwServerUDPKey, pServer->GetServerKeyUDP()));
...
void CUDPSocket::ProcessPacketError(UINT size, UINT opcode, uint32 nIP, uint16 nUDPPort, LPCTSTR pszError)
{
	if (thePrefs.GetVerbose()) {
		...
		DebugLogWarning(LOG_DEFAULT, _T("Error: Failed to process server UDP packet from %s:%u%s opcode=0x%02x size=%u - %s"), (LPCTSTR)ipstr(nIP), nUDPPort, (LPCTSTR)EscPercent(strName), opcode, size, pszError);
	}
}
...
void CUDPSocket::DnsLookupDone(WPARAM wp, LPARAM lp)
{
	...
	if (WSAGETASYNCERROR(lp) != 0) {
		if (thePrefs.GetVerbose())
			DebugLogWarning(_T("Error: Server UDP socket: Failed to resolve address for server '%s' (%s) - %s"), (LPCTSTR)EscPercent(pDNSReq->m_pServer->GetListName()), pDNSReq->m_pServer->GetAddress(), (LPCTSTR)EscPercent(GetErrorMessage(WSAGETASYNCERROR(lp), 1)));
		delete pDNSReq;
		return;
	}
	...
	if (nIP != INADDR_NONE) {
		...
		if (!bRemoveServer && thePrefs.GetFilterServerByIP() && theApp.ipfilter->IsFiltered(nIP)) {
			if (thePrefs.GetLogFilteredIPs())
				AddDebugLogLine(false, _T("IPFilter(UDP/DNSResolve): Filtered server \"%s\" (IP=%s) - IP filter (%s)"), pDNSReq->m_pServer->GetAddress(), (LPCTSTR)ipstr(nIP), (LPCTSTR)EscPercent(theApp.ipfilter->GetLastHit()));
			bRemoveServer = true;
		}
		...
		if (thePrefs.GetVerbose())
			DebugLogWarning(_T("Error: Server UDP socket: Failed to resolve address for server '%s' (%s)"), (LPCTSTR)EscPercent(pDNSReq->m_pServer->GetListName()), pDNSReq->m_pServer->GetAddress());
	}
	delete pDNSReq;
}
...
void CUDPSocket::OnSend(int nErrorCode)
{
	if (nErrorCode) {
		if (thePrefs.GetVerbose())
			DebugLogError(_T("Error: Server UDP socket: Failed to send packet - %s"), (LPCTSTR)EscPercent(GetErrorMessage(nErrorCode, 1)));
		return;
	}
	m_bWouldBlock = false;
...
void CUDPSocket::SendPacket(Packet *packet, CServer *pServer, uint16 nSpecialPort, BYTE *pInRawPacket, uint32 nRawLen)
{
	...
	if (packet != NULL) {
		...
		if (iLen) {
			...
			if (thePrefs.GetDebugServerUDPLevel() > 0)
				DEBUG_ONLY(DebugLog(_T("Sending encrypted packet to server %s, UDPKey %u"), (LPCTSTR)EscPercent(pServer->GetListName()), pServer->GetServerKeyUDP()));
			if (!nPort)
	...
	if (nIP == INADDR_NONE) {
		...
		if (pDNSReq->m_hDNSTask == NULL) {
			if (thePrefs.GetVerbose())
				DebugLogWarning(_T("Error: Server UDP socket: Failed to resolve address for '%s' - %s"), (LPCSTR)pServer->GetAddress(), (LPCTSTR)EscPercent(GetErrorMessage(CAsyncSocket::GetLastError(), 1)));
			delete pDNSReq;
			delete[] pRawPacket;
			return;
		}
