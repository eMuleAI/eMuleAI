...
CColorButton::CColorButton() :
	_Inherited(),
	m_Color(CLR_DEFAULT),
	m_DefaultColor(GetCustomSysColor(COLOR_APPWORKSPACE)),
...
BOOL CColorButton::OnClicked()
{
	...
	CWnd *pParent = GetParent();
	ApplyTheme(pPopup->GetSafeHwnd()); // dark mode style the popup immediately
...
void CColorButton::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	if (IsDarkModeEnabled()) {
		// fill button face with dark-mode brush
		pDC->FillRect(&rDraw, &CDarkMode::m_brBtn);
		// draw raised or sunken edge for normal/pressed state
		UINT uEdge = (uFrameState & DFCS_PUSHED) ? EDGE_SUNKEN : EDGE_RAISED;
		pDC->DrawEdge(&rDraw, uEdge, BF_RECT);
	} else
	...
	DrawArrow(pDC
		, &rArrow
		, 0
		, (state & ODS_DISABLED) ? GetCustomSysColor(COLOR_GRAYTEXT) : GetCustomSysColor(COLOR_BTNTEXT)
	);
