...
CFileDetailDialogName::CFileDetailDialogName()
	...
	, m_bSelf()
{
	m_strCaption = GetResString(_T("SW_NAME"));
...
BOOL CFileDetailDialogName::OnInitDialog()
{
	...
	AddOrReplaceAnchor(this, IDC_FD_SN, TOP_LEFT, BOTTOM_RIGHT);
	AddOrReplaceAnchor(this, IDC_LISTCTRLFILENAMES, TOP_LEFT, BOTTOM_RIGHT);
	AddOrReplaceAnchor(this, IDC_TAKEOVER, BOTTOM_LEFT);
	AddOrReplaceAnchor(this, IDC_BUTTONSTRIP, BOTTOM_RIGHT);
	AddOrReplaceAnchor(this, IDC_FILENAME, BOTTOM_LEFT, BOTTOM_RIGHT);
...
void CFileDetailDialogName::Localize()
{
	if (!m_hWnd)
		return;
	SetTabTitle(_T("SW_NAME"), this);

	SetDlgItemText(IDC_TAKEOVER, GetResString(_T("TAKEOVER")));
	SetDlgItemText(IDC_BUTTONSTRIP, GetResString(_T("CLEANUP")));
	SetDlgItemText(IDC_FD_SN, GetResString(_T("SOURCENAMES")));
}
...
void CFileDetailDialogName::OnLvnColumnClick(LPNMHDR pNMHDR, LRESULT *pResult)
{
	...
	m_listFileNames.UpdateSortHistory(pNMListView->iSubItem + (sortAscending ? 0 : 10));
...
void CFileDetailDialogName::OnNmRClickList(LPNMHDR, LRESULT *pResult)
{
	...
	popupMenu.AppendMenu(flag, MP_MESSAGE, GetResString(_T("TAKEOVER")));
	popupMenu.AppendMenu(flag, MP_COPYSELECTED, GetResString(_T("COPY")));
	popupMenu.AppendMenu(MF_STRING, MP_RESTORE, GetResString(_T("SV_UPDATE")));
