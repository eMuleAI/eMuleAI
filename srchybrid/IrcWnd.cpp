...
static LPCTSTR const s_szTimeStampColorPrefix = EMPTY;	// default foreground color
...
void CIrcWnd::SetAllIcons()
{
	...
	iml.Add(CTempIconLoader(_T("DARK_BOLD")));
	iml.Add(CTempIconLoader(_T("DARK_ITALIC")));
	iml.Add(CTempIconLoader(_T("DARK_UNDERLINE")));
	iml.Add(CTempIconLoader(_T("DARK_COLOUR")));
	iml.Add(CTempIconLoader(_T("DARK_RESETFORMAT")));
...
void CIrcWnd::Localize()
{
	...
	m_wndFormat.SetBtnText(IDC_BOLD, GetResString(_T("BOLD")));
	m_wndFormat.SetBtnText(IDC_ITALIC, GetResString(_T("ITALIC")));
	m_wndFormat.SetBtnText(IDC_UNDERLINE, GetResString(_T("UNDERLINE")));
	m_wndFormat.SetBtnText(IDC_COLOUR, GetResString(_T("COLOUR")));
	m_wndFormat.SetBtnText(IDC_RESET, GetResString(_T("RESETFORMAT")));
}
...
BOOL CIrcWnd::OnInitDialog()
{
	...
	AddOrReplaceAnchor(this, IDC_BN_IRCCONNECT, BOTTOM_LEFT);
	AddOrReplaceAnchor(this, IDC_CLOSECHAT, BOTTOM_LEFT);
	AddOrReplaceAnchor(this, IDC_CHATSEND, BOTTOM_RIGHT);
	AddOrReplaceAnchor(this, m_wndFormat, BOTTOM_LEFT);
	AddOrReplaceAnchor(this, m_wndInput, BOTTOM_LEFT, BOTTOM_RIGHT);
	AddOrReplaceAnchor(this, m_wndNicks, TOP_LEFT, BOTTOM_LEFT);
	AddOrReplaceAnchor(this, m_wndChanList, TOP_LEFT, BOTTOM_RIGHT);
	AddOrReplaceAnchor(this, m_wndChanSel, TOP_LEFT, TOP_RIGHT);
...
void CIrcWnd::OnBnClickedIrcConnect()
{
	if (!m_bConnected) {
		...
		while (sInput.IsEmpty() || sInput.CompareNoCase(_T("emule")) == 0 || stristr(sInput, _T("emuleirc")) != NULL) {
			InputBox inputBox;
			inputBox.SetLabels(GetResString(_T("IRC_NEWNICK")), GetResString(_T("IRC_NEWNICKDESC")), sInput);
...
void CIrcWnd::AddStatus(const CString &sLine, bool bShowActivity, UINT uStatusCode)
{
	...
	if (sLine.IsEmpty())
		...
		COLORREF clr;
		if (sLine[0] == _T('*'))
			clr = GetCustomSysColor(COLOR_IRC_STATUS_MSG);
		else if (uStatusCode >= 400) {
			if (sLine[0] != _T('-'))
				cs += _T("-Error- ");
			clr = GetCustomSysColor(COLOR_IRC_INFO_MSG);
		} else
			clr = CLR_DEFAULT;
...
void CIrcWnd::AddInfoMessage(Channel *pChannel, const CString &sLine)
{
	...
	if (sLine[0] == _T('*'))
		AddColorLine(cs, pChannel->m_wndLog, GetCustomSysColor(COLOR_IRC_STATUS_MSG));
	else if (sLine[0] == _T('-') && sLine.Find(_T('-'), 1) >= 0)
		AddColorLine(cs, pChannel->m_wndLog, GetCustomSysColor(COLOR_IRC_INFO_MSG));
	else
...
void CIrcWnd::AddColorLine(const CString &line, CHTRichEditCtrl &wnd, COLORREF crForeground)
{
	...
	const COLORREF defaultForeground = wnd.GetForegroundColor();
	const COLORREF defaultBackground = wnd.GetBackgroundColor();
	const COLORREF baseForeground = crForeground;
	const COLORREF baseBackground = CLR_DEFAULT;
	COLORREF cr = baseForeground; // set start foreground color
	COLORREF bgcr = baseBackground; // set start background color
	const COLORREF *const pIrcColors = GetIrcColorTable();
	const UINT uIrcColorCount = GetIrcColorTableSize();
	auto ResolveColor = [](COLORREF color, COLORREF fallback) {
		return color == CLR_DEFAULT ? fallback : color;
	};
	auto AppendReadableText = [&](const CString &rText) {
		COLORREF adjustedForeground = cr;
		COLORREF adjustedBackground = bgcr;
		EnsureReadableTextColors(adjustedForeground, adjustedBackground, defaultForeground, defaultBackground);
		wnd.AppendColoredText(rText, adjustedForeground, adjustedBackground, dwMask);
	};
	CString text;
	while (line.GetLength() > index) {
		...
		case 0x03: // foreground & background colour
			...
			if (_istdigit(line[index])) {
				...
				if (iColour >= 0 && static_cast<UINT>(iColour) < uIrcColorCount) {
					// If the first colour is not valid, don't look for a second background colour!
					cr = pIrcColors[iColour]; //if the number is a valid colour index, set new foreground colour
					++index;
					if (line[index] == _T(',') && _istdigit(line[index + 1])) { //is there a background colour
						...
						++index;
						if (iColour >= 0 && static_cast<UINT>(iColour) < uIrcColorCount)
							bgcr = pIrcColors[iColour]; //if the number is a valid colour index, set new foreground colour
					}
				}
			} else {
				// reset
		...
		case 0x16: { // Reverse (as per mIRC) toggle
			...
			++index;
			const COLORREF resolvedCurrentForeground = ResolveColor(cr, defaultForeground);
			const COLORREF resolvedCurrentBackground = ResolveColor(bgcr, defaultBackground);
			const COLORREF resolvedBaseForeground = ResolveColor(baseForeground, defaultForeground);
			const COLORREF resolvedBaseBackground = ResolveColor(baseBackground, defaultBackground);
			if (resolvedCurrentForeground != resolvedBaseBackground || resolvedCurrentBackground != resolvedBaseForeground) {
				// set inverse
				cr = resolvedBaseBackground;
				bgcr = resolvedBaseForeground;
			} else {
				// reset fg/bk colours
				cr = baseForeground;
				bgcr = baseBackground;
...
{
	m_bConnected = bFlag;
	SetDlgItemText(IDC_BN_IRCCONNECT, GetResString(bFlag ? _T("IRC_DISCONNECT") : _T("IRC_CONNECT")));
	if (bFlag)
		AddStatus(GetResString(_T("CONNECTED")));
	else {
		m_bLoggedIn = false;
		//detach all channels
		for (POSITION pos = m_wndChanSel.m_lstChannels.GetHeadPosition(); pos != NULL;) {
			...
			if (pChannel->m_eType != Channel::ctChannelList)
				AddInfoMessageC(pChannel, GetCustomSysColor(COLOR_IRC_EVENT_MSG), _T("* ") + GetResString(_T("DISCONNECTED")));
		}
	}
}
...
void CIrcWnd::NoticeMessage(const CString &sSource, const CString &sTarget, const CString &sMessage)
{
	...
	if (m_wndChanSel.FindChannelByName(sTarget) != NULL)
		...
		if (!bFlag) {
			...
			if (pStatusChannel)
				AddColorLine(cs, pStatusChannel->m_wndLog, GetCustomSysColor(COLOR_IRC_INFO_MSG));
		}
		}
	}
...
void CIrcWnd::SetTopic(const CString &sChannel, const CString &sTopic)
{
	...
	pChannel->m_wndTopic.SetWindowText(EMPTY);
...
void CIrcWnd::OnBnClickedIrcSend()
{
	...
	m_wndInput.SetWindowText(EMPTY);
	...
	try {
		if (sCommands.GetLength() >= 2) {
			...
			if (!thePrefs.GetIRCIgnoreMiscMessages())
				AddInfoMessageF(sChannel, GetResString(_T("IRC_SETSMODE")), (LPCTSTR)sChanger, (LPCTSTR)sCommandsOrig, (LPCTSTR)sParamsOrig);
		}
	} catch (...) {
		AddInfoMessage(sChannel, GetResString(_T("IRC_NOTSUPPORTED")));
		ASSERT(0);
	}
}
...
void CIrcWnd::OnBnClickedColour()
{
	...
	new CColourPopup(CPoint(rDraw.left + 1, rDraw.bottom - 92)	// Point to display popup
					...
					, this										// parent
					, GetResString(_T("DEFAULT"))					// "Default" text area
					, NULL										// Custom Text
					, (LPCOLORREF)GetIrcColorTable()				// Pointer to a COLORREF array
					, GetIrcColorTableSize());						// Size of the array
...
LRESULT CIrcWnd::OnSelEndOK(WPARAM wParam, LPARAM)
{
	...
	const COLORREF *const pIrcColors = GetIrcColorTable();
	int iColour = static_cast<int>(GetIrcColorTableSize());
	while (--iColour >= 0 && (COLORREF)wParam != pIrcColors[iColour]);
