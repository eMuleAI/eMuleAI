...
void CIrcSocket::OnReceive(int iErrorCode)
{
	if (iErrorCode) {
		if (thePrefs.GetVerbose())
			AddDebugLogLine(false, _T("IRC socket: Failed to read - %s"), (LPCTSTR)EscPercent(GetErrorMessage(iErrorCode, 1)));
		return;
	}
...
void CIrcSocket::OnSend(int iErrorCode)
{
	if (iErrorCode) {
		if (thePrefs.GetVerbose())
			AddDebugLogLine(false, _T("IRC socket: Failed to send - %s"), (LPCTSTR)EscPercent(GetErrorMessage(iErrorCode, 1)));
		return;
	}
...
void CIrcSocket::OnClose(int iErrorCode)
{
	if (iErrorCode && thePrefs.GetVerbose())
		AddDebugLogLine(false, _T("IRC socket: Failed to close - %s"), (LPCTSTR)EscPercent(GetErrorMessage(iErrorCode, 1)));
	m_pIrcMain->Disconnect();
}
