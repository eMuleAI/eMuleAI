...
{
	// Retrieve the inner CComboBox control
	CWnd* pCtrl = GetComboBoxCtrl();
	if (!pCtrl)
		return;

	HWND hInnerCombo = pCtrl->GetSafeHwnd();
	COMBOBOXINFO cbi = {};
	cbi.cbSize = sizeof(cbi);

	// Try to get the drop-down list window
	if (::GetComboBoxInfo(hInnerCombo, &cbi) && cbi.hwndList)
		ApplyThemeToWindow(cbi.hwndList, true, false);
}