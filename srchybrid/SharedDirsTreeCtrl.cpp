...
namespace
{
	const ULONGLONG kFnvOffsetBasis64 = 14695981039346656037ui64;
	const ULONGLONG kFnvPrime64 = 1099511628211ui64;

	void Fnv1aUpdateByte64(ULONGLONG& hash, BYTE value)
	{
		hash ^= value;
		hash *= kFnvPrime64;
	}

	ULONGLONG RotateLeft64(ULONGLONG value, unsigned int shift)
	{
		shift &= 63;
		return shift == 0 ? value : (value << shift) | (value >> (64 - shift));
	}

	CString NormalizeTreeStructurePath(const CString& path)
	{
		CString normalized(path);
		if (normalized.Right(1) == _T("\\"))
			normalized = normalized.Left(normalized.GetLength() - 1);
		return normalized;
	}

	bool MarkTreeStructurePathSeen(const CString& normalizedPath, CMapStringToPtr& seen)
	{
		if (normalizedPath.IsEmpty())
			return false;

		CString lower(normalizedPath);
		lower.MakeLower();
		void* pv = NULL;
		if (seen.Lookup(lower, pv))
			return false;

		seen.SetAt(lower, (void*)1);
		return true;
	}

	ULONGLONG HashTreeStructurePathNoCase(const CString& normalizedPath)
	{
		CString lower(normalizedPath);
		lower.MakeLower();

		ULONGLONG hash = kFnvOffsetBasis64;
		for (int i = 0; i < lower.GetLength(); ++i) {
			const WCHAR ch = lower[i];
			Fnv1aUpdateByte64(hash, (BYTE)(ch & 0xFF));
			Fnv1aUpdateByte64(hash, (BYTE)((ch >> 8) & 0xFF));
		}
		Fnv1aUpdateByte64(hash, 0x2F);
		return hash;
	}

	struct STreeStructureSignature
	{
		STreeStructureSignature()
			: ullSum(0)
			, ullXor(0)
			, ullSquareSum(0)
			, dwCount(0)
		...
		void AddPath(const CString& normalizedPath)
		{
			const ULONGLONG pathHash = HashTreeStructurePathNoCase(normalizedPath);
			ullSum += pathHash * 0x9E3779B185EBCA87ui64;
			ullXor ^= RotateLeft64(pathHash * 0xC2B2AE3D27D4EB4Fui64, (unsigned int)(pathHash & 63));
			ullSquareSum += (pathHash ^ 0x165667B19E3779F9ui64) * (pathHash ^ 0x85EBCA77C2B2AE63ui64);
			++dwCount;
		}

		ULONGLONG Finalize() const
		{
			ULONGLONG hash = kFnvOffsetBasis64;
			auto Mix = [&hash](ULONGLONG value) {
				for (int i = 0; i < 8; ++i) {
					Fnv1aUpdateByte64(hash, (BYTE)(value & 0xFF));
					value >>= 8;
				}
			};
		...
		ULONGLONG ullSum;
		ULONGLONG ullXor;
		ULONGLONG ullSquareSum;
		DWORD dwCount;
	};

	void AccumulateTreeStructurePath(const CString& path, CMapStringToPtr& seen, STreeStructureSignature& signature)
	{
		const CString normalized = NormalizeTreeStructurePath(path);
		if (!MarkTreeStructurePathSeen(normalized, seen))
			return;

		signature.AddPath(normalized);
	}


	CString BuildSharedTreePathKey(const CString& path)
	{
		CString key(path);
		slosh(key);
		key.MakeLower();
	...
	bool IsEffectivelySharedDirectory(const CString& dir)
	{
		if (theApp.sharedfiles != NULL)
			return theApp.sharedfiles->IsSharedByDirectoryRules(dir);
		...
		for (POSITION pos = sharedDirs.GetHeadPosition(); pos != NULL;) {
			CString strShared(sharedDirs.GetNext(pos));
			slosh(strShared);
			if (EqualPaths(strShared, sDir))
				return true;
			if (thePrefs.GetAutoShareSubdirs() && IsSubDirectoryOf(sDir, strShared))
				return true;
		}
	...
	CDirectoryItem* InsertSharedDirectoryItem(CSharedDirsTreeCtrl* pThis, CDirectoryItem* pParentItem, const CString& path, bool bTopFolder, bool bAccessible, bool& rbShowWarning)
	{
		const CString strName(GetFolderLabel(path, bTopFolder, bAccessible));
		CDirectoryItem* pNewItem = new CDirectoryItem(path);
		pNewItem->m_htItem = pThis->InsertItem(TVIF_TEXT | TVIF_PARAM | TVIF_IMAGE | TVIF_SELECTEDIMAGE, strName, 5, 5, 0, 0, (LPARAM)pNewItem, pParentItem->m_htItem, TVI_SORT);
		if (!bAccessible) {
			pThis->SetItemState(pNewItem->m_htItem, INDEXTOOVERLAYMASK(2), TVIS_OVERLAYMASK);
			rbShowWarning = true;
		}

		if (thePrefs.GetAutoShareSubdirs() && bAccessible) {
			TVITEM tvi = {};
			tvi.mask = TVIF_CHILDREN;
			tvi.hItem = pNewItem->m_htItem;
	...
	{
		explicit SSharedTreeEntry(const CString& path)
			: strPath(path)
			, iParentIndex(-1)
			, bAccessible(false)
			, pItem(NULL)
	...
	CString GetSharedTreeParentPath(const CString& path)
	{
		CString parent(path);
		slosh(parent);
		if (parent.IsEmpty())
		...
		unslosh(parent);
		const int iPos = parent.ReverseFind(_T('\\'));
		if (iPos < 0)
			return CString();

		parent = parent.Left(iPos + 1);
		return parent;
	}

	CDirectoryItem* FindNearestSharedTreeParentItem(const CString& path, CDirectoryItem* pRoot, CMapStringToPtr& rInsertedItems)
	{
		CString strParent = GetSharedTreeParentPath(path);
		while (!strParent.IsEmpty()) {
			void* pvItem = NULL;
			if (rInsertedItems.Lookup(BuildSharedTreePathKey(strParent), pvItem))
	...
	void BuildSharedDirectoryTree(CSharedDirsTreeCtrl* pThis, CDirectoryItem* pRoot, const CStringList& liDirs, bool& rbShowWarning)
	{
		CStringList visibleDirs;
		for (POSITION pos = liDirs.GetHeadPosition(); pos != NULL;)
			visibleDirs.AddTail(liDirs.GetNext(pos));
		if (thePrefs.GetAutoShareSubdirs())
			CPreferences::CollapseSharedDirsToRoots(visibleDirs);

		std::vector<SSharedTreeEntry> aEntries;
		aEntries.reserve((size_t)visibleDirs.GetCount());

		CMapStringToPtr mapSeen;
		for (POSITION pos = visibleDirs.GetHeadPosition(); pos != NULL;) {
			CString strDir(visibleDirs.GetNext(pos));
			slosh(strDir);
			if (strDir.IsEmpty())
				continue;
		...
		if (thePrefs.GetAutoShareSubdirs()) {
			CMapStringToPtr mapInsertedItems;
			for (INT_PTR i = 0; i < (INT_PTR)aEntries.size(); ++i) {
				SSharedTreeEntry& current = aEntries[(size_t)i];
				current.bAccessible = IsAccessibleDirectoryLongPath(current.strPath);
				current.pItem = InsertSharedDirectoryItem(pThis, pRoot, current.strPath, true, current.bAccessible, rbShowWarning);
				mapInsertedItems.SetAt(BuildSharedTreePathKey(current.strPath), current.pItem);
			}

			std::vector<SSharedTreeEntry> aExplicitEntries;
			aExplicitEntries.reserve((size_t)liDirs.GetCount());

			CMapStringToPtr mapExplicitSeen;
			for (POSITION pos = liDirs.GetHeadPosition(); pos != NULL;) {
				CString strDir(liDirs.GetNext(pos));
				slosh(strDir);
				if (strDir.IsEmpty())
					continue;
				...
				void* pvSeen = NULL;
				if (mapExplicitSeen.Lookup(strKey, pvSeen))
					continue;

				mapExplicitSeen.SetAt(strKey, (void*)1);
				aExplicitEntries.emplace_back(strDir);
			}

			std::sort(aExplicitEntries.begin(), aExplicitEntries.end(), [](const SSharedTreeEntry& left, const SSharedTreeEntry& right) {
				const int iLeftLen = left.strPath.GetLength();
				const int iRightLen = right.strPath.GetLength();
				if (iLeftLen != iRightLen)
					return iLeftLen < iRightLen;
			...
			for (INT_PTR i = 0; i < (INT_PTR)aExplicitEntries.size(); ++i) {
				SSharedTreeEntry& current = aExplicitEntries[(size_t)i];
				void* pvExisting = NULL;
				if (mapInsertedItems.Lookup(BuildSharedTreePathKey(current.strPath), pvExisting))
					continue;

				CDirectoryItem* pParentItem = FindNearestSharedTreeParentItem(current.strPath, pRoot, mapInsertedItems);
				const bool bParentAccessible = (pParentItem == pRoot) ? true : IsAccessibleDirectoryLongPath(pParentItem->m_strFullPath);
				current.bAccessible = bParentAccessible ? IsAccessibleDirectoryLongPath(current.strPath) : false;
				current.pItem = InsertSharedDirectoryItem(pThis, pParentItem, current.strPath, pParentItem == pRoot, current.bAccessible, rbShowWarning);
				mapInsertedItems.SetAt(BuildSharedTreePathKey(current.strPath), current.pItem);
			}
			return;
		}
		...
		for (INT_PTR i = 0; i < (INT_PTR)aEntries.size(); ++i)
			mapIndexByKey.SetAt(aEntries[(size_t)i].strKey, (void*)(i + 1));

		for (INT_PTR i = 0; i < (INT_PTR)aEntries.size(); ++i) {
			SSharedTreeEntry& current = aEntries[(size_t)i];

			CString strParent = GetSharedTreeParentPath(current.strPath);
			while (!strParent.IsEmpty()) {
				...
				if (mapIndexByKey.Lookup(strParentKey, pvIndex)) {
					current.iParentIndex = (int)((INT_PTR)pvIndex - 1);
					break;
				}
			...
			CDirectoryItem* pParentItem = pRoot;
			bool bParentAccessible = true;
			if (current.iParentIndex >= 0) {
				SSharedTreeEntry& parent = aEntries[(size_t)current.iParentIndex];
				if (parent.pItem != NULL)
					pParentItem = parent.pItem;
				bParentAccessible = parent.bAccessible;
			}

			current.bAccessible = bParentAccessible ? IsAccessibleDirectoryLongPath(current.strPath) : false;
			current.pItem = InsertSharedDirectoryItem(pThis, pParentItem, current.strPath, current.iParentIndex < 0, current.bAccessible, rbShowWarning);
		}
	}
}
...
HTREEITEM CDirectoryItem::FindItem(CDirectoryItem *pContentToFind) const
{
	...
	if (pContentToFind->m_eItemType == m_eItemType && EqualPaths(pContentToFind->m_strFullPath, m_strFullPath) && pContentToFind->m_nCatFilter == m_nCatFilter)
		return m_htItem;
...
BOOL CSharedDirsTreeCtrl::OnEraseBkgnd(CDC*)
{
	return TRUE;
}
...
static void FilterTreePopulateBranch(CSharedDirsTreeCtrl* pThis, CDirectoryItem* pCurrent, const CStringList& liSharedDirs, CStringList& rCatIncomingDirs, bool bIncludeSharedDirectoryTree, bool bIncludeRecursiveIncomingSubdirs);

void CSharedDirsTreeCtrl::LocalizeSkeleton()
{
	...
	for (POSITION pos = m_pRootDirectoryItem->liSubDirectories.GetHeadPosition(); pos != NULL;)
		FilterTreePopulateBranch(this, m_pRootDirectoryItem->liSubDirectories.GetNext(pos), m_strliSharedDirs, m_strliCatIncomingDirs, true, false);
	if (m_pRootDirectoryItem != NULL && !m_pRootDirectoryItem->liSubDirectories.IsEmpty())
...
void CSharedDirsTreeCtrl::InitalizeStandardItems()
{
	...
	pDir->m_htItem = InsertItem(TVIF_TEXT | TVIF_PARAM | TVIF_IMAGE | TVIF_SELECTEDIMAGE | TVIF_STATE, GetResString(_T("SHARED_DIRECTORIES")), 5, 5, 0, 0, (LPARAM)pDir, pAll->m_htItem, TVI_LAST);
...
static void FilterTreeAddEd2kTypeNodes(CSharedDirsTreeCtrl* pThis, CDirectoryItem* pParent)
{
	for (int i = 0; i < _countof(_aEd2kTypeView); ++i) {
		...
		pEd2kType->m_htItem = pThis->InsertItem(TVIF_TEXT | TVIF_PARAM | TVIF_IMAGE | TVIF_SELECTEDIMAGE, GetResString(_aEd2kTypeView[i].uStringId), i + 7, i + 7, 0, 0, (LPARAM)pEd2kType, pParent->m_htItem, TVI_LAST);
		pParent->liSubDirectories.AddTail(pEd2kType);
	}

	CDirectoryItem* pOther = new CDirectoryItem(CString(EMPTY), 0, SDI_ED2KFILETYPE, ED2KFT_OTHER);
	pOther->m_htItem = pThis->InsertItem(TVIF_TEXT | TVIF_PARAM | TVIF_IMAGE | TVIF_SELECTEDIMAGE, GetResString(_T("STATS_PRTOTHER")), 15, 15, 0, 0, (LPARAM)pOther, pParent->m_htItem, TVI_LAST);
	pParent->liSubDirectories.AddTail(pOther);
}

static void FilterTreeAddIncomingCategoryNodes(CSharedDirsTreeCtrl* pThis, CDirectoryItem* pParent, CStringList& rCatIncomingDirs, bool bIncludeRecursiveSubdirs)
{
	const CString strMainIncDir(thePrefs.GetMuleDirectory(EMULE_INCOMINGDIR));
	bool bShowWarning = false;

	if (thePrefs.GetCatCount() > 1) {
		rCatIncomingDirs.RemoveAll();
		for (INT_PTR i = 0; i < thePrefs.GetCatCount(); ++i) {
			...
			if (!strCatIncomingPath.IsEmpty() && strCatIncomingPath.CompareNoCase(strMainIncDir) != 0 && rCatIncomingDirs.Find(strCatIncomingPath) == NULL) {
				rCatIncomingDirs.AddTail(strCatIncomingPath);
				...
				pCatInc->m_htItem = pThis->InsertItem(TVIF_TEXT | TVIF_PARAM | TVIF_IMAGE | TVIF_SELECTEDIMAGE, strName, 5, 5, 0, 0, (LPARAM)pCatInc, pParent->m_htItem, TVI_SORT);
				if (!bAccessible) {
					pThis->SetItemState(pCatInc->m_htItem, INDEXTOOVERLAYMASK(2), TVIS_OVERLAYMASK);
					bShowWarning = true;
				}
				pParent->liSubDirectories.AddTail(pCatInc);
				if (bIncludeRecursiveSubdirs)
					FilterTreeAddIncomingSubdirectories(pThis, pCatInc, strCatIncomingPath, 0, bShowWarning);
			}
		}
	}

	if (bIncludeRecursiveSubdirs && !strMainIncDir.IsEmpty())
		FilterTreeAddIncomingSubdirectories(pThis, pParent, strMainIncDir, 0, bShowWarning);

	pThis->SetItemState(pParent->m_htItem, bShowWarning ? INDEXTOOVERLAYMASK(2) : 0, TVIS_OVERLAYMASK);
}

static void FilterTreeAddTempCategoryNodes(CSharedDirsTreeCtrl* pThis, CDirectoryItem* pParent)
{
	if (thePrefs.GetCatCount() <= 1)
		return;
	...
	for (INT_PTR i = 0; i < thePrefs.GetCatCount(); ++i) {
		...
		pCatTemp->m_htItem = pThis->InsertItem(TVIF_TEXT | TVIF_PARAM | TVIF_IMAGE | TVIF_SELECTEDIMAGE, thePrefs.GetCategoryDisplayTitle(i), 3, 3, 0, 0, (LPARAM)pCatTemp, pParent->m_htItem, TVI_LAST);
		pParent->liSubDirectories.AddTail(pCatTemp);
	}
}
...
static void FilterTreePopulateBranch(CSharedDirsTreeCtrl* pThis, CDirectoryItem* pCurrent, const CStringList& liSharedDirs, CStringList& rCatIncomingDirs, bool bIncludeSharedDirectoryTree, bool bIncludeRecursiveIncomingSubdirs)
{
	...
	case SDI_DUP:
	case SDI_ALLHISTORY:
		FilterTreeAddEd2kTypeNodes(pThis, pCurrent);
		break;
	case SDI_INCOMING:
		FilterTreeAddIncomingCategoryNodes(pThis, pCurrent, rCatIncomingDirs, bIncludeRecursiveIncomingSubdirs);
		break;
	case SDI_TEMP:
		FilterTreeAddTempCategoryNodes(pThis, pCurrent);
		break;
	case SDI_DIRECTORY:
		if (bIncludeSharedDirectoryTree) {
			bool bShowWarning = false;
			BuildSharedDirectoryTree(pThis, pCurrent, liSharedDirs, bShowWarning);
			pThis->SetItemState(pCurrent->m_htItem, bShowWarning ? INDEXTOOVERLAYMASK(2) : 0, TVIS_OVERLAYMASK);
		}
		break;
	default:
		ASSERT(0);
...
// Helper: add immediate filesystem subdirectories under a virtual tree node (used for Incoming folder)
static void FilterTreeAddIncomingSubdirectories(CSharedDirsTreeCtrl* pThis, CDirectoryItem* pParent, const CString& basePath, int nLevel, bool &rbShowWarning)
{
	UNREFERENCED_PARAMETER(nLevel);

	// Add only the visible level. Deep expansion is handled lazily by TVN_ITEMEXPANDING.
	EnumSubdirectoriesLongPath(basePath, [pThis, pParent, &rbShowWarning](const CString& childFullPath) {
		const bool bAccessible = IsAccessibleDirectoryLongPath(childFullPath);
		...
		tvi.mask = TVIF_CHILDREN;
		tvi.hItem = pNewItem->m_htItem;
		tvi.cChildren = bAccessible ? I_CHILDRENCALLBACK : 0;
		pThis->SetItem(&tvi);
		pParent->liSubDirectories.AddTail(pNewItem);
	});
}

static void FilterTreeAddAutoSharedImmediateSubdirectories(CSharedDirsTreeCtrl* pThis, CDirectoryItem* pParent, const CString& basePath, bool& rbShowWarning)
{
	EnumSubdirectoriesLongPath(basePath, [pThis, pParent, &rbShowWarning](const CString& childFullPath) {
		if (!IsEffectivelySharedDirectory(childFullPath))
			return;

		const bool bAccessible = IsAccessibleDirectoryLongPath(childFullPath);
		CDirectoryItem* pNewItem = InsertSharedDirectoryItem(pThis, pParent, childFullPath, false, bAccessible, rbShowWarning);
		if (pNewItem != NULL && bAccessible) {
			TVITEM tvi = {};
			tvi.mask = TVIF_CHILDREN;
			tvi.hItem = pNewItem->m_htItem;
			tvi.cChildren = I_CHILDRENCALLBACK;
...
void CSharedDirsTreeCtrl::FilterTreeReloadTree()
{
	...
	for (POSITION pos = m_pRootDirectoryItem->liSubDirectories.GetHeadPosition(); pos != NULL;) {
		...
		FilterTreePopulateBranch(this, pCurrent, m_strliSharedDirs, m_strliCatIncomingDirs, true, thePrefs.GetAutoShareSubdirs());
	}
	...
	m_bCreatingTree = false;
	m_ullLastTreeStructCRC = ComputeTreeStructureCRC(); // Update structure signature after a successful rebuild
	SetRedraw(TRUE);
	Invalidate(FALSE);
}

// Build a stable signature for the currently visible filter tree structure. Shared directories and incoming
// subdirectory changes are detected from persisted roots only; visible tree branches expand lazily.
ULONGLONG CSharedDirsTreeCtrl::ComputeTreeStructureCRC() const
{
	CMapStringToPtr seen;
	STreeStructureSignature signature;

	// Keep this signature cheap. The previous implementation recursively scanned every shared
	// directory from the UI thread and could freeze the Files window for large trees.
	AccumulateTreeStructurePath(thePrefs.GetAutoShareSubdirs() ? _T("<auto-share-subdirs:on>") : _T("<auto-share-subdirs:off>"), seen, signature);

	CStringList sharedDirs;
	thePrefs.CopySharedDirectoryList(sharedDirs);
	for (POSITION pos = sharedDirs.GetHeadPosition(); pos != NULL;)
		AccumulateTreeStructurePath(sharedDirs.GetNext(pos), seen, signature);

	AccumulateTreeStructurePath(thePrefs.GetMuleDirectory(EMULE_INCOMINGDIR), seen, signature);
	for (INT_PTR i = 0; i < thePrefs.GetCatCount(); ++i) {
		const Category_Struct* pCat = thePrefs.GetCategory(i);
		if (pCat != NULL)
			AccumulateTreeStructurePath(pCat->strIncomingPath, seen, signature);
	}
...
void CSharedDirsTreeCtrl::OnContextMenu(CWnd*, CPoint point)
{
	...
	// Rebuild popup menus on demand so runtime preference changes are reflected immediately.
	CreateMenus();
...
static CString NormalizeFileSystemTreeRefreshPath(const CString& rawPath)
{
...
static CDirectoryItem* FindSpecialDirectoryItem(CDirectoryItem* pRoot, ESpecialDirectoryItems eItemType)
{
	if (pRoot == NULL)
		return NULL;

	for (POSITION pos = pRoot->liSubDirectories.GetHeadPosition(); pos != NULL;) {
		CDirectoryItem* pChild = pRoot->liSubDirectories.GetNext(pos);
		if (pChild != NULL && pChild->m_eItemType == eItemType)
			return pChild;
	}
...
static CString GetFileSystemTreeRefreshParentPath(const CString& rawPath)
{
	CString path(NormalizeFileSystemTreeRefreshPath(rawPath));
	if (path.IsEmpty())
		return CString();
	...
	if (!::PathRemoveFileSpec(path.GetBuffer())) {
		path.ReleaseBuffer();
		return CString();
	}
...
static CDirectoryItem* FindLoadedDirectoryItemRecursive(CDirectoryItem* pRoot, const CString& targetPath)
{
	if (pRoot == NULL)
		return NULL;

	if (!pRoot->m_strFullPath.IsEmpty() && EqualPaths(pRoot->m_strFullPath, targetPath))
		return pRoot;

	for (POSITION pos = pRoot->liSubDirectories.GetHeadPosition(); pos != NULL;) {
		CDirectoryItem* pChild = pRoot->liSubDirectories.GetNext(pos);
		CDirectoryItem* pMatch = FindLoadedDirectoryItemRecursive(pChild, targetPath);
		if (pMatch != NULL)
			return pMatch;
	}
...
static bool IsTreeItemExpanded(const CTreeCtrl* pTree, HTREEITEM hItem)
{
	return pTree != NULL && hItem != NULL && (pTree->GetItemState(hItem, TVIS_EXPANDED) & TVIS_EXPANDED) != 0;
}

static bool IsSharedRefreshTargetAncestor(const CDirectoryItem* pCandidate, const CDirectoryItem* pOther)
{
	if (pCandidate == NULL || pOther == NULL)
		return false;
	if (pCandidate == pOther)
		return true;
	if (pCandidate->m_eItemType == SDI_DIRECTORY)
		return true;
	if (pCandidate->m_strFullPath.IsEmpty() || pOther->m_strFullPath.IsEmpty())
		return false;
	return IsSubDirectoryOf(pOther->m_strFullPath, pCandidate->m_strFullPath);
}

static bool IsDirectoryItemSameOrDescendantOf(const CDirectoryItem* pCandidate, const CDirectoryItem* pAncestor)
{
	if (pCandidate == NULL || pAncestor == NULL)
		return false;
	if (pCandidate == pAncestor)
		return true;
	if (!pAncestor->m_strFullPath.IsEmpty() && !pCandidate->m_strFullPath.IsEmpty()) {
		if (EqualPaths(pCandidate->m_strFullPath, pAncestor->m_strFullPath))
			return true;
		if (IsSubDirectoryOf(pCandidate->m_strFullPath, pAncestor->m_strFullPath))
			return true;
	}
	return pAncestor->m_eItemType == SDI_DIRECTORY && pCandidate->m_eItemType == SDI_NO && !pCandidate->m_strFullPath.IsEmpty();
}

static void SortComparablePaths(std::vector<CString>& paths)
{
	std::sort(paths.begin(), paths.end(), [](const CString& left, const CString& right) {
		return left.CompareNoCase(right) < 0;
	});
}

static void CollectImmediateSubdirectoryPaths(const CString& rootPath, std::vector<CString>& outPaths)
{
	outPaths.clear();
	EnumSubdirectoriesLongPath(rootPath, [&outPaths](const CString& childFullPath) {
		CString normalized(NormalizeFileSystemTreeRefreshPath(childFullPath));
		if (!normalized.IsEmpty())
			outPaths.push_back(normalized);
	});
	SortComparablePaths(outPaths);
}

static bool ImmediateFileSystemChildrenDiffer(const CDirectoryItem* pParent)
{
	if (pParent == NULL || pParent->m_strFullPath.IsEmpty() || !IsAccessibleDirectoryLongPath(pParent->m_strFullPath))
		return false;

	std::vector<CString> diskChildren;
	std::vector<CString> loadedChildren;
	CollectImmediateSubdirectoryPaths(pParent->m_strFullPath, diskChildren);
	loadedChildren.reserve((size_t)pParent->liSubDirectories.GetCount());

	for (POSITION pos = pParent->liSubDirectories.GetHeadPosition(); pos != NULL;) {
		const CDirectoryItem* pChild = pParent->liSubDirectories.GetNext(pos);
		if (pChild == NULL || pChild->m_strFullPath.IsEmpty())
			continue;

		CString normalized(NormalizeFileSystemTreeRefreshPath(pChild->m_strFullPath));
		if (!normalized.IsEmpty())
			loadedChildren.push_back(normalized);
	}

	SortComparablePaths(loadedChildren);
	if (loadedChildren.size() != diskChildren.size())
		return true;

	for (size_t i = 0; i < loadedChildren.size(); ++i) {
		if (!EqualPaths(loadedChildren[i], diskChildren[i]))
			return true;
	}
...
static HTREEITEM DetermineRefreshFallbackSelection(const std::vector<CDirectoryItem*>& refreshParents, const CDirectoryItem* pSelected)
{
	if (pSelected == NULL)
		return NULL;

	for (size_t i = 0; i < refreshParents.size(); ++i) {
		CDirectoryItem* pParent = refreshParents[i];
		if (pParent != NULL && pParent->m_htItem != NULL && IsDirectoryItemSameOrDescendantOf(pSelected, pParent))
			return pParent->m_htItem;
	}

	return NULL;
}

static void RestoreRefreshedSelection(CSharedDirsTreeCtrl* pTree, CDirectoryItem* pRoot, CDirectoryItem* pOldSelectedItem, HTREEITEM hFallbackSelection)
{
	if (pTree == NULL || pRoot == NULL)
		return;

	if (pOldSelectedItem != NULL) {
		const HTREEITEM hOldSelection = pRoot->FindItem(pOldSelectedItem);
		if (hOldSelection != NULL) {
			pTree->Select(hOldSelection, TVGN_CARET);
			pTree->EnsureVisible(hOldSelection);
			return;
		}
	}

	if (hFallbackSelection != NULL && pTree->GetSelectedItem() != hFallbackSelection) {
		pTree->Select(hFallbackSelection, TVGN_CARET);
		pTree->EnsureVisible(hFallbackSelection);
	}
}

static CDirectoryItem* FindLoadedSharedRefreshParent(CSharedDirsTreeCtrl* pTree, CDirectoryItem* pSharedRoot, const CString& changedDir)
{
	if (pTree == NULL || pSharedRoot == NULL)
		return NULL;

	const CString normalizedChanged(NormalizeFileSystemTreeRefreshPath(changedDir));
	if (normalizedChanged.IsEmpty())
		return NULL;

	CDirectoryItem* pLoadedChanged = FindLoadedDirectoryItemRecursive(pSharedRoot, normalizedChanged);
	if (pLoadedChanged != NULL && pLoadedChanged->m_htItem != NULL) {
		HTREEITEM hParent = pTree->GetParentItem(pLoadedChanged->m_htItem);
		if (hParent != NULL) {
			CDirectoryItem* pParent = reinterpret_cast<CDirectoryItem*>(pTree->GetItemData(hParent));
			if (pParent != NULL && IsTreeItemExpanded(pTree, pParent->m_htItem))
				return pParent;
		}
	}

	CString probe(GetSharedTreeParentPath(normalizedChanged));
	while (!probe.IsEmpty()) {
		CDirectoryItem* pLoadedParent = FindLoadedDirectoryItemRecursive(pSharedRoot, probe);
		if (pLoadedParent != NULL && pLoadedParent->m_htItem != NULL && IsTreeItemExpanded(pTree, pLoadedParent->m_htItem))
			return pLoadedParent;

		probe = GetSharedTreeParentPath(probe);
	}
...
void CSharedDirsTreeCtrl::RefreshVisibleFileSystemBranches(const CStringArray& changedDirs)
{
	if (changedDirs.GetCount() == 0 || m_pRootUnsharedDirectries == NULL)
		return;

	std::vector<CDirectoryItem*> refreshParents;
	refreshParents.reserve((size_t)changedDirs.GetCount());

	for (INT_PTR i = 0; i < changedDirs.GetCount(); ++i) {
		const CString parentPath(GetFileSystemTreeRefreshParentPath(changedDirs[i]));
		if (parentPath.IsEmpty())
			continue;

		CDirectoryItem* pParent = FindLoadedDirectoryItemRecursive(m_pRootUnsharedDirectries, parentPath);
		if (pParent == NULL || pParent->m_htItem == NULL || !IsTreeItemExpanded(this, pParent->m_htItem))
			continue;

		bool bSkip = false;
		for (size_t k = 0; k < refreshParents.size();) {
			CDirectoryItem* pExisting = refreshParents[k];
			if (IsDirectoryItemSameOrDescendantOf(pParent, pExisting)) {
				bSkip = true;
				break;
			}

			if (IsDirectoryItemSameOrDescendantOf(pExisting, pParent)) {
				refreshParents.erase(refreshParents.begin() + k);
				continue;
			}
		...
		if (!bSkip)
			refreshParents.push_back(pParent);
	}

	if (refreshParents.empty())
		return;

	CDirectoryItem* pOldSelectedItem = NULL;
	CDirectoryItem* pSelectedItem = GetSelectedFilter();
	HTREEITEM hFallbackSelection = DetermineRefreshFallbackSelection(refreshParents, pSelectedItem);
	if (pSelectedItem != NULL)
		pOldSelectedItem = pSelectedItem->CloneContent();

	SetRedraw(FALSE);
	if (hFallbackSelection != NULL && GetSelectedItem() != hFallbackSelection)
		Select(hFallbackSelection, TVGN_CARET);

	for (size_t i = 0; i < refreshParents.size(); ++i) {
		CDirectoryItem* pParent = refreshParents[i];
		DeleteChildItems(pParent);
		FileSystemTreeAddSubdirectories(pParent);

		TVITEM tvi = {};
		tvi.mask = TVIF_CHILDREN;
		tvi.hItem = pParent->m_htItem;
		tvi.cChildren = FileSystemTreeHasSubdirectories(pParent->m_strFullPath) ? I_CHILDRENCALLBACK : 0;
		SetItem(&tvi);
	}

	FileSystemTreeUpdateShareState();
	FileSystemTreeUpdateBoldState();
	RestoreRefreshedSelection(this, m_pRootUnsharedDirectries, pOldSelectedItem, hFallbackSelection);
	delete pOldSelectedItem;
	SetRedraw(TRUE);
	Invalidate(FALSE);
}

bool CSharedDirsTreeCtrl::RefreshVisibleFileSystemBranchesIfNeeded()
{
	if (!::IsWindow(m_hWnd) || !IsWindowVisible() || m_pRootUnsharedDirectries == NULL || m_pRootUnsharedDirectries->m_htItem == NULL || !IsTreeItemExpanded(this, m_pRootUnsharedDirectries->m_htItem))
		return false;

	std::vector<CDirectoryItem*> refreshParents;
	const int nVisibleCount = static_cast<int>(GetVisibleCount());
	const int nVisibleBudget = (nVisibleCount + 4 > 16) ? (nVisibleCount + 4) : 16;
	HTREEITEM hItem = GetFirstVisibleItem();
	for (int i = 0; hItem != NULL && i < nVisibleBudget; ++i, hItem = GetNextVisibleItem(hItem)) {
		CDirectoryItem* pItem = reinterpret_cast<CDirectoryItem*>(GetItemData(hItem));
		if (pItem == NULL || pItem->m_eItemType != SDI_UNSHAREDDIRECTORY || !IsTreeItemExpanded(this, hItem))
			continue;
		if (!ImmediateFileSystemChildrenDiffer(pItem))
			continue;

		bool bSkip = false;
		for (size_t k = 0; k < refreshParents.size();) {
			CDirectoryItem* pExisting = refreshParents[k];
			if (IsDirectoryItemSameOrDescendantOf(pItem, pExisting)) {
				bSkip = true;
				break;
			}

			if (IsDirectoryItemSameOrDescendantOf(pExisting, pItem)) {
				refreshParents.erase(refreshParents.begin() + k);
				continue;
			}
		...
		if (!bSkip)
			refreshParents.push_back(pItem);
	}
...
void CSharedDirsTreeCtrl::RefreshVisibleSharedBranches(const CStringArray& changedDirs)
{
	if (changedDirs.GetCount() == 0 || m_pRootDirectoryItem == NULL)
		return;

	CDirectoryItem* pSharedRoot = FindSpecialDirectoryItem(m_pRootDirectoryItem, SDI_DIRECTORY);
	if (pSharedRoot == NULL || pSharedRoot->m_htItem == NULL || !IsTreeItemExpanded(this, pSharedRoot->m_htItem))
		return;
	...
	for (INT_PTR i = 0; i < changedDirs.GetCount(); ++i) {
		CDirectoryItem* pRefreshParent = FindLoadedSharedRefreshParent(this, pSharedRoot, changedDirs[i]);
		if (pRefreshParent == NULL || pRefreshParent->m_htItem == NULL)
			continue;

		bool bSkip = false;
		for (size_t k = 0; k < refreshParents.size();) {
			CDirectoryItem* pExisting = refreshParents[k];
			if (IsSharedRefreshTargetAncestor(pExisting, pRefreshParent)) {
				bSkip = true;
				break;
			}

			if (IsSharedRefreshTargetAncestor(pRefreshParent, pExisting)) {
				refreshParents.erase(refreshParents.begin() + k);
				continue;
			}
		...
		if (!bSkip)
			refreshParents.push_back(pRefreshParent);
	}
	...
	for (size_t i = 0; i < refreshParents.size(); ++i) {
		...
		bool bShowWarning = false;
		if (pParent->m_eItemType == SDI_DIRECTORY)
			BuildSharedDirectoryTree(this, pParent, sharedDirs, bShowWarning);
		else {
			const bool bParentAccessible = IsAccessibleDirectoryLongPath(pParent->m_strFullPath);
			if (thePrefs.GetAutoShareSubdirs() && bParentAccessible)
				FilterTreeAddAutoSharedImmediateSubdirectories(this, pParent, pParent->m_strFullPath, bShowWarning);
			else
				FilterTreeAddSubDirectories(pParent, sharedDirs, 0, bShowWarning, bParentAccessible);
		}

		SetItemState(pParent->m_htItem, bShowWarning ? INDEXTOOVERLAYMASK(2) : 0, TVIS_OVERLAYMASK);
	}

	RestoreRefreshedSelection(this, m_pRootDirectoryItem, pOldSelectedItem, hFallbackSelection);
	delete pOldSelectedItem;
	SetRedraw(TRUE);
	Invalidate(FALSE);
}
...
void CSharedDirsTreeCtrl::OnTvnItemexpanding(LPNMHDR pNMHDR, LRESULT *pResult)
{
	...
	if (pExpanded != NULL && pNMTreeView->action == TVE_EXPAND) {
		...
		} else if (pExpanded->m_eItemType == SDI_NO && thePrefs.GetAutoShareSubdirs() && !pExpanded->m_strFullPath.IsEmpty()) {
			DeleteChildItems(pExpanded);
			bool bShowWarning = false;
			FilterTreeAddAutoSharedImmediateSubdirectories(this, pExpanded, pExpanded->m_strFullPath, bShowWarning);
			SetItemState(pExpanded->m_htItem, bShowWarning ? INDEXTOOVERLAYMASK(2) : 0, TVIS_OVERLAYMASK);
		}
	} else if (pExpanded == NULL)
		ASSERT(0);
...
void CSharedDirsTreeCtrl::DeleteChildItems(CDirectoryItem *pParent)
{
	while (!pParent->liSubDirectories.IsEmpty()) {
		...
		if (pToDelete->m_htItem != NULL && ::IsWindow(m_hWnd))
...
{
	if (theApp.sharedfiles != NULL)
		return theApp.sharedfiles->IsSharedByDirectoryRules(strDir);

	for (POSITION pos = m_strliSharedDirs.GetHeadPosition(); pos != NULL;) {
		const CString &strShared(m_strliSharedDirs.GetNext(pos));
		if (EqualPaths(strShared, strDir))
			return true;
		if (thePrefs.GetAutoShareSubdirs() && IsSubDirectoryOf(strDir, strShared))
			return true;
	}
	return false;
}
...
void CSharedDirsTreeCtrl::AddSharedDirectory(const CString &strDir, bool bSubDirectories)
{
	...
	for (POSITION pos = m_strliSharedDirs.GetHeadPosition(); pos != NULL;) {
		if (EqualPaths(m_strliSharedDirs.GetNext(pos), sDir)) {
			bAlreadyStored = true;
			break;
		}
	}

	if (!bAlreadyStored && thePrefs.IsShareableDirectory(sDir))
...
void CSharedDirsTreeCtrl::EditSharedDirectories(const CDirectoryItem *pDir, bool bAdd, bool bSubDirectories)
{
	...
	// AutoShareSubdirs means a manual parent decision becomes the authoritative rule for the whole subtree.
	const bool bAutoShareSubdirs = thePrefs.GetAutoShareSubdirs();
	const bool bRecursiveListUpdate = bSubDirectories || bAutoShareSubdirs;
	const bool bUpdateLoadedChildren = bRecursiveListUpdate;
	const bool bClearExcludedSubdirs = bRecursiveListUpdate;
	const bool bSyncWholeSubTree = bAutoShareSubdirs && pDir->m_eItemType != SDI_DIRECTORY;
	const bool bWasEffectivelyShared = (pDir->m_eItemType != SDI_DIRECTORY) && FileSystemTreeIsShared(pDir->m_strFullPath);

	if (bAdd) {
		if (theApp.sharedfiles != NULL)
			theApp.sharedfiles->RemoveExcludedSharedDirectory(pDir->m_strFullPath, bClearExcludedSubdirs);

		if (bSyncWholeSubTree)
			RemoveSharedDirectory(pDir->m_strFullPath, true);

		const bool bNeedsRecursiveUpgrade = bRecursiveListUpdate && !bAutoShareSubdirs;
		if (bAutoShareSubdirs)
			AddSharedDirectory(pDir->m_strFullPath, false);
		else if (!FileSystemTreeIsShared(pDir->m_strFullPath) || bNeedsRecursiveUpgrade)
			AddSharedDirectory(pDir->m_strFullPath, bRecursiveListUpdate);
	} else if (pDir->m_eItemType == SDI_DIRECTORY) {
		RemoveAllSharedDirectories();
		if (theApp.sharedfiles != NULL)
			theApp.sharedfiles->ClearExcludedSharedDirectories();
	} else {
		RemoveSharedDirectory(pDir->m_strFullPath, bRecursiveListUpdate);
		if (theApp.sharedfiles != NULL)
			theApp.sharedfiles->RemoveExcludedSharedDirectory(pDir->m_strFullPath, bClearExcludedSubdirs);
	}

	// Sync the persisted root list before recalculating visual state, because effective sharing
	// now also depends on excluded subdirectories stored by CSharedFileList.
	thePrefs.ReplaceSharedDirectoryList(m_strliSharedDirs);

	if (!bAdd && pDir->m_eItemType != SDI_DIRECTORY && theApp.sharedfiles != NULL
		&& bWasEffectivelyShared
		&& !theApp.sharedfiles->ShouldBeShared(pDir->m_strFullPath, NULL, true))
	{
		theApp.sharedfiles->AddExcludedSharedDirectory(pDir->m_strFullPath);
	}
	...
	if (pDir->m_eItemType == SDI_NO || pDir->m_eItemType == SDI_DIRECTORY)
		...
		FileSystemTreeSetShareState(pDir, bUpdateLoadedChildren);

	if (pDir->m_eItemType == SDI_UNSHAREDDIRECTORY) {
		if (bUpdateLoadedChildren)
			FileSystemTreeUpdateBoldState(pDir);
		else
			SetItemState(pDir->m_htItem, ((FileSystemTreeHasSharedSubdirectory(pDir->m_strFullPath, true) || FileSystemTreeIsShared(pDir->m_strFullPath)) ? TVIS_BOLD : 0), TVIS_BOLD);
	} else
		FileSystemTreeUpdateBoldState();
	...
	if (GetSelectedFilter() != NULL && GetSelectedFilter()->m_eItemType == SDI_UNSHAREDDIRECTORY && m_pSharedFilesCtrl != NULL) // if in file system view, update the list to reflect the changes in the checkboxes
		m_pSharedFilesCtrl->Invalidate(FALSE);
...
void CSharedDirsTreeCtrl::Reload(bool bForce)
{
	...
	if (!bForce) {
		...
		if (!bForce) {
			...
			if (!bForce) {
				ULONGLONG curCrc = ComputeTreeStructureCRC();
				if (curCrc != m_ullLastTreeStructCRC)
					bForce = true;
			}
		}
	}
...
void CSharedDirsTreeCtrl::FetchSharedDirsList()
{
	...
	CStringList sharedDirs;
	thePrefs.CopySharedDirectoryList(sharedDirs);
	m_strliSharedDirs.AddTail(&sharedDirs);
}
...
bool CSharedDirsTreeCtrl::ShowSharedDirectory(const CString &strDir)
{
	...
	for (POSITION pos = m_pRootDirectoryItem->liSubDirectories.GetHeadPosition(); pos != NULL;) {
		...
		if (pTemp->m_eItemType == SDI_DIRECTORY) {
			...
			CDirectoryItem target(strDir);
			const HTREEITEM htMatch = pTemp->FindItem(&target);
			if (htMatch != NULL) {
				Select(htMatch, TVGN_CARET);
				EnsureVisible(htMatch);
				return true;
			}
		}
	}
