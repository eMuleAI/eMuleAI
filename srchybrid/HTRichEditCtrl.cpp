...
void CHTRichEditCtrl::ForcePurgeSmileysForShutdown()
{
	// Force global smiley caches to be freed regardless of client refcount.
	sm_iSmileyClients = 0;
	PurgeSmileyCaches();
}
...
void CHTRichEditCtrl::OnContextMenu(CWnd*, CPoint point)
{
	...
	menu.AddMenuSidebar(GetResString(_T("LOGENTRY")));
	menu.AppendMenu(MF_STRING | (lSelEnd > lSelStart ? MF_ENABLED : MF_GRAYED), MP_COPYSELECTED, GetResString(_T("COPY")));
	menu.AppendMenu(MF_SEPARATOR);
	menu.AppendMenu(MF_STRING | (iTextLen > 0 ? MF_ENABLED : MF_GRAYED), MP_SELECTALL, GetResString(_T("SELECTALL")));
	menu.AppendMenu(MF_STRING | (iTextLen > 0 ? MF_ENABLED : MF_GRAYED), MP_REMOVEALL, GetResString(_T("PW_RESET")));
	menu.AppendMenu(MF_STRING | (iTextLen > 0 ? MF_ENABLED : MF_GRAYED), MP_SAVELOG, GetResString(_T("SAVELOG")) + _T("..."));
	menu.AppendMenu(MF_SEPARATOR);
	menu.AppendMenu(MF_STRING | (m_bAutoScroll ? MF_CHECKED : MF_UNCHECKED), MP_AUTOSCROLL, GetResString(_T("AUTOSCROLL")));
...
bool CHTRichEditCtrl::SaveLog(LPCTSTR pszDefName)
{
	...
	if (dlg.DoModal() == IDOK) {
		...
		if (!IsWin32LongPathsEnabled() && savePath.GetLength() >= MAX_PATH) {
			CString strError;
			strError.Format(_T("Failed to create log file \"%s\" - path too long (%u). Enable long path support to allow this."), (LPCTSTR)savePath, (UINT)savePath.GetLength());
			CDarkMode::MessageBox(strError, MB_ICONERROR);
		} else {
			const CString longPath = PreparePathForWin32LongPath(savePath);
			HANDLE hFile = ::CreateFile(longPath, GENERIC_WRITE, FILE_SHARE_READ, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
			if (hFile != INVALID_HANDLE_VALUE) {
				DWORD _dwWritten = 0;
				const WCHAR _wBOM = 0xFEFF;
				BOOL _bOK = ::WriteFile(hFile, &_wBOM, sizeof(_wBOM), &_dwWritten, NULL);
				CString strText;
				if (_bOK) {
					GetWindowText(strText);
					const DWORD cbText = (DWORD)(strText.GetLength() * sizeof(TCHAR));
					_bOK = ::WriteFile(hFile, (LPCTSTR)strText, cbText, &_dwWritten, NULL) && (_dwWritten == cbText);
				}

				if (!_bOK) {
					CString strError;
					strError.Format(_T("Failed to write log file \"%s\" - %s"), (LPCTSTR)savePath, (LPCTSTR)EscPercent(GetErrorMessage(::GetLastError())));
					CDarkMode::MessageBox(strError, MB_ICONERROR);
				} else
					bResult = true;
				...
				CString strError;
				strError.Format(_T("Failed to create log file \"%s\" - %s"), (LPCTSTR)savePath, (LPCTSTR)EscPercent(GetErrorMessage(::GetLastError())));
				CDarkMode::MessageBox(strError, MB_ICONERROR);
			}
		}
	}
...
void CHTRichEditCtrl::ApplySkin()
{
	if (m_strSkinKey.IsEmpty()) {
		...
		m_crForeground = m_bDfltForeground ? GetCustomSysColor(COLOR_WINDOWTEXT) : m_crDfltForeground;

		m_bDfltBackground = m_crDfltBackground == CLR_DEFAULT;
		m_crBackground = m_bDfltBackground ? GetCustomSysColor(COLOR_WINDOW) : m_crDfltBackground;
