...
{
	if (m_hWnd) {
		SetDlgItemText(IDC_GRAPHS, GetResString(_T("GRAPHS")));
		...
		int iItem;
		iItem = m_colors.AddString(GetResString(_T("SP_BACKGROUND")));
		m_colors.SetItemData(iItem, 0);
		iItem = m_colors.AddString(GetResString(_T("SP_GRID")));
		m_colors.SetItemData(iItem, 1);

		iItem = m_colors.AddString(GetResString(_T("SP_DL3")));
		m_colors.SetItemData(iItem, 4);
		iItem = m_colors.AddString(GetResString(_T("SP_DL2")));
		m_colors.SetItemData(iItem, 3);
		iItem = m_colors.AddString(GetResString(_T("SP_DL1")));
		m_colors.SetItemData(iItem, 2);

		iItem = m_colors.AddString(GetResString(_T("SP_UL3")));
		m_colors.SetItemData(iItem, 7);
		iItem = m_colors.AddString(GetResString(_T("SP_UL2")));
		m_colors.SetItemData(iItem, 6);
		iItem = m_colors.AddString(GetResString(_T("SP_UL1")));
		m_colors.SetItemData(iItem, 5);
		iItem = m_colors.AddString(GetResString(_T("SP_ULSLOTSNOOVERHEAD")));
		m_colors.SetItemData(iItem, 14);
		iItem = m_colors.AddString(GetResString(_T("SP_ULFRIENDS")));
		m_colors.SetItemData(iItem, 13);

		iItem = m_colors.AddString(GetResString(_T("SP_ACTCON")));
		m_colors.SetItemData(iItem, 8);
		iItem = m_colors.AddString(GetResString(_T("SP_ACTUL")));
		m_colors.SetItemData(iItem, 10);
		iItem = m_colors.AddString(GetResString(_T("SP_TOTALUL")));
		m_colors.SetItemData(iItem, 9);
		iItem = m_colors.AddString(GetResString(_T("SP_ACTDL")));
		m_colors.SetItemData(iItem, 12);
		iItem = m_colors.AddString(GetResString(_T("SP_ICONBAR")));
...
void CPPgStats::ShowInterval()
{
	...
	if (m_iGraphsUpdate == 0)
		strLabel = GetResString(_T("DISABLED"));
	else
		strLabel.Format(GetResString(_T("STATS_UPDATELABEL")), m_iGraphsUpdate);
	SetDlgItemText(IDC_SLIDERINFO, strLabel);

	if (m_iGraphsAvgTime == 0)
		strLabel = GetResString(_T("DISABLED"));
	else
		strLabel.Format(GetResString(_T("STATS_UPDATELABEL")), m_iGraphsAvgTime);
	SetDlgItemText(IDC_SLIDERINFO2, strLabel);

	strLabel.Format(GetResString(_T("STATS_AVGLABEL")), m_iStatsUpdate);
