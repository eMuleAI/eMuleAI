...
bool CUPnPImplWinServ::IsReady()
{
	...
	if (!bResult)
		Log(GetResString(_T("UPNP_NOSERVICE")));
...
void CUPnPImplWinServ::AddDevice(DevicePointer device, bool bAddChildren, int nLevel)
{
	...
	if (deviceSet == m_pDevices.end()) {
		...
		DebugLog(_T("Found UPnP device: %s (ChildLevel: %i, UID: %s)"), (LPCTSTR)EscPercent(CString(bsFriendlyName)), nLevel, (LPCTSTR)EscPercent(CString(bsUniqueName)));
	}
...
{
	DebugLog(_T("Finder asked to remove: %ls"), (LPCTSTR)EscPercent((LPCWSTR)bsUDN));
	...
	if (device != m_pDevices.end()) {
		DebugLog(_T("Device removed: %s"), (LPCTSTR)EscPercent((LPCTSTR)bsUDN));
...
HRESULT CUPnPImplWinServ::MapPort(const ServicePointer &service)
{
	...
	DebugLog(_T("Got status info from the service %s: %s"), (LPCTSTR)EscPercent(strServiceId), (LPCTSTR)EscPercent(strResult));
	...
	if (stristr(strResult, _T("|VT_BSTR=Connected|")) != NULL) {
		...
		if (FAILED(hr))
			...
			DebugLog(_T("Callback added for the service %s"), (LPCTSTR)EscPercent(strServiceId));
	...
	do {
		...
		if (SUCCEEDED(hr) && !strActionResult.IsEmpty()) {
			...
			if (oTokens.GetCount() != 8) {
				DebugLogWarning(_T("GetGenericPortMappingEntry delivered malformed response: '%s'"), (LPCTSTR)EscPercent(strActionResult));
				break;
			}
				...
				|| stristr(strActionResult, _T("|VT_BSTR=eMule UDP|")) != NULL)
			{
				...
				if (_tcsstr(oTokens[4], m_sLocalIP) != NULL || stristr(oTokens[4], szComputerName) != NULL) {
					...
					DebugLog(_T("Port %s is used by %s"), (LPCTSTR)EscPercent(oTokens[1]), (LPCTSTR)EscPercent(oTokens[4]));
...
HRESULT __stdcall CServiceCallback::ServiceInstanceDied(IUPnPService *pService)
{
	...
	HRESULT hr = pService->get_Id(&bsServiceId);
	if (SUCCEEDED(hr)) {
		DebugLogError(_T("UPnP service %s died"), (LPCTSTR)EscPercent((LPCTSTR)bsServiceId));
		return hr;
	}
