...
{
	m_strXaxis = GetResString(_T("TIME"));
	m_strYaxis = GetResString(_T("KADDISTANCE"));
	if (m_hWnd)
		Invalidate();
}
...
{
	if (pLookupHistory != NULL && m_pLookupHistory == pLookupHistory && m_hWnd)
		Invalidate();
}

void CKadLookupGraph::SetSearch(CLookupHistory *pLookupHistory)
{
	if (pLookupHistory == m_pLookupHistory) {
		if (m_hWnd)
			Invalidate();
		return;
	}
...
void CKadLookupGraph::OnPaint()
{
	...
	dc.FillSolidRect(rcClient, GetCustomSysColor(COLOR_WINDOW));
	rcClient.DeflateRect(1, 1, 1, 1);
	COLORREF crOldTextColor = dc.SetTextColor(GetCustomSysColor(COLOR_WINDOWTEXT));
	...
	if (m_pLookupHistory != NULL && m_pLookupHistory->GetHistoryEntries().GetCount() > 0) {
		const CArray<CLookupHistory::SLookupHistoryEntry*> &he = m_pLookupHistory->GetHistoryEntries();
		const INT_PTR hecount = he.GetCount();
		// How many nodes can we show without scrolling?
		int iMaxNodes = iHistWidth / NODE_ENTRY_WIDTH;
		int iNodeEntryWidth;
		if (hecount > iMaxNodes)
		...
		if (Gdiplus::GdiplusStartup(&gdiplusToken, &gdiplusStartupInput, NULL) == Gdiplus::Ok) {
			{
				...
				for (INT_PTR i = 0; i < (INT_PTR)iVisibleNodes; ++i) {
					...
					for (INT_PTR j = 0; j < static_cast<INT_PTR>(sEntry->m_liReceivedFromIdx.GetCount()); ++j) {
						INT_PTR iIdx = sEntry->m_liReceivedFromIdx[j];
						const INT_PTR iFromNode = hecount - (iIdx + 1);
						if (iIdx >= hecount - iVisibleNodes && iIdx < hecount && iFromNode >= 0 && iFromNode < m_aNodesDrawRects.GetCount()) {
							CPoint pFrom = m_aNodesDrawRects[iFromNode].CenterPoint();
							CPoint pointTo = m_aNodesDrawRects[i].CenterPoint();

							Gdiplus::Pen *pen;
							if (iFromNode == m_iHotItemIdx) {
								abHotItemConnected[i] = true;
								pen = &gdipPenRed;
							} else if (i == m_iHotItemIdx) {
								abHotItemConnected[iFromNode] = true;
								pen = &gdipPenDarkGray;
							} else
								pen = &gdipPenGray;
		...
		for (INT_PTR i = 0; i < (INT_PTR)iVisibleNodes; ++i) {
			...
			if (m_iHotItemIdx >= 0 && !abHotItemConnected[i])
				SafeImageListDrawEx(&m_iml, &dc, iIconIdx, pointNode, CSize(), CLR_NONE, GetCustomSysColor(COLOR_WINDOW), ILD_BLEND50);
			else
				SafeImageListDraw(&m_iml, &dc, iIconIdx, pointNode, ILD_NORMAL);
				...
				case Kademlia::CSearch::NOTES:
					{
						...
						SafeImageListDraw(&m_iml, &dc, 4, pointIndicator, ILD_NORMAL | INDEXTOOVERLAYMASK(nOverlayImage));
					}
					break;
				case Kademlia::CSearch::STOREFILE:
					SafeImageListDraw(&m_iml, &dc, 6, pointIndicator, ILD_NORMAL);
					break;
				case Kademlia::CSearch::STOREKEYWORD:
				case Kademlia::CSearch::STORENOTES:
					SafeImageListDraw(&m_iml, &dc, 5, pointIndicator, ILD_NORMAL);
				}
			}
		}
	}
...
{
	CString strToolText;
	if (m_iHotItemIdx >= 0 && m_iHotItemIdx < m_aNodesDrawRects.GetCount() && m_pLookupHistory != NULL && m_pLookupHistory->GetHistoryEntries().GetCount() > 0) {
		const CArray<CLookupHistory::SLookupHistoryEntry*> &he = m_pLookupHistory->GetHistoryEntries();
		const INT_PTR hecount = he.GetCount();
		INT_PTR iHotItemRealIdx = hecount - (m_iHotItemIdx + 1);
		if (iHotItemRealIdx < 0 || iHotItemRealIdx >= hecount) {
		...
		if (sEntry->m_liReceivedFromIdx.GetCount() == 0)
			strDiscovered = GetResString(_T("ROUTINGTABLE"));
		else
			strDiscovered.Format(_T("%i %s"), static_cast<int>(sEntry->m_liReceivedFromIdx.GetCount()), (LPCTSTR)GetResString(_T("NODES")));

		CString strFoundNodes;
		if (sEntry->m_dwAskedContactsTime > 0) {
			if (sEntry->m_uRespondedContact == 0) {
				if (::GetTickCount() >= sEntry->m_dwAskedContactsTime + SEC2MS(3))
					strFoundNodes = GetResString(_T("NORESPONSE"));
				else
					strFoundNodes = GetResString(_T("ASKING"));
			} else {
				unsigned useful = 0;
				for (INT_PTR i = hecount; --i >= 0;)
					for (INT_PTR j = static_cast<INT_PTR>(he[i]->m_liReceivedFromIdx.GetCount()); --j >= 0;)
			...
			strFoundNodes = GetResString(_T("NOTASKED"));
		...
		if (sEntry->m_dwAskedSearchItemTime > 0) {
			...
			strFoundResults = GetResString(_T("NOTASKED"));
		...
		strToolText.Format(_T("%s\n")
			...
			, (LPCTSTR)GetResString(_T("KADDISTANCE")), (LPCTSTR)sEntry->m_uDistance.ToHexString()
			, (LPCTSTR)GetResString(_T("PROTOCOLVERSION")), (unsigned)sEntry->m_byContactVersion
			, (LPCTSTR)GetResString(_T("DISCOVEREDBY")), (LPCTSTR)strDiscovered
			, (LPCTSTR)GetResString(_T("FOUNDNODESUSEFUL")), (LPCTSTR)strFoundNodes
			, (LPCTSTR)GetResString(_T("SW_RESULT")), (LPCTSTR)strFoundResults
		);
	}
	m_pToolTip->UpdateTipText(strToolText, this);
	m_pToolTip->Update();
...
void CKadLookupGraph::OnContextMenu(CWnd*, CPoint point)
{
	...
	menu.AppendMenu(MF_STRING, MP_AUTOKADLOOKUPGRAPH, GetResString(_T("AUTOKADLOOKUPGRAPH")));
