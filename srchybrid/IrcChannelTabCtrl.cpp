...
void CIrcChannelTabCtrl::Init()
{
	...
	m_pChanStatus = NewChannel(GetResString(_T("STATUS")), Channel::ctStatus);
...
Channel* CIrcChannelTabCtrl::NewChannel(const CString &sChannel, Channel::EType eType)
{
	...
	if (eType != Channel::ctChannelList) {
			...
			)
		{
			...
			if (GetCustomSysColor(COLOR_WINDOW) == RGB(255, 255, 255)) {
...
void CIrcChannelTabCtrl::ChatSend(CString sSend)
{
	...
	if (bMe) {
		...
		m_pParent->AddInfoMessageCF(m_pCurrentChannel->m_sName, GetCustomSysColor(COLOR_IRC_ACTION_MSG), _T("* %s %s"), (LPCTSTR)m_pParent->m_pIrcMain->GetNick(), CPTR(sSend, 4));
