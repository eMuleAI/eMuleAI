...
/////////////////////////////////////////////////////////////////////////////
// CDarkColorDialog
// subclass the standard color dialog to apply dark mode in OnInitDialog
class CDarkColorDialog : public CColorDialog
{
public:
	// inherit constructors
	using CColorDialog::CColorDialog;

protected:
	BOOL OnInitDialog() override
	{
		BOOL bResult = CColorDialog::OnInitDialog();
		// apply dark mode to all controls in the color dialog
						...
						, LPCTSTR szCustomText  /* = NULL */)
{
	...
	if (!CWnd::CreateEx(0, szClassName, EMPTY, WS_VISIBLE|WS_POPUP
...
void CColourPopup::CreateToolTips()
{
	...
	for (int i = 0; i < m_nNumColours; ++i) {
		...
		if (GetCellRect(i, &rect))
			m_ToolTip.AddTool(this, EMPTY, &rect, 1); //  GetColourName(i)
	}
...
void CColourPopup::EndSelection(int nMessage)
{
	...
	if (nMessage != UM_CPN_SELENDCANCEL && m_nCurrentSel == CUSTOM_BOX_VALUE) {
		...
		CDarkColorDialog dlg(m_crInitialColour, CC_FULLOPEN | CC_ANYCOLOR, this);
...
void CColourPopup::DrawCell(CDC *pDC, int nIndex)
{
	...
	if (m_strCustomText.GetLength() && nIndex == CUSTOM_BOX_VALUE) {
		...
		// Fill background
		pDC->FillSolidRect(TextButtonRect, GetCustomSysColor(COLOR_3DFACE));

		// Draw horizontal line
		pDC->FillSolidRect(m_CustomTextRect.left + 2 * m_nMargin, m_CustomTextRect.top
			, m_CustomTextRect.Width() - 4 * m_nMargin, 1, GetCustomSysColor(COLOR_3DSHADOW));
		pDC->FillSolidRect(m_CustomTextRect.left + 2 * m_nMargin, m_CustomTextRect.top + 1
			, m_CustomTextRect.Width() - 4 * m_nMargin, 1, GetCustomSysColor(COLOR_3DHILIGHT));

		TextButtonRect.DeflateRect(1, 1);

		// fill background
		bool b = (m_nChosenColourSel == nIndex && m_nCurrentSel != nIndex);
		pDC->FillSolidRect(TextButtonRect, GetCustomSysColor(b ? COLOR_3DLIGHT : COLOR_3DFACE));
	...
	if (m_strDefaultText.GetLength() && nIndex == DEFAULT_BOX_VALUE) {
		// Fill background
		pDC->FillSolidRect(m_DefaultTextRect, GetCustomSysColor(COLOR_3DFACE));
		...
		CPen pen(PS_SOLID, 1, GetCustomSysColor(COLOR_3DSHADOW));
	...
	// fill background
	bool b = (m_nChosenColourSel == nIndex && m_nCurrentSel != nIndex);
	pDC->FillSolidRect(rect, GetCustomSysColor(b ? COLOR_3DHILIGHT : COLOR_3DFACE));
	...
	CPen pen;
	pen.CreatePen(PS_SOLID, 1, GetCustomSysColor(COLOR_3DSHADOW));
