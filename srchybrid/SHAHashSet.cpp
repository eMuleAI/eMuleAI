...
// Rrecursively calculates missing hash from the existing ones overwrites existing hashes fails if no hash is found for any branch.
bool CAICHHashTree::ReCalculateHash(CAICHHashAlgo *hashalg, bool bDontReplace)
{
	// Allow incomplete branches: tree nodes can be created lazily and may temporarily have only one child. In that case there is nothing to recalculate at this level.
...
bool CAICHRecoveryHashSet::SaveHashSet()
{
	...
	// and CAICHSyncThread threads. So waiting for the lock will not hurt.
	lockKnown2Met.Lock();
...
void CAICHRecoveryHashSet::ClearStoredAICHHashes()
{
	// deterministically clear stored AICH hash positions before CRT leak check
	m_mapAICHHashsStored.RemoveAll();
}
