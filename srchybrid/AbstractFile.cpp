...
CString CAbstractFile::GetED2kLink(bool bHashset, bool bHTML, bool bHostname, bool bSource, CAddress IP) const
{
	CString strLink;
	...
	else if (bSource && !IP.IsNull())
		strLink.AppendFormat(_T("|sources,%s:%i|/"), ipstr(IP), thePrefs.GetPort());
