...
void CSplitterControl::CreateWnd(DWORD dwStyle, const CRect &rect, CWnd *pParent, UINT nID)
{
	...
	CStatic::Create(EMPTY, dwStyle, rc, pParent, nID);
...
void CSplitterControl::OnPaint()
{
	if (m_bDrawBorder) {
		...
		dc.FillSolidRect(&rcClient, GetCustomSysColor(COLOR_3DFACE));
		dc.Draw3dRect(&rcClient, GetCustomSysColor(COLOR_BTNHIGHLIGHT), GetCustomSysColor(COLOR_BTNSHADOW));
	} else
		CStatic::OnPaint();
}
