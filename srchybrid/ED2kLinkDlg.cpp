...
CED2kLinkDlg::CED2kLinkDlg()
	...
	, m_bReducedDlg()
{
	m_strCaption = GetResString(_T("SW_LINK"));
...
BOOL CED2kLinkDlg::OnInitDialog()
{
	...
	if (!m_bReducedDlg) {
		AddOrReplaceAnchor(this, IDC_LD_BASICGROUP, BOTTOM_LEFT, BOTTOM_RIGHT);
		AddOrReplaceAnchor(this, IDC_LD_SOURCECHE, BOTTOM_LEFT, BOTTOM_LEFT);
		AddOrReplaceAnchor(this, IDC_LD_ADVANCEDGROUP, BOTTOM_LEFT, BOTTOM_RIGHT);
		AddOrReplaceAnchor(this, IDC_LD_HTMLCHE, BOTTOM_LEFT, BOTTOM_LEFT);
		AddOrReplaceAnchor(this, IDC_LD_HASHSETCHE, BOTTOM_LEFT, BOTTOM_LEFT);
		AddOrReplaceAnchor(this, IDC_LD_HOSTNAMECHE, BOTTOM_LEFT, BOTTOM_LEFT);
	...
	AddOrReplaceAnchor(this, IDC_LD_LINKGROUP, TOP_LEFT, BOTTOM_RIGHT);
	AddOrReplaceAnchor(this, IDC_LD_LINKEDI, TOP_LEFT, BOTTOM_RIGHT);
	AddOrReplaceAnchor(this, IDC_LD_CLIPBOARDBUT, BOTTOM_RIGHT);
	Localize();
...
void CED2kLinkDlg::Localize()
{
	...
	if (!m_bReducedDlg) {
		...
		SetDlgItemText(IDC_LD_ADVANCEDGROUP, GetResString(_T("LD_ADVANCEDOPT")));
		SetDlgItemText(IDC_LD_HTMLCHE, GetResString(_T("LD_ADDHTML")));
		SetDlgItemText(IDC_LD_HASHSETCHE, GetResString(_T("LD_ADDHASHSET")));
		SetDlgItemText(IDC_LD_HOSTNAMECHE, GetResString(_T("LD_HOSTNAME")));
	}
}
...
void CED2kLinkDlg::UpdateLink()
{
	...
	const bool bSource = IsDlgButtonChecked(IDC_LD_SOURCECHE) != 0 && theApp.IsConnected() && !theApp.GetPublicIP().IsNull() && !theApp.IsFirewalled();
