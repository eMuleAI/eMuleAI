...
#define SUPPRESSTHISLOG // Comment this line to enable debug logs defined below
#endif

#ifdef SUPPRESSTHISLOG
	#define TRACE2 __noop
	#define ASSERT2(expr) __noop
	#define VERIFY2(expr) ((void)(expr))
	#define DEBUG_ONLY2(f) __noop
#else
	#define TRACE2(...) TRACE(__VA_ARGS__)
	#define ASSERT2(expr) ASSERT(expr)
	#define VERIFY2(expr) VERIFY(expr)
...
#define MAX_RESULTS				0			// 0 = unlimited. (Was 100) Server-side limits natively restrict search results, no need for a client-side cap.
...
#define SOURCECLIENTREASKS		MIN2MS(30)	// Decreased to 30m (was 40m). Balances faster general peer queries on modern connections while maintaining optimal internal algorithmic limits.
#define SOURCECLIENTREASKF		MIN2MS(3)	// Decreased to 3m (was 5m). Speeds up Source Exchange inquiries for files. Modern connections handle the frequency without overhead or duplicate results.
#define KADEMLIAASKTIME			SEC2MS(1)	//1 second
#define KADEMLIATOTALFILE		100			// Increased to 100 (was 5). Searches sources for more files simultaneously on modern hardware.
#define KADEMLIAPUBLISHTIME		SEC(2)		//2 second
#define KADEMLIAREASKTIME		MIN2MS(30)	// Decreased to 30 min (was 1 hour). Keeps source lists fresher as peers go online/offline frequently.
...
#define KADEMLIASEARCHLISTRELOADELAY 500	// Delay in ms before the kad search list is reloaded
#define RENDEZVOUSFALLBACKDELAY SEC2MS(12)  // TCP callback time before NAT-T fallback 
...
#define CLIENTBANTIME			HR2S(2) // 2h
#define TRACKED_CLEANUP_TIME	HR2S(1)	// 1 hour
#define KEEPTRACK_TIME			HR2S(2)	// 2h -	how long to keep track of clients which were once in the upload queue
#define LOCALSERVERREQUESTS		SEC2MS(20)	// only one local src request during this timespan (WHERE IS THIS USED?)
#define DISKSPACERECHECKTIME	15
#define CLIENTLIST_CLEANUP_TIME	MIN2S(34)	// 34 min
...
#define OP_ESERVER_BUDDY_DISCONNECT	0xB6	// Buddy disconnect notification
#define OP_ESERVER_RELAY_REQUEST	0xB7	// LowID→Buddy: Request relay
#define OP_ESERVER_RELAY_RESPONSE	0xB8	// Buddy→LowID: Target info
#define OP_ESERVER_PEER_INFO		0xB9	// Buddy→Target: Requester info
#define OP_ESERVER_PEER_INFO_ACK	0xBA	// Target→Buddy: Own info
#define OP_ESERVER_UDP_PROBE		0xBB	// LowID→Buddy (UDP): External UDP port probe

// eServer Buddy timeout constants
...
#define ESERVERBUDDY_MAGIC_MAX_PROBES_PER_ROUND	3u	// Max source probes in one discovery round

// eServer Buddy slot limits
#define ESERVERBUDDY_MIN_SLOTS			5		// Minimum buddy slots
#define ESERVERBUDDY_MAX_SLOTS			100		// Maximum buddy slots
#define ESERVERBUDDY_DEFAULT_SLOTS		5		// Default buddy slots
#define ESERVERBUDDY_MAX_REQUESTS_PER_WINDOW	3	// Max buddy requests per 10 min window
#define ESERVERBUDDY_RELAY_REQUEST_WINDOW	SEC2MS(20)	// Relay request rate-limit window per served buddy
#define ESERVERBUDDY_MAX_RELAY_REQUESTS_PER_WINDOW	12	// Max relay requests per window from one served buddy

// eServer Buddy status codes
#define ESERVERBUDDY_STATUS_REJECTED	0x00	// LowID rejection / general reject
...
#define ESERVERBUDDY_FLAG_MAGIC_PROOF	0x08	// Capability bit: supports magic-file proof on buddy requests
...
#define ESERVERBUDDY_MAGIC_FILE_SIZE ESERVERBUDDY_MAGIC_PROOF_FILE_SIZE
#define ESERVERBUDDY_MAGIC_FILE_HASH_BYTES ESERVERBUDDY_MAGIC_PROOF_FILE_HASH_BYTES
...
#define  FT_FILEPATH			 0x06	// <string>
...
#define TAG_SERVINGBUDDYHASH	"\xF8"	// <string>
...
#define ED2KFTSTR_OTHER			"Other" // This is not a native type and is added just to use for transfer window filter. Don't use elsewhere!
...
#define CT_EMULE_SERVINGBUDDYIP 0xfc
#define CT_EMULE_SERVINGBUDDYUDP 0xfd
...
#define CT_EMULE_ADDRESS		0xB0 // MOD SX
#define CT_EMULE_SERVERIP		0xBA // MOD SX
...
#define CT_MOD_RESERVED_5		0xA5
#define CT_MOD_RESERVED_6		0xA6
#define CT_ESERVER_BUDDY_FLAGS	0xA7 // eServer Buddy capability tag (Hello packet). Bit 0: Supports eServer Buddy, Bit 1: Has available slot, Bit 2: Supports external UDP port discovery, Bit 3: Supports magic-file proof
#define OP_SERVINGBUDDYPULL_REQ	0xA8 // Serving buddy info pull request
#define OP_SERVINGBUDDYPULL_RES	0xA9 // Serving buddy info pull response
#define CT_MOD_MISCOPTIONS		0xAA
#define CT_MOD_RESERVED_B		0xAB
#define OP_CHANGE_CLIENT_IP		0xAC // <NewIPv6 16>
#define CT_MOD_YOUR_IP			0xAD
#define CT_MOD_IP_V6			0xAE
#define CT_MOD_SVR_IP_V6		0xAF
...
#define KADEMLIA_FINDSERVINGBUDDY_REQ	0x51	// <TCPPORT (sender) [2]>
...
#define KADEMLIA_FINDSERVINGBUDDY_RES	0x5A	// <TCPPORT (sender) [2]>
