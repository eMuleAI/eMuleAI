...
void CIrcMain::PreParseMessage(const char *pszBufferA)
{
	...
	CATCH_DFLT_ALL_AFTER_MFC(_T(__FUNCTION__))
}
	...
	try {
			...
			if (sTarget[0] != _T('#'))
				...
				if (_tcsncmp(CPTR(sRawMessage, iIndex), _T(":\001"), 2) == 0) {
					...
					if (_tcsnicmp(CPTR(sRawMessage, iIndex + 2), _T("ACTION "), 7) == 0) {
						...
						//Channel Action.
						m_pwndIRC->AddInfoMessageCF(sTarget, GetCustomSysColor(COLOR_IRC_ACTION_MSG), _T("* %s %s"), (LPCTSTR)sNickname, (LPCTSTR)sMessage);
						return;
					}
				...
				if (_tcsnicmp(CPTR(sRawMessage, iIndex + 2), _T("RQSFRIEND"), 9) == 0) {
					...
					//Create our response.
					CString sBuild;
					sBuild.Format(_T("PRIVMSG %s :\001REPFRIEND %s%s|%s|%u:%u|%s:%s|%s|\001"), (LPCTSTR)sTarget, (LPCTSTR)theApp.GetAppVersion(), Irc_Version, (LPCTSTR)sVerify, theApp.IsFirewalled() ? 0 : theApp.GetID(), thePrefs.GetPort(), (LPCTSTR)sIP, (LPCTSTR)sPort, (LPCTSTR)md4str(thePrefs.GetUserHash()));
					m_pIRCSocket->SendString(sBuild);
					sBuild.Format(_T("%s %s"), (LPCTSTR)sTarget, (LPCTSTR)GetResString(_T("IRC_ADDASFRIEND")));
					if (!thePrefs.GetIRCIgnoreEmuleAddFriendMsgs())
						m_pwndIRC->NoticeMessage(_T("*EmuleProto*"), EMPTY, sBuild);
					return;
				}
				if (_tcsnicmp(CPTR(sRawMessage, iIndex + 2), _T("REPFRIEND"), 9) == 0) {
					...
					if (!theApp.friendlist->IsAlreadyFriend(sHash)) {
						...
						theApp.friendlist->AddFriend(ucharUserID, 0, CAddress(uNewClientID, false), uNewClientPort, 0, sTarget, 1);
					}
					return;
				}
				if (_tcsnicmp(CPTR(sRawMessage, iIndex + 2), _T("SENDLINK"), 8) == 0) {
					...
					if (!thePrefs.GetIRCAcceptLinks()) {
						if (!thePrefs.GetIRCIgnoreEmuleSendLinkMsgs())
							m_pwndIRC->NoticeMessage(_T("*EmuleProto*"), EMPTY, sTarget + _T(" attempted to send you a file. If you wanted to accept the files from this person, enable Receive files in the IRC Preferences."));
						return;
					}
					const CString &sHash(sRawMessage.Tokenize(_T("|"), iIndex));
					if (thePrefs.GetIRCAcceptLinksFriendsOnly() && !theApp.friendlist->IsAlreadyFriend(sHash)) {
						if (!thePrefs.GetIRCIgnoreEmuleSendLinkMsgs())
							m_pwndIRC->NoticeMessage(_T("*EmuleProto*"), EMPTY, sTarget + _T(" attempted to send you a file. If you wanted to accept the files from this person, add him as a friend or change your IRC Preferences."));
						return;
					}
					...
					if (!sLink.IsEmpty()) {
						if (!thePrefs.GetIRCIgnoreEmuleSendLinkMsgs()) {
							CString sBuild;
							sBuild.Format(GetResString(_T("IRC_RECIEVEDLINK")), (LPCTSTR)sTarget, (LPCTSTR)sLink);
							m_pwndIRC->NoticeMessage(_T("*EmuleProto*"), EMPTY, sBuild);
						}
						ProcessLink(sLink);
					}
		...
		if (sCommand == _T("JOIN")) {
			...
			if (!thePrefs.GetIRCIgnoreJoinMessages() || sNickname == m_sNick) {
				...
				m_pwndIRC->AddInfoMessageF(sChannel, GetResString(_T("IRC_HASJOINED")), (LPCTSTR)sNickname, (LPCTSTR)sChannel);
				if (sNickname == m_sNick)
					return;
			}
		...
		if (sCommand == _T("PART")) {
			...
			if (!thePrefs.GetIRCIgnorePartMessages())
				m_pwndIRC->AddInfoMessageF(sChannel, GetResString(_T("IRC_HASPARTED")), (LPCTSTR)sNickname, (LPCTSTR)sChannel, CPTR(sRawMessage, iIndex));
		...
		if (sCommand == _T("NICK")) {
			...
			if (!m_pwndIRC->m_wndNicks.ChangeAllNick(sNickname, sNewNick) && bOwnNick)
				m_pwndIRC->AddStatusF(GetResString(_T("IRC_NOWKNOWNAS")), (LPCTSTR)sNickname, (LPCTSTR)sNewNick);
			return;
		}
		...
		if (sCommand == _T("KICK")) {
			...
			if (!thePrefs.GetIRCIgnoreMiscMessages() || sNick == m_sNick) {
				if (sNick == m_sNick) { //It was you!
					...
					m_pwndIRC->AddStatusF(GetResString(_T("IRC_WASKICKEDBY")), (LPCTSTR)sNick, (LPCTSTR)sNickname, CPTR(sRawMessage, iIndex)); //is it needed?
				}
				m_pwndIRC->AddInfoMessageF(sChannel, GetResString(_T("IRC_WASKICKEDBY")), (LPCTSTR)sNick, (LPCTSTR)sNickname, CPTR(sRawMessage, iIndex));
			}
			//Remove nick from your channel.
			if (sNick != m_sNick)
...
// This also allows the help string to do more then join one channel. It could add other features later.
	if (thePrefs.GetIRCJoinHelpChannel())
		PerformString(GetResString(_T("IRC_HELPCHANNELPERFORM")));
}
...
void CIrcMain::Connect()
{
	try {
		...
		m_sVersion.Format(_T("%s%s"), (LPCTSTR)theApp.GetAppVersion(), (LPCTSTR)Irc_Version);
		m_sUser.Format(_T("USER %s 8 * :%s"), (LPCTSTR)sIdent, (LPCTSTR)m_sVersion);
		m_pIRCSocket->Create();
		m_pwndIRC->AddStatus(GetResString(_T("CONNECTING")));
