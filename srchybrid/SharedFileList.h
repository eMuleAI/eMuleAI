...
public:
	...
	void	SeedAutoShareSubdirsForRoot(const CString &rootDir);
...
public:
	...
	CSharedFileListSearchThread() : m_notify(true), m_busy(false) {}
	...
	// Pop will unmark the item from the in-queue set to prevent duplicates.
		FoundFile* PopFoundFile()
		{
			...
			if (!m_foundFiles.IsEmpty()) {
				...
				m_inQueue.RemoveKey(f->path); // Unmark: no longer queued
...
private:
	void CheckSingleFile(const WIN32_FIND_DATA& wfd, const CString& rootDir);
	...
	CMapStringToPtr m_inQueue;          // Paths currently enqueued (awaiting Pop)
