		...
		if (needMoreHosts) {
			...
			if (cnt > 0) {
				...
				for (cnt = 0; needMoreHosts && cnt < list.GetCount(); ++cnt) {
					AddHostToCheckNoLock(list.GetNext(pos)->GetIP().ToUInt32(false));
					if (pos == NULL)
						pos = list.GetHeadPosition();
				}
			}
...
CurrentPingStruct LastCommonRouteFinder::GetCurrentPing()
{
	...
	if (m_enabled) {
		pingLocker.Lock();
		if (!IsEmpty(m_strState))
			returnVal.state = GetResString(m_strState);
...
UINT LastCommonRouteFinder::RunInternal()
{
	...
	while (m_bRun) {
		...
		while (m_bRun && enabled) {
			...
			m_pingAverage = 0;
			m_lowestPing = 0;
			m_strState = _T("USS_STATE_PREPARING");
			pingLocker.Unlock();
			...
			while (m_bRun && enabled && !foundLastCommonHost) {
				...
				if (enabled) {
					...
					if (foundLastCommonHost) {
						...
						theApp.QueueDebugLogLine(false, GetResString(_T("USS_TRACEROUTEOFTENFAILED")));
						theApp.QueueLogLine(true, GetResString(_T("USS_TRACEROUTEOFTENFAILED")));
						enabled = false;
			...
			if (!m_enabled)
				...
				theApp.QueueDebugLogLine(false, GetResString(_T("USS_STARTING")));
				theApp.QueueLogLine(true, GetResString(_T("USS_STARTING")));
			}
			...
			while (m_bRun && enabled && !restart) {
				...
				if (::InterlockedExchange8(&m_initiateFastReactionPeriod, 0)) {
					theApp.QueueDebugLogLine(false, GetResString(_T("USS_MANUALUPLOADLIMITDETECTED")));
					theApp.QueueLogLine(true, GetResString(_T("USS_MANUALUPLOADLIMITDETECTED")));
