...
BOOL CCommentDialog::OnInitDialog()
{
	...
	AddOrReplaceAnchor(this, IDC_LST, TOP_LEFT, BOTTOM_RIGHT);
	AddOrReplaceAnchor(this, IDC_CMT_LQUEST, TOP_LEFT, TOP_RIGHT);
	AddOrReplaceAnchor(this, IDC_CMT_LAIDE, TOP_LEFT, TOP_RIGHT);
	AddOrReplaceAnchor(this, IDC_CMT_TEXT, TOP_LEFT, TOP_RIGHT);
	AddOrReplaceAnchor(this, IDC_RATEQUEST, TOP_LEFT, TOP_RIGHT);
	AddOrReplaceAnchor(this, IDC_RATEHELP, TOP_LEFT, TOP_RIGHT);
	AddOrReplaceAnchor(this, IDC_USERCOMMENTS, TOP_LEFT, BOTTOM_RIGHT);
	AddOrReplaceAnchor(this, IDC_RESET, TOP_RIGHT);
	AddOrReplaceAnchor(this, IDC_SEARCHKAD, BOTTOM_RIGHT);
...
BOOL CCommentDialog::OnSetActive()
{
	...
	if (m_bDataChanged) {
		...
		// This fixes a UI glitch which causes header control text to be erased when the tabs are updated.
		CHeaderCtrl* pHeaderCtrl = m_lstComments.GetHeaderCtrl();
		if (pHeaderCtrl)
			pHeaderCtrl->Invalidate(); //Force redraw
	}
...
{
	SetDlgItemText(IDC_CMT_TEXT, EMPTY);
	m_bMergedComment = false;
...
void CCommentDialog::Localize()
{
	...
	SetDlgItemText(IDC_RESET, GetResString(_T("PW_RESET")));

	SetDlgItemText(IDC_CMT_LQUEST, GetResString(_T("CMT_QUEST")));
	SetDlgItemText(IDC_CMT_LAIDE, GetResString(_T("CMT_AIDE")));

	SetDlgItemText(IDC_RATEQUEST, GetResString(_T("CMT_RATEQUEST")));
	SetDlgItemText(IDC_RATEHELP, GetResString(_T("CMT_RATEHELP")));

	SetDlgItemText(IDC_USERCOMMENTS, GetResString(_T("COMMENT")));
	...
	m_ratebox.AddItem(GetResString(_T("CMT_NOTRATED")), 0);
	m_ratebox.AddItem(GetResString(_T("CMT_FAKE")), 1);
	m_ratebox.AddItem(GetResString(_T("CMT_POOR")), 2);
	m_ratebox.AddItem(GetResString(_T("CMT_FAIR")), 3);
	m_ratebox.AddItem(GetResString(_T("CMT_GOOD")), 4);
	m_ratebox.AddItem(GetResString(_T("CMT_EXCELLENT")), 5);
...
void CCommentDialog::RefreshData(bool deleteOld)
{
	...
	if (Kademlia::CKademlia::IsConnected()) {
		SetDlgItemText(IDC_SEARCHKAD, GetResString(kadsearchable ? _T("SEARCHKAD") : _T("KADSEARCHACTIVE")));
...
void CCommentDialog::OnBnClickedSearchKad()
{
	if (m_bEnabled && Kademlia::CKademlia::IsConnected()) {
		...
		if (bSkipped)
			LocMessageBox(_T("KADSEARCHALREADY"), MB_OK | MB_ICONINFORMATION, 0);
	}
	RefreshData();
}
