...
int CALLBACK CIrcNickListCtrl::SortProc(const LPARAM lParam1, const LPARAM lParam2, const LPARAM lParamSort)
{
...
void CIrcNickListCtrl::OnContextMenu(CWnd*, CPoint point)
{
	...
	menuNick.AppendMenu(MF_STRING, Irc_AddFriend, GetResString(_T("IRC_ADDTOFRIENDLIST")));
	menuNick.AppendMenu(MF_STRING, Irc_SendLink, GetResString(_T("IRC_SENDLINK"))
		+ (m_pParent->GetSendFileString().IsEmpty() ? GetResString(_T("IRC_NOSFS")) : m_pParent->GetSendFileString()));
	menuNick.AppendMenu(MF_STRING, Irc_Kick, GetResString(_T("IRC_KICK")));
	menuNick.AppendMenu(MF_STRING, Irc_Ban, GetResString(_T("IRC_BAN")));
	//Ban currently uses chanserv to ban which seems to kick also. May change this later.
	menuNick.AppendMenu(MF_STRING, Irc_Slap, GetResString(_T("IRC_SLAP")));
...
void CIrcNickListCtrl::OpenPrivateChannel(const Nick *pNick)
{
	...
	m_pParent->AddInfoMessage(sNick, GetResString(_T("IRC_PRIVATECHANSTART")), true);
}
	...
	for (POSITION pos = m_pParent->m_wndChanSel.m_lstChannels.GetHeadPosition(); pos != NULL;) {
		...
		if (RemoveNick(pChannel->m_sName, sNick)) {
			if (!thePrefs.GetIRCIgnoreQuitMessages())
				m_pParent->AddInfoMessageCF(pChannel->m_sName, GetCustomSysColor(COLOR_IRC_EVENT_MSG), GetResString(_T("IRC_HASQUIT")), (LPCTSTR)sNick, (LPCTSTR)sMessage);
		}
	}
}
	...
	for (POSITION pos = ChanSel.m_lstChannels.GetHeadPosition(); pos != NULL;) {
		...
		if (ChangeNick(pChannel->m_sName, sOldNick, sNewNick)) {
			if (!thePrefs.GetIRCIgnoreMiscMessages())
				m_pParent->AddInfoMessageF(pChannel->m_sName, GetResString(_T("IRC_NOWKNOWNAS")), (LPCTSTR)sOldNick, (LPCTSTR)sNewNick);
			bChanged = true;
		}
	}
	return bChanged;
...
{
	const CString &strRes(GetResString(_T("STATUS")));
	HDITEM hdi;
	hdi.pszText = const_cast<LPTSTR>((LPCTSTR)strRes);
	hdi.mask = HDI_TEXT;
...
BOOL CIrcNickListCtrl::OnCommand(WPARAM wParam, LPARAM)
{
	...
	case Irc_Slap:
		if (pNick && pChannel) {
			CString sSend;
			sSend.Format(GetResString(_T("IRC_SLAPMSGSEND")), (LPCTSTR)pChannel->m_sName, (LPCTSTR)pNick->m_sNick);
			m_pParent->AddInfoMessageF(pChannel->m_sName, GetResString(_T("IRC_SLAPMSG")), (LPCTSTR)m_pParent->m_pIrcMain->GetNick(), (LPCTSTR)pNick->m_sNick);
			m_pParent->m_pIrcMain->SendString(sSend);
		}
		break;
	case Irc_AddFriend:
