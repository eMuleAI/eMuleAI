...
{
	if (m_hWnd) {
		SetWindowText(GetResString(_T("SCHEDULER")));

		SetDlgItemText(IDC_ENABLE, GetResString(_T("ENABLED")));
		SetDlgItemText(IDC_S_ENABLE, GetResString(_T("ENABLED")));
		SetDlgItemText(IDC_STATIC_S_TITLE, GetResString(_T("TITLE")));
		SetDlgItemText(IDC_STATIC_DETAILS, GetResString(_T("DETAILS")));
		SetDlgItemText(IDC_STATIC_S_TIME, GetResString(_T("TIME")));

		SetDlgItemText(IDC_STATIC_S_ACTION, GetResString(_T("ACTION")));
		SetDlgItemText(IDC_APPLY, GetResString(_T("PW_APPLY")));
		SetDlgItemText(IDC_REMOVE, GetResString(_T("REMOVE")));
		SetDlgItemText(IDC_NEW, GetResString(_T("NEW")));
		SetDlgItemText(IDC_CHECKNOENDTIME, GetResString(_T("CHECKNOENDTIME")));
...
CString CPPgScheduler::GetActionLabel(int index)
{
	...
	case ACTION_SOURCESL:
		...
		break;
	case ACTION_CON5SEC:
		uid = _T("LIMITCONS5SEC");
		break;
	case ACTION_CATSTOP:
		uid = _T("SCHED_CATSTOP");
		break;
	case ACTION_CATRESUME:
		uid = _T("SCHED_CATRESUME");
		break;
	case ACTION_CONS:
		uid = _T("PW_MAXC");
		break;
...
void CPPgScheduler::OnNmRClickActionlist(LPNMHDR, LRESULT *pResult)
{
	...
	CMenuXP m_ActionMenu;
	CMenuXP m_ActionSel;
	CMenuXP m_CatActionSel;
	...
	m_ActionSel.AppendMenu(MF_STRING, MP_SCHACTIONS + ACTION_SETUPL, GetResString(_T("PW_UPL")));
	m_ActionSel.AppendMenu(MF_STRING, MP_SCHACTIONS + ACTION_SETDOWNL, GetResString(_T("PW_DOWNL")));
	m_ActionSel.AppendMenu(MF_STRING, MP_SCHACTIONS + ACTION_SOURCESL, GetResString(_T("LIMITSOURCES")));
	m_ActionSel.AppendMenu(MF_STRING, MP_SCHACTIONS + ACTION_CON5SEC, GetResString(_T("LIMITCONS5SEC")));
	m_ActionSel.AppendMenu(MF_STRING, MP_SCHACTIONS + ACTION_CONS, GetResString(_T("PW_MAXC")));
	m_ActionSel.AppendMenu(MF_STRING, MP_SCHACTIONS + ACTION_CATSTOP, GetResString(_T("SCHED_CATSTOP")));
	m_ActionSel.AppendMenu(MF_STRING, MP_SCHACTIONS + ACTION_CATRESUME, GetResString(_T("SCHED_CATRESUME")));

	m_ActionMenu.AddMenuSidebar(GetResString(_T("ACTION")));
	m_ActionMenu.AppendMenu(MF_POPUP, (UINT_PTR)m_ActionSel.m_hMenu, GetResString(_T("ADD")));
	...
	m_ActionMenu.AppendMenu(nFlag, MP_CAT_REMOVE, GetResString(_T("REMOVE")));
