...
#include "PartFile.h"
#include "zlib/zlib.h"
#include "OtherFunctions.h" // Long path helpers
...
void CArchiveRecovery::recover(CPartFile *partFile, bool preview, bool bCreatePartFileCopy)
{
	...
	AddLogLine(true, _T("%s \"%s\""), (LPCTSTR)GetResString(_T("ATTEMPTING_RECOVERY")), (LPCTSTR)EscPercent(partFile->GetFileName()));
	...
	if (!AfxBeginThread(run, (LPVOID)tp, THREAD_PRIORITY_IDLE)) {
		...
		LogError(LOG_STATUSBAR, _T("%s \"%s\""), (LPCTSTR)GetResString(_T("RECOVERY_FAILED")), (LPCTSTR)partFile->GetFileName());
...
UINT AFX_CDECL CArchiveRecovery::run(LPVOID lpParam)
{
	...
	if (!performRecovery(tp->partFile, tp->filled, tp->preview, tp->bCreatePartFileCopy))
		if (theApp.IsClosing() || tp->partFile == NULL) {
			DeleteMemory(tp);
			return 0;
		}
		theApp.QueueLogLine(true, _T("%s \"%s\""), (LPCTSTR)GetResString(_T("RECOVERY_FAILED")), (LPCTSTR)tp->partFile->GetFileName());
	...
	, bool preview, bool bCreatePartFileCopy)
{
	...
	try {
		...
		if (bCreatePartFileCopy) {
			...
			if (!temp.Open(tempOpenPath, CFile::modeRead | CFile::shareDenyWrite))
				return false;
		} else {
			...
			if (!temp.Open(srcOpenPath, CFile::modeRead | CFile::shareDenyNone))
				return false;
		}
		...
		if (output.Open(outOpenPath, CFile::modeWrite | CFile::shareDenyWrite | CFile::modeCreate)) {
		...
		// Report success
		if (success) {
			theApp.QueueLogLine(true, _T("%s \"%s\""), (LPCTSTR)GetResString(_T("RECOVERY_SUCCESSFUL")), (LPCTSTR)partFile->GetFileName());
...
#define SECSIZE sizeof(ISO_PVD_s)
	...
	ISOReadDirectory(aitp, LODWORD(rootdir.sector1OfExtension), isoInput, EMPTY);
...
void CArchiveRecovery::LogRecovered(INT_PTR fileCount)
{
	...
	CString msg;
	if (fileCount == 1)
		msg = GetResString(_T("RECOVER_SINGLE"));
	else
		msg.Format(GetResString(_T("RECOVER_MULTIPLE")), (unsigned)fileCount);
