	...
	, m_uAutoCloseTimer()
{
	...
	m_bAutoClose = CPreferences::bMiniMuleAutoClose;
	m_uWndTransparency = CPreferences::iMiniMuleTransparency;
...
BOOL CMiniMule::OnInitDialog()
{
	...
	SetWindowText(theApp.GetAppVersion());
	if (IsDarkModeEnabled())
		ApplyThemeToWindow(m_hWnd, true, false);
...
void CMiniMule::Localize()
{
	...
	if (a) {
		a->put_title(CComBSTR(GetResNoAmp(_T("OPENINC"))));
		a.Release();
	}
	...
	if (a) {
		a->put_title(CComBSTR(GetResNoAmp(_T("MAIN_POPUP_RESTORE"))));
		a.Release();
	}
	...
	if (img) {
		img->put_alt(CComBSTR(GetResNoAmp(_T("OPENINC"))));
		img.Release();
	}
	...
	if (img) {
		img->put_alt(CComBSTR(GetResNoAmp(_T("MAIN_POPUP_RESTORE"))));
		img.Release();
	}
}
...
void CMiniMule::UpdateContent(UINT uUpDatarate, UINT uDownDatarate)
{
	...
	SetElementHtml(_T("connected"), CComBSTR(GetResString(theApp.IsConnected() ? _T("YES") : _T("NO"))));
